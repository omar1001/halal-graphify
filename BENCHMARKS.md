# halal-graphify Benchmarks

How halal-graphify performs as conversational long-term memory and as a
...
halal-graphify's deterministic graph plus hybrid retrieval has the best retrieval
...
| Suite | Dataset (n) | Metric | halal-graphify | Field |
|---|---|---|---|---|
...
halal-graphify's own harness. Competing systems (mem0, supermemory) are run as
...
- Memory suite (`memory/`): halal-graphify's graph retrieval vs dedicated memory
...
|---|---|---|---|
| **halal-graphify** (graph-expand) | **45.3%** | **0.497** | ~$1.40 |
| hybrid RRF | 43.3% | 0.493 | $0 (shared index) |
| halal-graphify (SurrealDB engine) | 43.3% | 0.485 | $0 (shared index) |
| dense RAG | 41.3% | 0.439 | $0 (shared index) |
| BM25 | 31.3% | 0.362 | $0 (shared index) |
| supermemory | 49.7% | 0.149* | $15.67 |
| mem0 | 27.3% | 0.048 | $3.48 |

Bold marks halal-graphify's primary configuration, not the column maximum. Baselines
...
ingest cost ($15.67 vs $1.40) and with about 3x worse retrieval recall. halal-graphify
...
|---|---|---|
| **halal-graphify** (graph-expand) | **76%** | **0.844** |
| dense RAG | 76% | 0.848 |
| halal-graphify (SurrealDB engine) | 74% | 0.833 |
| hybrid RRF | 74% | 0.822 |
| BM25 | 70% | 0.710 |
| mem0 | 70% | 0.344 |

halal-graphify ties dense RAG for the best QA accuracy (76%); dense RAG edges it on
...
halal-graphify tool lifts key-fact coverage across the graded question set (n=6) from
70.8% (a grep and read baseline) to 82.0%, at about 140K tokens per query.
halal-graphify pays for itself in accuracy against searching raw files, and avoids the
...
- Graph construction costs zero LLM credits. halal-graphify extracts with tree-sitter
...
- Memory ingest is about 11x cheaper: halal-graphify's LOCOMO ingest runs around
