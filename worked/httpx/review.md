...
**Pipeline:** halal-graphify AST extractor + graph_builder + Leiden clusterer + analyzer + reporter
**Method:** Full deterministic code tracing of every halal-graphify source module against
...
## Hub Nodes (most connected - your core abstractions)
...
### 5. Hub Nodes - Score: 7/10

**Are the most-connected nodes actually the core abstractions?**

**Very good:**
- `client.py` as #1 hub node makes sense - it imports from 5 other modules and contains the
...
- The hub nodes list would be more useful if file-level hub nodes were filtered out or
  labeled as "module" rather than "hub node". The real hub nodes are `BaseClient`, `Response`,
...
### Issue 2: File nodes dominate Hub Nodes (MODERATE)
**Location:** `analyzer.py` hub_nodes(), `ast_extractor.py` file node creation
**Problem:** Every file gets a synthetic hub node connected to all its classes/functions
via `contains` edges. This makes file nodes always appear as hub nodes. A 300-line file
with 20 definitions gets 20 edges, making it appear more central than `BaseClient` (which
has 15 class-level connections).
**Fix:** Exclude nodes whose `label` ends in `.py` from hub_node ranking, or subtract
...
| Hub nodes | 7/10 | Core abstractions identified; file hub nodes dominate misleadingly |
...
The halal-graphify AST extractor is deterministic, fast, and accurate for what it extracts.
...
- File hub nodes excluded from Hub Nodes ranking
