...
A small mixed-input corpus: Python source files, a markdown paper with arXiv citations, and one image. Tests halal-graphify on different file types in a single run.

## Corpus (5 files)

```
raw/
├── analyze.py          — graph analysis module (hub_nodes, surprising_connections)
...
- Hub nodes: `analyze.py`, `cluster.py`, `build.py`
