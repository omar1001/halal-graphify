...
|-----------|-------|----------|-------|
| 0 | Graph Analysis | 0.22 | analyze.py, `hub_nodes()`, `surprising_connections()`, `suggest_questions()`, `graph_diff()`, `_is_concept_node()`, `_is_file_node()`, `_cross_*()` |
...
**Finding:** Communities are semantically correct - the three halal-graphify modules map cleanly
...
- `graph_diff()` lives in analyze.py alongside `hub_nodes()` and `surprising_connections()`
...
**What halal-graphify extracts (Claude vision reads directly, no reshaper/bidi needed):**
...
**Nodes halal-graphify would extract:**
...
identically to English. The image node in halal-graphify is just a path - the vision subagent does
the rest.
...
list. The function requires more signal (AMBIGUOUS edges, bridge nodes, underexplored hub nodes)
to generate questions - correct behavior, but the skill should handle the empty case gracefully.

### Issue 2: Hub nodes empty when all nodes are file-level (MINOR)
`hub_nodes()` correctly excludes file hub nodes. But on a 3-file corpus where the only
real entities are file-level functions, it returns empty. The evaluation fell back to showing
degree-ranked nodes manually. Fix: emit a notice ("corpus too small for meaningful hub nodes")
...
Primary gaps are edge-level semantics (no INFERRED edges from AST-only) and hub_nodes/
