...
1. `halal-graphify extract` rebuilds the full graph from scratch every run — re-sends all files to the LLM regardless of what changed. For a 1000-file Markdown corpus updated daily this is expensive.
...
## Pipeline (every `halal-graphify extract` run)
...
cluster (full graph, always re-run)
    ↓
score_all + hub_nodes + surprising_connections
    ↓
...
### Output summary
```
[halal-graphify extract] incremental: 20 changed, 980 cached, 2 deleted
[halal-graphify extract] graph: 4,821 nodes, 12,304 edges, 43 communities
[halal-graphify extract] tokens: 18,432 in / 6,201 out, est. cost: $0.08
```

### Files changed
- `halal_graphify/__main__.py` — `elif cmd == "extract":` block only (~5 targeted changes)

---

## Feature 2: Entity Deduplication

### New module: `halal_graphify/dedup.py`
...
### Files changed
- `halal_graphify/dedup.py` — new module, full pipeline
- `halal_graphify/build.py` — call `deduplicate_entities` after graph construction; wire dormant `deduplicate_by_label`
- `halal_graphify/__main__.py` — add `--dedup-llm` flag parsing in extract block
...
- Incremental support for `halal-graphify update` (AST-only) — already handled by existing AST cache
