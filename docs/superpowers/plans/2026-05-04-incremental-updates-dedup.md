...
**Goal:** Add semantic cache + incremental graph updates to `halal-graphify extract`, and add a new `halal_graphify/dedup.py` module that runs MinHash/LSH + Jaro-Winkler entity deduplication before clustering on every run.

**Architecture:** Two independent features wired into the same pipeline: (1) `__main__.py` extract block uses `check_semantic_cache`/`save_semantic_cache` + `detect_incremental` + `build_merge` for incremental runs; (2) new `halal_graphify/dedup.py` implements the full dedup pipeline called from `build.py` after graph construction and before `cluster()`.

**Tech Stack:** Python 3.10+, `datasketch` (MinHash/LSH), `rapidfuzz` (Jaro-Winkler), `networkx` (union-find via `nx.utils.UnionFind`), existing `halal_graphify.cache`, `halal_graphify.detect`, `halal_graphify.build`.
...
|---|---|---|
| `halal_graphify/dedup.py` | **Create** | Full dedup pipeline: entropy gate → MinHash/LSH → Jaro-Winkler → community boost → union-find merge |
| `halal_graphify/build.py` | **Modify** | Call `deduplicate_entities()` from `build()` and `build_merge()`; wire dormant `deduplicate_by_label` |
| `halal_graphify/__main__.py` | **Modify** | Semantic cache wrapping, incremental mode auto-detection, `build_merge` swap, manifest save, `--dedup-llm` flag |
...
## Task 2: Create `halal_graphify/dedup.py` — entropy gate + MinHash/LSH + Jaro-Winkler

**Files:**
- Create: `halal_graphify/dedup.py`
...
```python
"""Tests for halal_graphify/dedup.py entity deduplication pipeline."""
from __future__ import annotations
import pytest
from halal_graphify.dedup import deduplicate_entities, _entropy, _shingles
...
Expected: all fail with `ModuleNotFoundError: No module named 'halal_graphify.dedup'`

- [ ] **Step 3: Create `halal_graphify/dedup.py`**

```python
"""Entity deduplication pipeline for halal-graphify knowledge graphs.
...
def deduplicate_entities(
    ...
    *,
    ...
    total = len(remap)
    msg = f"[halal-graphify] Deduplicated {total} node(s)"
    if exact_merges:
...
```bash
git add halal_graphify/dedup.py tests/test_dedup.py
git commit -m "Add halal_graphify/dedup.py: entropy gate + MinHash/LSH + Jaro-Winkler entity deduplication"
```
...
**Files:**
- Modify: `halal_graphify/build.py` (lines 119-137 `build()`, lines 191-244 `build_merge()`)
...
def test_build_calls_dedup():
    ...
    from halal_graphify.build import build
    chunk1 = {
...
def build(extractions: list[dict], *, directed: bool = False, dedup: bool = True) -> nx.Graph:
    ...
    """
    from halal_graphify.dedup import deduplicate_entities
    combined: dict = {"nodes": [], "edges": [], "hyperedges": [], "input_tokens": 0, "output_tokens": 0}
    for ext in extractions:
        combined["nodes"].extend(ext.get("nodes", []))
        combined["edges"].extend(ext.get("edges", []))
...
```python
    if graph_path.exists() and not dedup and not prune_sources:
        ...
        if new_n < existing_n:
            raise ValueError(
                f"halal-graphify: build_merge would shrink graph from {existing_n} → {new_n} nodes. "
...
```bash
git add halal_graphify/build.py
...
**Files:**
- Modify: `halal_graphify/__main__.py` (lines 1971–2117, the extract block)
...
```python
"""Integration tests for incremental halal-graphify extract behavior."""
from __future__ import annotations
import json
import subprocess
import sys
...
Find line 1971 (the `from halal_graphify.detect import detect as _detect` line). Replace the detect + file-list block (lines 1971–1984) with:
        ...
        if semantic_files:
            ...
            if uncached_paths:
                ...
                try:
                    ...
                    print(f"[halal-graphify extract] semantic extraction failed: {exc}", file=sys.stderr)
...
```python
        from halal_graphify.build import build_from_json as _build_from_json
        ...
...
```bash
git add halal_graphify/__main__.py tests/test_incremental.py
git commit -m "Add incremental updates to halal-graphify extract: semantic cache + build_merge + manifest"
```
...
**Files:**
- Modify: `halal_graphify/dedup.py`
...
Update the signature in `halal_graphify/dedup.py`:
...
def _llm_tiebreak(
    ...
    *,
    ...
    try:
        from halal_graphify.llm import extract_corpus_parallel as _llm  # noqa: F401
        import os
        from halal_graphify.llm import BACKENDS
        env_key = BACKENDS.get(backend, {}).get("env_key", "")
        if not os.environ.get(env_key):
            print(f"[halal-graphify] --dedup-llm: {env_key} not set, skipping LLM tiebreaker.", flush=True)
            return
    except ImportError:
        return
    ...
    try:
        from halal_graphify.llm import _call_llm
    except ImportError:
        return
    ...
    for batch_start in range(0, len(ambiguous), batch_size):
        ...
        try:
            ...
            print(f"[halal-graphify] --dedup-llm batch failed: {exc}", flush=True)
```
...
- Feat: `halal-graphify extract` now runs incrementally — auto-detects prior `manifest.json` and re-extracts only changed/new files; semantic results cached by content hash so unchanged docs cost zero LLM tokens on repeat runs (#698)
...
- Feat: `--dedup-llm` flag for `halal-graphify extract` — optional LLM tiebreaker for ambiguous entity pairs (~$0.01 for 10k-node graphs), off by default
