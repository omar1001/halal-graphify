...
This RFC proposes an optional way for halal-graphify to expose compact file-level
...
`halal-graphify query`, `halal-graphify explain`, MCP node lookup, and graph navigation.

## Goals

- Help agents choose relevant files with less context.
- Preserve halal-graphify's offline, deterministic behavior by default.
...
- Replacing `halal-graphify explain`; summaries should make `explain` more useful.
...
- Display a summary in `halal-graphify explain <node>` when one exists.
- Include summaries in `halal-graphify serve` / MCP node lookup when one exists.
...
{
  "label": "extract.py",
  "source_file": "halal_graphify/extract.py",
...
be fetched through `halal-graphify explain`, `halal-graphify query`, or direct file reads
when needed.
...
```bash
halal-graphify . --summarize-nodes
halal-graphify explain "extract.py"
```
...
3. Surface summaries in `halal-graphify explain`.
4. Surface summaries in `halal-graphify serve` / MCP node lookup.
...
- Extend summaries to class or module-level nodes if file-level summaries prove
  useful.
- Allow `halal-graphify query` to include summaries for returned nodes under a budget.
- Add cache/freshness metadata if summaries are generated separately from the
  main graph.

## Questions for maintainers and users

1. Should halal-graphify prefer one artifact (`graph.json`) or keep generated text in a
   sidecar?
2. Should deterministic file-level summaries be generated during graph creation,
   or only through an explicit command such as `halal-graphify summarize`?
