...
**Umiejętność dla asystenta kodowania AI.** Wpisz `/halal-graphify` w Claude Code, Codex, OpenCode, Cursor, Gemini CLI, GitHub Copilot CLI, VS Code Copilot Chat, Aider, OpenClaw, Factory Droid, Trae, Hermes, Kiro lub Google Antigravity — czyta Twoje pliki, buduje graf wiedzy i zwraca Ci strukturę, o której nie wiedziałeś, że istnieje. Rozumiej bazę kodu szybciej. Znajdź „dlaczego" za decyzjami architektonicznymi.

W pełni multimodalny. Dodaj kod, PDF, markdown, zrzuty ekranu, diagramy, zdjęcia tablic, obrazy w innych językach lub pliki wideo i audio — halal-graphify wyodrębnia koncepcje i relacje ze wszystkiego i łączy je w jeden graf. Wideo są transkrybowane lokalnie za pomocą Whisper. Obsługuje 25 języków programowania przez tree-sitter AST.

> Andrej Karpathy prowadzi folder `/raw`, gdzie wrzuca artykuły, tweety, zrzuty ekranu i notatki. halal-graphify jest odpowiedzią na ten problem — **71,5x** mniej tokenów na zapytanie w porównaniu z czytaniem surowych plików, trwały między sesjami.

```
/halal-graphify .                        # działa na dowolnym folderze
```
...
> **Oficjalny pakiet:** Pakiet PyPI nazywa się `halal-graphify`. Jedyne oficjalne repozytorium to [safishamsi/graphify](https://github.com/safishamsi/graphify).

## Użycie

```
/halal-graphify .
/halal-graphify ./raw --update           # tylko zmienione pliki
/halal-graphify ./raw --mode deep
/halal-graphify query "co łączy Attention z optymalizatorem?"
...
## Zbudowane na halal-graphify — Penpax

[**Penpax**](https://safishamsi.github.io/penpax.ai) to warstwa enterprise nad halal-graphify. **Bezpłatna wersja próbna wkrótce.** [Dołącz do listy oczekujących →](https://safishamsi.github.io/penpax.ai)
