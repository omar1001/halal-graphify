...
**Навык для AI-ассистента по написанию кода.** Введите `/halal-graphify` в Claude Code, Codex, OpenCode, Cursor, Gemini CLI, GitHub Copilot CLI, VS Code Copilot Chat, Aider, OpenClaw, Factory Droid, Trae, Hermes, Kiro или Google Antigravity — он прочитает ваши файлы, построит граф знаний и вернёт вам структуру, о существовании которой вы не подозревали. Понимайте кодовую базу быстрее. Находите «почему» за архитектурными решениями.

Полностью мультимодальный. Добавляйте код, PDF, markdown, скриншоты, диаграммы, фотографии досок, изображения на других языках, видео и аудиофайлы — halal-graphify извлекает концепции и связи из всего этого и объединяет их в один граф. Видео транскрибируются локально с Whisper, используя доменный промпт из вашего корпуса. Поддерживается 25 языков программирования через tree-sitter AST (Python, JS, TS, Go, Rust, Java, C, C++, Ruby, C#, Kotlin, Scala, PHP, Swift, Lua, Zig, PowerShell, Elixir, Objective-C, Julia, Verilog, SystemVerilog, Vue, Svelte, Dart).

> Андрей Карпати ведёт папку `/raw`, куда складывает статьи, твиты, скриншоты и заметки. halal-graphify — ответ на эту проблему: в **71,5 раза** меньше токенов на запрос по сравнению с чтением сырых файлов, сохранение между сессиями, честность относительно того, что найдено, а что выведено.

```
/halal-graphify .                        # работает с любой папкой — код, заметки, статьи, всё что угодно
```
...
> **Официальный пакет:** Пакет PyPI называется `halal-graphify` (установить через `pip install halal-graphify`). Другие пакеты с именем `halal-graphify*` на PyPI не связаны с этим проектом. Единственный официальный репозиторий — [safishamsi/graphify](https://github.com/safishamsi/graphify).
...
| Claude Code (Windows) | `halal-graphify install` (авто-определение) или `halal-graphify install --platform windows` |
...
Примечание: Codex использует `$` вместо `/` для навыков, поэтому вводите `$halal-graphify .`.
...
```
/halal-graphify                          # текущая директория
/halal-graphify ./raw                    # конкретная папка
/halal-graphify ./raw --mode deep        # более агрессивное извлечение INFERRED-рёбер
/halal-graphify ./raw --update           # повторно извлечь только изменённые файлы
/halal-graphify ./raw --directed         # направленный граф
/halal-graphify ./raw --cluster-only     # перезапустить кластеризацию на существующем графе
/halal-graphify ./raw --no-viz           # без HTML, только отчёт + JSON
/halal-graphify ./raw --obsidian         # создать Obsidian vault (opt-in)

/halal-graphify add https://arxiv.org/abs/1706.03762   # получить статью
/halal-graphify add <video-url>                         # скачать аудио, транскрибировать, добавить
/halal-graphify query "что связывает Attention с оптимизатором?"
/halal-graphify path "DigestAuth" "Response"
/halal-graphify explain "SwinTransformer"

halal-graphify hook install              # установить Git-хуки
halal-graphify update ./src              # повторно извлечь файлы кода, без LLM
halal-graphify watch ./src               # автоматическое обновление графа
```
...
**Git-хуки** (`halal-graphify hook install`) — устанавливает post-commit и post-checkout хуки.

## Конфиденциальность

halal-graphify отправляет содержимое файлов в API модели вашего AI-ассистента для семантического извлечения из документов, статей и изображений. Файлы кода обрабатываются локально через tree-sitter AST. Видео и аудиофайлы транскрибируются локально с faster-whisper. Никакой телеметрии, никакого отслеживания использования.
...
## Построено на halal-graphify — Penpax

[**Penpax**](https://safishamsi.github.io/penpax.ai) — корпоративный слой поверх halal-graphify. Там, где halal-graphify превращает папку с файлами в граф знаний, Penpax применяет тот же граф ко всей вашей рабочей жизни — непрерывно.
