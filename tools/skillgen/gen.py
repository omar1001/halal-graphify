"""skillgen: render halal-graphify's committed skill artifacts from edited fragments.
...
files under ``halal_graphify/skill*.md`` and ``halal_graphify/skills/<platform>/references/``
...
except ModuleNotFoundError:  # Python 3.10 - halal-graphify supports >=3.10
...
# straight from git instead of from disk. claude's v8 body is halal_graphify/skill.md;
# every other split host has its own halal_graphify/skill-<host>.md. Auditing each host
...
def _v8_baseline_ref(platform_key: str) -> str:
    ...
    if platform_key == "claude":
        return f"{_V8_BASELINE_SHA}:halal_graphify/skill.md"
    if platform_key == "agents":
        ...
        return f"{_V8_BASELINE_SHA}:halal_graphify/skill-amp.md"
    return f"{_V8_BASELINE_SHA}:halal_graphify/skill-{platform_key}.md"

# Immutable baseline for --always-on-roundtrip. The six always-on instruction
# blocks used to be triple-quoted constants in halal_graphify/__main__.py; they are now
# packaged halal_graphify/always_on/*.md files the module reads at load. This ref points
...
ALWAYS_ON_BASELINE_REF = f"{_V8_BASELINE_SHA}:halal_graphify/__main__.py"

# The always-on instruction blocks: rendered-file basename -> the __main__.py
# constant it must reproduce. Rendered to halal_graphify/always_on/<basename>.md from
...
ALWAYS_ON_SANCTIONED_EDITS: dict[str, tuple[tuple[str, str], ...]] = {
    ...
    # invoke a literal `skill` tool with `skill: "halal-graphify"`, which is
...
# always-on via `halal-graphify claude install` (the shared body). The agents-md hosts
# (trae, trae-cn, amp) read AGENTS.md and wire it via `halal-graphify <host> install`.
...
_TRAE_PRETOOLUSE_NOTE = (
    ...
    "rebuild on tool use. Run `/halal-graphify --update` manually after code changes if "
...
_AGENTS_MD_HOOKS: dict[str, dict[str, str]] = {
    "trae": {
        "heading_suffix": " (Trae)",
        "host_display": "Trae",
        "install_block": "halal-graphify trae install       # or: halal-graphify trae-cn install",
        "uninstall_block": "halal-graphify trae uninstall     # or: halal-graphify trae-cn uninstall   # remove the section",
        "pretooluse_note": _TRAE_PRETOOLUSE_NOTE,
    },
    "amp": {
        "heading_suffix": "",
        "host_display": "Amp",
        "install_block": "halal-graphify amp install",
        "uninstall_block": "halal-graphify amp uninstall  # remove the section",
        "pretooluse_note": "",
    },
    "agents": {
        ...
        # pointing at `halal-graphify agents install` (which wires AGENTS.md, like amp).
        "heading_suffix": "",
        "host_display": "your agent",
        "install_block": "halal-graphify agents install",
        "uninstall_block": "halal-graphify agents uninstall  # remove the section",
        "pretooluse_note": "",
    },
}
...
# them. Two classes:
#
# 1. SHARED_INTRO_ALLOWLIST — the lean intro consolidation. "## What halal-graphify is
...
SHARED_INTRO_ALLOWLIST: frozenset[str] = frozenset({
    "## What halal-graphify is for",  # lean intro; v8 hosts had verbose intro prose, no heading.
})
...
def load_platforms() -> dict[str, Platform]:
    ...
    for key, cfg in data.get("platform", {}).items():
        out[key] = Platform(
            ...
            name=cfg.get("name", "halal-graphify"),
...
def _translate_bash_block(lines: list[str]) -> list[str]:
    ...
    for line in lines:
        ...
        elif not line.strip() or line.lstrip().startswith("#") or line.startswith("halal-graphify "):
            out.append(line)  # blank lines, comments, and halal-graphify CLI calls are shell-neutral
        else:
...
def render_always_on() -> list[RenderedArtifact]:
    """Render the six always-on instruction blocks to halal_graphify/always_on/*.md.
    ...
    for basename in sorted(ALWAYS_ON_BLOCKS):
        ...
        out.append(RenderedArtifact(f"halal_graphify/always_on/{basename}.md", body))
    return out
...
def audit_coverage(platform: Platform) -> list[str]:
    ...
    The audit reads the host's OWN v8 skill body (halal_graphify/skill.md for claude,
    halal_graphify/skill-<host>.md otherwise) and checks that every v8 heading lands in
    ...
    v8 headings are exempt and documented as deltas, not holes:
      - waves 2-3 consolidations (the lean "## What halal-graphify is for" intro and the
...
def _is_frontmatter_description_line(line: str) -> bool:
    ...
    The unified description (halal-graphify #1106) rewrites the frontmatter
...
def _is_chunk_cleanup_line(line: str) -> bool:
    ...
    error there, unlike bash). The fix (halal-graphify #1172) drops the glob from the
...
def _is_manifest_root_fix_line(line: str) -> bool:
    ...
    relativized to the scan root, matching the native ``halal-graphify update`` path.
    Both the old bare call (removed) and the new rooted call (added) match here;
    the ``import`` guard avoids matching the ``from halal_graphify.detect import
    save_manifest`` line.
    """
...
def _is_manifest_stamp_fix_line(line: str) -> bool:
    ...
    ``halal-graphify extract`` path. The rooted ``save_manifest`` call itself is
...
def _is_no_api_key_fix_line(line: str) -> bool:
    ...
    stating that halal-graphify needs no API key, and (like the subagent-host skills)
    ...
    """
    return "halal-graphify needs no API key" in line
...
def _is_shebang_allowlist_fix_line(line: str) -> bool:
    ...
    detection fell through to a bare ``python3`` that lacked halal-graphify. ``@`` is now
...
def _is_obsidian_usage_comment_line(line: str) -> bool:
    """Whether a line is part of the ``/halal-graphify`` usage-comment fix (#1681).

    The Usage block's bare ``/halal-graphify`` comment said "full pipeline on current
...
def _is_uv_from_interpreter_fix_line(line: str) -> bool:
    ...
    Step 1's POSIX interpreter probe ran ``uv tool run halal-graphify python -c ...``,
    but ``halal-graphify`` exposes its executable as ``halal-graphify``, so uv treated
    ``python`` as a missing ``halal-graphify`` command and the probe silently failed
    (the ``2>/dev/null`` swallowed uv's "use --from" hint), leaving PYTHON on a
    graphify-less system interpreter. The probe now runs
    ``uv tool run --from halal-graphify python -c ...``. Both the old (removed) and new
    (added) forms match here.
    """
    return "uv tool run" in line and "halal-graphify python" in line
...
def _is_community_label_export_fix_line(line: str) -> bool:
    ...
    return (
        ...
        or stripped == "from halal_graphify.export import to_json"
...
def always_on_roundtrip() -> list[str]:
    ...
    for basename, const_name in sorted(ALWAYS_ON_BLOCKS.items()):
        path = f"halal_graphify/always_on/{basename}.md"
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        ...
        description="Render and guard halal-graphify's committed skill artifacts.",
    )
