```powershell
# Detect Python with halal-graphify — uv/pipx-aware (fixes #831)
...
function Find-GraphifyPython {
    ...
    if (Get-Command uv -ErrorAction SilentlyContinue) {
        ...
        if ($uvDir) {
            $py = Join-Path $uvDir "halal-graphify\Scripts\python.exe"
            if (Test-Path $py) {
                & $py -c "import halal_graphify" 2>$null
    ...
    if (Get-Command pipx -ErrorAction SilentlyContinue) {
        ...
        if ($venvs) {
            $py = Join-Path $venvs "halal-graphify\Scripts\python.exe"
            if (Test-Path $py) {
                & $py -c "import halal_graphify" 2>$null
                if ($LASTEXITCODE -eq 0) { return $py }
            }
    ...
    if ($pyCmd) {
        & $pyCmd.Source -c "import halal_graphify" 2>$null
...
if (-not $GRAPHIFY_PYTHON) {
    if (Get-Command uv -ErrorAction SilentlyContinue) {
        uv tool install --upgrade halal-graphify -q 2>&1 | Select-Object -Last 3
    } else {
        pip install halal-graphify -q 2>&1 | Select-Object -Last 3
    }
    $GRAPHIFY_PYTHON = Find-GraphifyPython
}
...
# Save scan root so `halal-graphify update` (no args) knows where to look next time
(Resolve-Path INPUT_PATH).Path | Out-File -FilePath graphify-out\.graphify_root -Encoding utf8 -NoNewline
```

If the import succeeds, print nothing and move straight to Step 2.

**In every subsequent block, run Python through the saved interpreter — `& (Get-Content graphify-out\.graphify_python)` in place of a bare `python3` — so every step uses the interpreter that actually has halal-graphify.**
