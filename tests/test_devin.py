"""Tests for halal-graphify devin install / uninstall commands."""
...
def _skill_path_user(tmp_path):
    return tmp_path / ".config" / "devin" / "skills" / "halal-graphify" / "SKILL.md"


def _skill_path_project(project_dir):
    return project_dir / ".devin" / "skills" / "halal-graphify" / "SKILL.md"
...
# ---------------------------------------------------------------------------
# User-scope install (halal-graphify install --platform devin / halal-graphify devin install)
# ---------------------------------------------------------------------------

def test_devin_install_user_creates_skill_file(tmp_path):
    """User-scope install copies skill to ~/.config/devin/skills/halal-halal_graphify/SKILL.md."""
...
# ---------------------------------------------------------------------------
# Project-scope install (halal-graphify devin install --project)
# ---------------------------------------------------------------------------

def test_devin_install_project_creates_skill_file(tmp_path, monkeypatch):
    """Project-scope install copies skill to .devin/skills/halal-halal_graphify/SKILL.md."""
    from halal_graphify.__main__ import main
    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
    monkeypatch.chdir(project)
    monkeypatch.setattr(sys, "argv", ["halal-graphify", "devin", "install", "--project"])
...
def test_devin_install_project_creates_rules_file(tmp_path, monkeypatch):
    ...
    assert rules.exists()
    assert "halal-graphify" in rules.read_text()
...
def test_devin_rules_content_recommends_graphify_query(tmp_path):
    ...
    from halal_graphify.__main__ import _devin_rules_install
    _devin_rules_install(tmp_path)
    content = _rules_path(tmp_path).read_text()
    assert "halal-graphify query" in content
...
def test_devin_uninstall_user_removes_skill_file(tmp_path):
    ...
    from halal_graphify.__main__ import _remove_skill_file
    with patch("halal_graphify.__main__.Path.home", return_value=tmp_path):
...
def test_devin_uninstall_user_noop_when_not_installed(tmp_path, capsys):
    ...
    try:
        os.chdir(tmp_path)
        with patch("halal_graphify.__main__.Path.home", return_value=tmp_path):
            sys.argv = ["halal-graphify", "devin", "uninstall"]
            main()
    finally:
        os.chdir(old_cwd)
    out = capsys.readouterr().out
...
def test_devin_uninstall_project_removes_skill_file(tmp_path, monkeypatch):
    """Project-scope uninstall removes .devin/skills/halal-halal_graphify/SKILL.md."""
    from halal_graphify.__main__ import main
    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
    ...
    with patch("halal_graphify.__main__.Path.home", return_value=home):
        monkeypatch.setattr(sys, "argv", ["halal-graphify", "devin", "install", "--project"])
        main()
        monkeypatch.setattr(sys, "argv", ["halal-graphify", "devin", "uninstall", "--project"])
        main()
...
def test_devin_rules_uninstall_noop_when_not_installed(tmp_path):
    ...
    from halal_graphify.__main__ import _devin_rules_uninstall
...
def test_devin_skill_file_exists_in_package():
    ...
    skill = Path(halal_graphify.__file__).parent / "skill-devin.md"
...
def test_devin_skill_file_uses_python_c_syntax():
    ...
    All mature halal-graphify skills use the interpreter-detection pattern
    ``$(cat graphify-out/.graphify_python) -c "..."`` rather than bare
    ``python -c "..."`` so they work in pipx / venv environments.
    """
    import halal_graphify
    skill = (Path(halal_graphify.__file__).parent / "skill-devin.md").read_text()
...
def test_devin_platform_skill_destination_user_scope(tmp_path):
    """User-scope destination must be ~/.config/devin/skills/halal-halal_graphify/SKILL.md."""
    from halal_graphify.__main__ import _platform_skill_destination
    with patch("halal_graphify.__main__.Path.home", return_value=tmp_path):
        dst = _platform_skill_destination("devin", project=False)
    assert dst == tmp_path / ".config" / "devin" / "skills" / "halal-graphify" / "SKILL.md"


def test_devin_in_main_help_text(capsys, monkeypatch):
    """`halal-graphify --help` must list devin in the platform list and in the per-platform section."""
    from halal_graphify.__main__ import main
    ...
    assert "|devin)" in captured or "|devin |" in captured or "|devin" in captured, (
        "devin missing from `halal-graphify --help` platform list"
    )
...
def test_devin_platform_skill_destination_project_scope(tmp_path):
    """Project-scope destination must be <project>/.devin/skills/halal-halal_graphify/SKILL.md."""
    from halal_graphify.__main__ import _platform_skill_destination
    dst = _platform_skill_destination("devin", project=True, project_dir=tmp_path)
    assert dst == tmp_path / ".devin" / "skills" / "halal-graphify" / "SKILL.md"
