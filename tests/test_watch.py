...
def test_batch_doc_only_deletion_triggers_rebuild(tmp_path):
    """#2580: deleting ONLY doc files while `halal-graphify watch` runs must trigger
...
def test_check_update_with_flag_returns_true_and_prints(tmp_path, capsys):
    ...
    assert "halal-graphify --update" in out
...
def test_watch_raises_without_watchdog(tmp_path, monkeypatch):
    ...
    from halal_graphify.watch import watch
...
def test_rebuild_code_writes_community_name(tmp_path):
    """#1808: `halal-graphify update` / _rebuild_code must forward community_labels to
...
def test_rebuild_code_keeps_a_visualization_when_over_the_viz_cap(tmp_path, monkeypatch):
    ...
    with monkeypatch.context() as failed_render:
        failed_render.setattr("halal_graphify.paths.os.replace", fail_html_publish)
        assert _rebuild_code(corpus, acquire_lock=False) is True
    ...
    with monkeypatch.context() as recovery_patch:
        recovery_patch.setattr("halal_graphify.cluster.cluster", fail_cluster)
        assert _rebuild_code(corpus, acquire_lock=False) is True
...
def test_missing_html_recovery_preserves_multigraph_edge_counts(tmp_path, monkeypatch):
    ...
    from halal_graphify.watch import _reconcile_graph_html
    ...
    with monkeypatch.context() as failed_render:
        ...
        failed_render.setattr("halal_graphify.paths.os.replace", fail_replace)
...
def test_update_rebuilds_with_nested_star_gitignore(tmp_path):
    """#1880: `halal-graphify update` must not emit 0 nodes (and then refuse to
...
def test_update_discovers_newly_added_files_and_dirs(tmp_path):
    """#1837: after an initial build, a plain `halal-graphify update` (full re-scan, no
...
def test_rebuild_honors_persisted_excludes(tmp_path):
    ...
    import json
    from halal_graphify.watch import _rebuild_code, _write_build_config
...
def test_rebuild_code_evicts_nodes_from_deleted_files(tmp_path):
    """#1007: halal-graphify update (_rebuild_code with no changed_paths) must remove
...
def test_rebuild_code_evicts_removed_symbol_from_surviving_file(tmp_path):
    """#1116: halal-graphify update (_rebuild_code with no changed_paths) must prune a
    ...
    # shrink, so the shrink-guard must let `halal-graphify update` refresh the graph
...
def test_rebuild_code_preupgrade_marker_less_node_one_cycle_lag(tmp_path):
    ...
    markers. On the first `halal-graphify update` after upgrading, a symbol removed
...
def test_rebuild_code_is_idempotent_when_cluster_ids_flap(tmp_path, monkeypatch):
    from halal_graphify import cluster as cluster_mod
    from halal_graphify.watch import _rebuild_code
...
def test_watch_handler_honors_graphifyignore(tmp_path, monkeypatch):
    ...
    """
    import threading
    from halal_graphify import watch as watch_mod
...
def test_watch_loads_graphifyignore_once(tmp_path, monkeypatch):
    ...
    import threading
    from halal_graphify import watch as watch_mod
    from halal_graphify import detect as detect_mod
...
def test_queue_and_drain_pending_round_trip(tmp_path):
    ...
    from halal_graphify.watch import _queue_pending, _drain_pending, _PENDING_FILENAME
...
def test_drain_pending_dedupes_and_skips_blank_lines(tmp_path):
    ...
    from halal_graphify.watch import _queue_pending, _drain_pending
...
def test_queue_pending_noop_on_empty_list(tmp_path):
    ...
    from halal_graphify.watch import _queue_pending, _PENDING_FILENAME
...
def test_rebuild_code_queues_on_lock_contention(tmp_path, monkeypatch, capsys):
    ...
    from halal_graphify.watch import _rebuild_code, _rebuild_lock, _PENDING_FILENAME
...
def test_rebuild_code_drains_late_arrivals(tmp_path, monkeypatch):
    ...
    from halal_graphify import watch as watch_mod
    from halal_graphify.watch import _rebuild_code as orig_rebuild
...
def test_merge_changed_paths_dedupes_in_order():
    ...
    from halal_graphify.watch import _merge_changed_paths
...
# ── #2495: `halal-graphify update` purges newly-ignored files that still exist ───────
...
def test_update_prunes_newly_graphifyignored_file(tmp_path, monkeypatch, capsys):
    ...
    from the graph by a full `halal-graphify update` — nodes, edges, and hyperedges —
...
def test_gitignore_eviction_gated_on_full_rebuild(tmp_path, capsys):
    ...
    `halal-graphify update` honors it and purges the file."""
    from halal_graphify.watch import _rebuild_code
...
def _seed_semantic_doc_graph(corpus):
    ...
    Mimics a graph produced by the CLI ``halal-graphify . --update`` path: code AST
...
def test_rebuild_code_evicts_semantic_nodes_from_deleted_non_ast_source(tmp_path):
    """#2051: a full `halal-graphify update` must evict semantic nodes whose non-AST
...
def test_rebuild_code_preserves_remote_source_across_repeated_updates(tmp_path):
    ...
    (gdoc://, s3://, http://) must survive REPEATED `halal-graphify update`s. Path
    ...
    collapse."""
    from halal_graphify.watch import _rebuild_code, _is_remote_source
...
# --- #2342: `halal-graphify update` rebuild must inherit the on-disk directed flag ---

def test_rebuild_code_inherits_directed_flag_clustered(tmp_path):
    """#2342: the clustered `halal-graphify update` rebuild path built the graph via
    ...
    assert after.get("directed") is True, (
        "halal-graphify update (clustered) must preserve an existing directed=True graph"
    )
    edge = next(
...
def test_rebuild_code_inherits_directed_flag_no_cluster(tmp_path):
    ...
    assert after.get("directed") is True, (
        "halal-graphify update --no-cluster must preserve an existing directed=True graph"
    )
...
def test_incremental_rebuild_does_not_reparse_unchanged_targets(tmp_path, monkeypatch):
    ...
    import halal_graphify.extract as extract_mod
    from halal_graphify.watch import _rebuild_code
