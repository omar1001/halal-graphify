...
def test_antigravity_workflow_names_no_skill_path(tmp_path):
    ...
    point at ~/.gemini/config/skills/halal-halal_graphify/SKILL.md, which it never writes.
    """
    m._project_install("antigravity", tmp_path)
    ...
    assert "halal-graphify skill" in body, "workflow should still point at the skill by name"
...
def test_antigravity_global_install_workflow_names_no_skill_path(tmp_path, monkeypatch):
    ...
    assert (
        tmp_path / "home" / ".gemini" / "config" / "skills" / "halal-graphify" / "SKILL.md"
    ).exists()
...
def test_antigravity_project_uninstall_clears_rules_and_workflows(tmp_path):
    ...
    assert not (tmp_path / ".agents" / "skills" / "halal-graphify" / "SKILL.md").exists()
