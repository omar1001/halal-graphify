"""`halal-graphify extract --no-dedup` (#2881).
...
def _capture_dedup(monkeypatch):
    ...
    Patching `halal_graphify.build` does reach the CLI's `_build` / `_build_merge`
    aliases, even though it refers to them by those names: the

        from halal_graphify.build import build as _build, build_merge as _build_merge
    ...
    nothing.
    """
    import halal_graphify.build as buildmod
...
def _assert_spied(seen: dict, entry_point: str) -> None:
    ...
    assert entry_point in seen, (
        ...
        f"halal_graphify.build symbol — the spy is inert and every dedup assertion "
