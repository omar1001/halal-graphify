"""#1688 - halal-graphify's shared-file section update must not destroy user content.

_replace_or_append_section used to locate its marker (`## halal-graphify`) as a
...
from halal_graphify.__main__ import _replace_or_append_section

MARKER = "## halal-graphify"
...
def test_inline_reference_to_marker_is_not_treated_as_the_section():
    before = (
        "# My Project\n\n"
        "## Setup\n"
        "- See the `## halal-graphify` section for graph usage.\n\n"
        "## Release Process\n"
        "Critical steps that must not be lost.\n"
    )
    after = _replace_or_append_section(before, MARKER, NEW)
    assert "See the `## halal-graphify` section" in after       # bullet preserved
