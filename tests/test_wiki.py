"""Tests for halal_graphify.wiki — Wikipedia-style article generation."""
import re
import pytest
from pathlib import Path
import networkx as nx
from halal_graphify.wiki import to_wiki, _index_md, _community_article, _hub_node_article
...
HUB_NODES = [{"id": "n1", "label": "parse", "degree": 2}]


def test_to_wiki_writes_index(tmp_path):
    G = _make_graph()
    n = to_wiki(G, COMMUNITIES, tmp_path, community_labels=LABELS, cohesion=COHESION, hub_nodes_data=HUB_NODES)
    assert (tmp_path / "index.md").exists()


def test_to_wiki_returns_article_count(tmp_path):
    G = _make_graph()
    # 2 communities + 1 hub node = 3
    n = to_wiki(G, COMMUNITIES, tmp_path, community_labels=LABELS, cohesion=COHESION, hub_nodes_data=HUB_NODES)
    assert n == 3
...
def test_to_wiki_hub_node_article_created(tmp_path):
    G = _make_graph()
    to_wiki(G, COMMUNITIES, tmp_path, community_labels=LABELS, hub_nodes_data=HUB_NODES)
...
def test_index_lists_hub_nodes(tmp_path):
    G = _make_graph()
    to_wiki(G, COMMUNITIES, tmp_path, community_labels=LABELS, hub_nodes_data=HUB_NODES)
    index = (tmp_path / "index.md").read_text()
...
def test_hub_node_article_has_connections(tmp_path):
    G = _make_graph()
    to_wiki(G, COMMUNITIES, tmp_path, community_labels=LABELS, hub_nodes_data=HUB_NODES)
...
def test_hub_node_article_links_community(tmp_path):
    G = _make_graph()
    to_wiki(G, COMMUNITIES, tmp_path, community_labels=LABELS, hub_nodes_data=HUB_NODES)
    article = (tmp_path / "parse.md").read_text()
...
def test_to_wiki_skips_missing_hub_node_ids(tmp_path):
    """Hub node with bad ID should not crash."""
    G = _make_graph()
    bad_hubs = [{"id": "nonexistent", "label": "ghost", "degree": 99}]
    n = to_wiki(G, COMMUNITIES, tmp_path, community_labels=LABELS, hub_nodes_data=bad_hubs)
    # 2 communities + 0 hub nodes (nonexistent skipped) = 2
    assert n == 2
...
def test_hub_node_article_community_without_node_attr(tmp_path):
    """Hub node article must show community name even when node has no 'community' attr (#925)."""
    G = nx.Graph()
    G.add_node("n1", label="parse", file_type="code", source_file="parser.py")
    G.add_node("n2", label="validate", file_type="code", source_file="parser.py")
    G.add_edge("n1", "n2", relation="calls", confidence="EXTRACTED", weight=1.0)
    communities = {0: ["n1", "n2"]}
    labels = {0: "Core Logic"}
    hub_nodes = [{"id": "n1", "label": "parse", "degree": 1}]
    to_wiki(G, communities, tmp_path, community_labels=labels, hub_nodes_data=hub_nodes)
    article = (tmp_path / "parse.md").read_text()
...
def test_to_wiki_hub_node_label_case_collides_with_community(tmp_path):
    """Community and hub-node articles share one slug-dedup set, so a hub-node
    ...
    communities = {0: ["n1", "n2"]}
    labels = {0: "Parser"}
    hub_nodes = [{"id": "n1", "label": "parser", "degree": 1}]
    n = to_wiki(G, communities, tmp_path, community_labels=labels, hub_nodes_data=hub_nodes)
    articles = [p for p in tmp_path.glob("*.md") if p.name != "index.md"]
    assert len(articles) == n == 2, [p.name for p in articles]
    lowered = [p.stem.lower() for p in articles]
    assert len(set(lowered)) == len(lowered), [p.name for p in articles]
...
def test_wiki_emits_no_obsidian_wikilinks(tmp_path):
    ...
    G = _make_graph()
    to_wiki(G, COMMUNITIES, tmp_path, community_labels=LABELS, cohesion=COHESION, hub_nodes_data=HUB_NODES)
...
def test_wiki_links_to_nodes_without_articles_are_plain_text(tmp_path):
    """A hub node links its neighbours, but only communities and hub nodes get
    article files — neighbours without one must render as plain text, not as a
    link (dead even inside Obsidian)."""
    G = _make_graph()
    # only `parse` (n1) is a hub node; its neighbours validate/render are not,
