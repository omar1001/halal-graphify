...
own ``halal_graphify/extractors/<lang>.py`` module, kept a verbatim re-export in
``halal_graphify.extract`` (the facade every existing importer uses), and seeded a
``halal_graphify.extractors.LANGUAGE_EXTRACTORS`` registry. Three things must stay
true for that split to be behavior-preserving:

- the function is importable from its new per-language module,
- ``halal_graphify.extract`` still re-exports the SAME function object (facade
...
import halal_graphify.extract as facade
from halal_graphify.extractors import LANGUAGE_EXTRACTORS
...
def test_every_registry_extractor_is_reexported_from_facade():
    ...
    assert not missing, f"registry extractors not re-exported from halal_graphify.extract: {missing}"
