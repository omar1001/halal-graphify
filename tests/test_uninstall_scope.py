...
- `halal-graphify uninstall --project` for codebuddy no longer nukes the global skill
"""
...
def _plant_skill_tree(root: Path, dot_dir: str) -> Path:
    """Create <root>/<dot_dir>/skills/halal-halal_graphify/{SKILL.md, references/x.md, .graphify_version}."""
    skill_dir = root / dot_dir / "skills" / "halal-graphify"
    (skill_dir / "references").mkdir(parents=True)
    (skill_dir / "SKILL.md").write_text("# halal-graphify skill\n", encoding="utf-8")
...
def test_project_uninstall_codebuddy_spares_global(tmp_path):
    """`halal-graphify uninstall --project` (codebuddy branch) must not delete ~/.codebuddy (#2215)."""
