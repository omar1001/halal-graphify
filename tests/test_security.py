"""Tests for halal_graphify/security.py - URL validation, safe fetch, path guards, label sanitisation."""
...
def test_safe_fetch_returns_bytes(tmp_path):
    ...
    with patch("halal_graphify.security._build_opener") as mock_opener_fn:
...
def test_validate_graph_path_default_base_honours_graphify_out_override(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("halal_graphify.security.GRAPHIFY_OUT_NAME", "custom-out")
    monkeypatch.setattr("halal_graphify.security.GRAPHIFY_OUT", "custom-out")
...
def test_graph_size_cap_at_boundary_passes(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("halal_graphify.security._MAX_GRAPH_FILE_BYTES", 32)
    assert check_graph_file_size_cap(p) is None
    monkeypatch.setattr("halal_graphify.security._MAX_GRAPH_FILE_BYTES", 31)
    with pytest.raises(ValueError):
        check_graph_file_size_cap(p)
