...
`halal-graphify install --platform agents` (and the friendly `--platform skills`
...
The bare ``halal-graphify install`` behaviour (claude/windows only) is unchanged; the
named platform is opt-in. The ``halal-graphify agents install`` subcommand is the
...
def test_agents_user_destination_is_user_global_dot_agents(tmp_path):
    ...
    assert dst == tmp_path / ".agents" / "skills" / "halal-graphify" / "SKILL.md"
...
def _run(tmp_path, argv, home):
    ...
    try:
        os.chdir(tmp_path)
        with patch.object(sys, "argv", ["halal-graphify", *argv]):
...
def test_install_platform_agents_writes_user_global_skill_only(tmp_path, platform_arg):
    """`halal-graphify install --platform agents|skills` writes ~/.agents/skills/...
    ...
    skill = home / ".agents" / "skills" / "halal-graphify" / "SKILL.md"
    assert skill.exists()
...
def test_uninstall_platform_agents_removes_user_global_skill(tmp_path):
    """Bare `halal-graphify uninstall` clears the ~/.agents/skills skill the AGENTS.md and
...
def test_uninstall_platform_flag_global_removes_skill(tmp_path, platform_arg):
    """`halal-graphify uninstall --platform agents|skills` (global) clears ~/.agents/skills.
...
def test_project_uninstall_all_removes_agents_skill(tmp_path):
    """`halal-graphify uninstall --project` (no platform) removes the agents project skill
    ...
    project_skill = proj / ".agents" / "skills" / "halal-graphify" / "SKILL.md"
...
def test_install_platform_agents_project_writes_dot_agents(tmp_path):
    """`halal-graphify install --project --platform agents` writes ./.agents/skills and
...
# --- the amp-twin subcommand (halal-graphify agents install) -------------------------


def test_agents_subcommand_install_also_wires_agents_md(tmp_path):
    """`halal-graphify agents install` is the amp-twin: skill at ~/.agents/skills PLUS a
    `## halal-graphify` section in AGENTS.md (so the rendered hooks reference, which
    points at `halal-graphify agents install`, stays honest)."""
    home = tmp_path / "home"
    cwd = tmp_path / "cwd"
    home.mkdir()
    cwd.mkdir()
    ...
    assert "## halal-graphify" in agents_md.read_text(encoding="utf-8")
    ...
    assert not agents_md.exists() or "## halal-graphify" not in agents_md.read_text(encoding="utf-8")


def test_agents_subcommand_install_is_idempotent(tmp_path):
    """Running `halal-graphify agents install` twice leaves a single AGENTS.md section."""
    home = tmp_path / "home"
    cwd = tmp_path / "cwd"
    home.mkdir()
    cwd.mkdir()
    ...
    assert body.count("## halal-graphify") == 1, "AGENTS.md gained a duplicate halal-graphify section"


def test_skills_subcommand_is_the_agents_subcommand(tmp_path):
    """`halal-graphify skills install`/`uninstall` behaves exactly like the agents form:
...
def test_bare_install_does_not_touch_dot_agents(tmp_path):
    """`halal-graphify install` (no platform) stays single-platform claude/windows and
