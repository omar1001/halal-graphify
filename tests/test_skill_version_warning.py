...
`_check_skill_version` used to advise `halal-graphify install` on ANY version
...
def _make_skill(tmp_path: Path, stamped: str) -> Path:
    skill_dst = tmp_path / "skills" / "halal-graphify" / "SKILL.md"
    skill_dst.parent.mkdir(parents=True, exist_ok=True)
    skill_dst.write_text("# halal-graphify skill\n", encoding="utf-8")
...
def test_skill_older_than_package_recommends_install(tmp_path, monkeypatch, capsys):
    ...
    assert "Run 'halal-graphify install' to update" in err
...
def test_skill_newer_than_package_recommends_upgrade_not_install(tmp_path, monkeypatch, capsys):
    ...
    assert "Run 'halal-graphify install' to update" not in err
