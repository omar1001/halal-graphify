"""Tests for halal-graphify install --platform routing."""
import os
...
PLATFORMS = {
    "claude": (".claude/skills/halal-halal_graphify/SKILL.md",),
    "codebuddy": (".codebuddy/skills/halal-halal_graphify/SKILL.md",),
    "codex": (".codex/skills/halal-halal_graphify/SKILL.md",),
    "opencode": (".config/opencode/skills/halal-halal_graphify/SKILL.md",),
    "kilo": (
        ".config/kilo/skills/halal-halal_graphify/SKILL.md",
        ".config/kilo/command/graphify.md",
    ),
    "claw": (".openclaw/skills/halal-halal_graphify/SKILL.md",),
    "droid": (".factory/skills/halal-halal_graphify/SKILL.md",),
    "trae": (".trae/skills/halal-halal_graphify/SKILL.md",),
    "trae-cn": (".trae-cn/skills/halal-halal_graphify/SKILL.md",),
    "windows": (".claude/skills/halal-halal_graphify/SKILL.md",),
}


def _install(tmp_path, platform):
    from halal_graphify.__main__ import install

    old_cwd = Path.cwd()
    try:
        os.chdir(tmp_path)
        with patch("halal_graphify.__main__.Path.home", return_value=tmp_path):
...
def test_install_default_claude(tmp_path):
    ...
    assert (tmp_path / ".claude" / "skills" / "halal-graphify" / "SKILL.md").exists()
...
def test_install_claude_md_honors_claude_config_dir(tmp_path, monkeypatch):
    ...
    try:
        os.chdir(tmp_path)
        with patch("halal_graphify.__main__.Path.home", return_value=home):
...
def test_install_claude_md_defaults_to_home_when_config_dir_unset(tmp_path, monkeypatch):
    ...
    assert md.exists()
    assert "~/.claude/skills/halal-halal_graphify/SKILL.md" in md.read_text()


def test_install_codebuddy(tmp_path):
    _install(tmp_path, "codebuddy")
    assert (tmp_path / ".codebuddy" / "skills" / "halal-graphify" / "SKILL.md").exists()


def test_install_codex(tmp_path):
    _install(tmp_path, "codex")
    assert (tmp_path / ".codex" / "skills" / "halal-graphify" / "SKILL.md").exists()


def test_install_opencode(tmp_path):
    _install(tmp_path, "opencode")
    assert (
        tmp_path / ".config" / "opencode" / "skills" / "halal-graphify" / "SKILL.md"
    ).exists()
...
def test_install_positional_platform_opencode(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(sys, "argv", ["halal-graphify", "install", "opencode"])
    with patch("halal_graphify.__main__.Path.home", return_value=tmp_path):
        main()
    assert (tmp_path / ".config" / "opencode" / "skills" / "halal-graphify" / "SKILL.md").exists()
    assert not (tmp_path / ".claude" / "skills" / "halal-graphify" / "SKILL.md").exists()
...
def test_install_project_claude_writes_project_scope(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr(sys, "argv", ["halal-graphify", "install", "--project"])
    with patch("halal_graphify.__main__.Path.home", return_value=home):
        main()
    assert (project / ".claude" / "skills" / "halal-graphify" / "SKILL.md").exists()
    assert (project / ".claude" / "CLAUDE.md").exists()
    assert not (home / ".claude" / "skills" / "halal-graphify" / "SKILL.md").exists()
    assert ".claude/skills/halal-halal_graphify/SKILL.md" in (project / ".claude" / "CLAUDE.md").read_text()
    assert "~/.claude/skills/halal-halal_graphify/SKILL.md" not in (project / ".claude" / "CLAUDE.md").read_text()
...
def test_install_project_codex_writes_skill_and_agents(tmp_path, monkeypatch):
    ...
    project.mkdir()
    monkeypatch.chdir(project)
    monkeypatch.setattr(sys, "argv", ["halal-graphify", "install", "--project", "--platform", "codex"])
    with patch("halal_graphify.__main__.Path.home", return_value=home):
        main()
    assert (project / ".codex" / "skills" / "halal-graphify" / "SKILL.md").exists()
    assert (project / "AGENTS.md").exists()
    assert (project / ".codex" / "hooks.json").exists()
    assert not (home / ".codex" / "skills" / "halal-graphify" / "SKILL.md").exists()
...
def test_claude_subcommand_project_install_and_uninstall_are_project_scoped(tmp_path, monkeypatch):
    ...
    project = tmp_path / "project"
    project.mkdir()
    user_skill = home / ".claude" / "skills" / "halal-graphify" / "SKILL.md"
    user_skill.parent.mkdir(parents=True)
    user_skill.write_text("user skill")
    monkeypatch.chdir(project)
    with patch("halal_graphify.__main__.Path.home", return_value=home):
        monkeypatch.setattr(sys, "argv", ["halal-graphify", "claude", "install", "--project"])
        main()
        assert (project / ".claude" / "skills" / "halal-graphify" / "SKILL.md").exists()
        assert (project / ".claude" / "CLAUDE.md").exists()
        ...
        monkeypatch.setattr(sys, "argv", ["halal-graphify", "claude", "uninstall", "--project"])
        main()

    assert user_skill.exists()
    assert not (project / ".claude" / "skills" / "halal-graphify" / "SKILL.md").exists()
...
def test_codex_subcommand_project_install_and_uninstall_are_project_scoped(tmp_path, monkeypatch):
    ...
    project = tmp_path / "project"
    project.mkdir()
    user_skill = home / ".codex" / "skills" / "halal-graphify" / "SKILL.md"
    user_skill.parent.mkdir(parents=True)
    user_skill.write_text("user skill")
    monkeypatch.chdir(project)
    with patch("halal_graphify.__main__.Path.home", return_value=home):
        monkeypatch.setattr(sys, "argv", ["halal-graphify", "codex", "install", "--project"])
        main()
        assert (project / ".codex" / "skills" / "halal-graphify" / "SKILL.md").exists()
        assert (project / "AGENTS.md").exists()
        assert (project / ".codex" / "hooks.json").exists()
        ...
        monkeypatch.setattr(sys, "argv", ["halal-graphify", "codex", "uninstall", "--project"])
        main()

    assert user_skill.exists()
    assert not (project / ".codex" / "skills" / "halal-graphify" / "SKILL.md").exists()
    assert not (project / "AGENTS.md").exists()
    hooks_path = project / ".codex" / "hooks.json"
    assert hooks_path.exists()
    assert "halal-graphify" not in hooks_path.read_text()
...
def test_antigravity_install_project_writes_project_skill(tmp_path, monkeypatch):
    ...
    project.mkdir()
    monkeypatch.chdir(project)
    monkeypatch.setattr(sys, "argv", ["halal-graphify", "antigravity", "install", "--project"])
    with patch("halal_graphify.__main__.Path.home", return_value=home):
        main()
    assert (project / ".agents" / "skills" / "halal-graphify" / "SKILL.md").exists()
    assert not (home / ".agents" / "skills" / "halal-graphify" / "SKILL.md").exists()


def test_install_help_does_not_install_default(tmp_path, monkeypatch, capsys):
    from halal_graphify.__main__ import main
    monkeypatch.chdir(tmp_path)
    monkeypatch.setattr(sys, "argv", ["halal-graphify", "install", "opencode", "--help"])
    with patch("halal_graphify.__main__.Path.home", return_value=tmp_path):
        main()
    out = capsys.readouterr().out
    assert "Usage: halal-graphify install" in out
...
def test_install_claw(tmp_path):
    ...
    assert (tmp_path / ".openclaw" / "skills" / "halal-graphify" / "SKILL.md").exists()


def test_install_droid(tmp_path):
    _install(tmp_path, "droid")
    assert (tmp_path / ".factory" / "skills" / "halal-graphify" / "SKILL.md").exists()


def test_install_trae(tmp_path):
    _install(tmp_path, "trae")
    assert (tmp_path / ".trae" / "skills" / "halal-graphify" / "SKILL.md").exists()


def test_install_trae_cn(tmp_path):
    _install(tmp_path, "trae-cn")
    assert (tmp_path / ".trae-cn" / "skills" / "halal-graphify" / "SKILL.md").exists()
...
def test_codex_skill_contains_spawn_agent():
    ...
    import halal_graphify

    skill = (Path(halal_graphify.__file__).parent / "skill-codex.md").read_text()
...
def test_codex_skill_uses_graphify_with_existing_graph():
    ...
    assert "skip Steps 1–5 entirely and jump straight to `## For /halal-graphify query`" in skill
    assert "halal-graphify query" in skill
    assert "halal-graphify explain" in skill
    assert "halal-graphify path" in skill
...
def test_codex_agents_install_mentions_dirty_graph_output(tmp_path):
    ...
    assert "not a reason to skip halal-graphify" in content
...
def test_opencode_skill_contains_mention():
    ...
    skill = (Path(halal_graphify.__file__).parent / "skill-opencode.md").read_text()
...
def test_kilo_skill_mentions_task_tool():
    ...
    skill = (Path(halal_graphify.__file__).parent / "skill-kilo.md").read_text()
...
def test_claw_skill_uses_agent_tool_dispatch():
    ...
    skill = (Path(halal_graphify.__file__).parent / "skill-claw.md").read_text()
    b2 = skill[skill.index("**Step B2"):skill.index("**Step B3")]
...
def test_all_skill_files_exist_in_package():
    ...
    pkg = Path(halal_graphify.__file__).parent
    for name in (
        "skill.md",
        "skill-codex.md",
        "skill-opencode.md",
...
def test_codebuddy_install_writes_codebuddy_md(tmp_path):
    from halal_graphify.__main__ import codebuddy_install
...
def test_codebuddy_install_writes_hook(tmp_path):
    ...
    assert any("halal-graphify" in str(h) for h in hooks)
...
def test_claude_hook_is_shell_agnostic(tmp_path):
    ...
    import json as _json
    from halal_graphify.__main__ import _install_claude_hook
    ...
    for h in hooks:
        ...
        assert "halal-graphify" in cmd and "hook-guard" in cmd
...
def test_claude_hook_install_idempotent_and_replaces_old_bash_hook(tmp_path):
    ...
    # Pre-seed a legacy bash-style halal-graphify hook (the thing #522 shipped before).
    ...
    graphify_hooks = [h for h in hooks if "halal-graphify" in str(h)]
...
def test_codebuddy_install_idempotent(tmp_path):
    ...
    codebuddy_install(tmp_path)
    md = tmp_path / "CODEBUDDY.md"
    assert md.read_text().count("## halal-graphify") == 1
...
def test_codebuddy_uninstall_removes_section(tmp_path):
    from halal_graphify.__main__ import codebuddy_install, codebuddy_uninstall
    codebuddy_install(tmp_path)
...
def test_codebuddy_uninstall_removes_hook(tmp_path):
    ...
    if settings_path.exists():
        ...
        assert not any("halal-graphify" in str(h) for h in hooks)


def test_codebuddy_uninstall_noop_if_not_installed(tmp_path):
    from halal_graphify.__main__ import codebuddy_uninstall
...
def test_uninstall_project_removes_project_skill_only(tmp_path, monkeypatch):
    ...
    with patch("halal_graphify.__main__.Path.home", return_value=home):
        monkeypatch.setattr(sys, "argv", ["halal-graphify", "install", "--project", "--platform", "codex"])
        main()
        monkeypatch.setattr(sys, "argv", ["halal-graphify", "uninstall", "--project", "--platform", "codex"])
        main()
    assert user_skill.exists()
    assert not (project / ".codex" / "skills" / "halal-graphify" / "SKILL.md").exists()
    assert not (project / "AGENTS.md").exists()
...
def test_uninstall_project_without_platform_removes_project_installs(tmp_path, monkeypatch):
    ...
    with patch("halal_graphify.__main__.Path.home", return_value=home):
        monkeypatch.setattr(sys, "argv", ["halal-graphify", "install", "--project"])
        main()
        monkeypatch.setattr(sys, "argv", ["halal-graphify", "uninstall", "--project"])
        main()
    assert user_skill.exists()
    assert not (project / ".claude" / "skills" / "halal-graphify" / "SKILL.md").exists()
    assert not (project / ".claude" / "CLAUDE.md").exists()
...
def test_antigravity_uninstall_project_removes_project_skill_only(tmp_path, monkeypatch):
    ...
    global_skill = home / ".gemini" / "config" / "skills" / "halal-graphify" / "SKILL.md"
    ...
    with patch("halal_graphify.__main__.Path.home", return_value=home):
        monkeypatch.setattr(sys, "argv", ["halal-graphify", "antigravity", "install", "--project"])
        main()
        monkeypatch.setattr(sys, "argv", ["halal-graphify", "antigravity", "uninstall", "--project"])
        main()
    assert global_skill.exists(), "project uninstall must not touch global skill"
    assert not (project / ".agents" / "skills" / "halal-graphify" / "SKILL.md").exists()


def test_antigravity_global_install_writes_gemini_config_skills(tmp_path, monkeypatch):
    """Global `halal-graphify antigravity install` must write to ~/.gemini/config/skills/ (#1079)."""
    from halal_graphify.__main__ import main
    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
    monkeypatch.chdir(project)
    with patch("halal_graphify.__main__.Path.home", return_value=home):
        monkeypatch.setattr(sys, "argv", ["halal-graphify", "antigravity", "install"])
        main()
    global_skill = home / ".gemini" / "config" / "skills" / "halal-graphify" / "SKILL.md"
    wrong_skill = home / ".agents" / "skills" / "halal-graphify" / "SKILL.md"
...
def test_antigravity_global_uninstall_removes_gemini_config_skill(tmp_path, monkeypatch):
    """Global `halal-graphify antigravity uninstall` must remove from ~/.gemini/config/skills/ (#1079)."""
    from halal_graphify.__main__ import main
    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
    ...
    with patch("halal_graphify.__main__.Path.home", return_value=home):
        ...
        monkeypatch.setattr(sys, "argv", ["halal-graphify", "antigravity", "uninstall"])
        main()
...
def _agents_install(tmp_path, platform):
    from halal_graphify.__main__ import _agents_install as _install_fn

    _install_fn(tmp_path, platform)


def _agents_uninstall(tmp_path, platform=""):
    from halal_graphify.__main__ import _agents_uninstall as _uninstall_fn

    _uninstall_fn(tmp_path, platform=platform)


def _kilo_install(project_dir, home_dir):
    from halal_graphify.__main__ import _kilo_install as _install_fn

    with patch("halal_graphify.__main__.Path.home", return_value=home_dir):
        _install_fn(project_dir)


def _kilo_uninstall(project_dir, home_dir):
    from halal_graphify.__main__ import _kilo_uninstall as _uninstall_fn
...
def test_codex_agents_install_writes_agents_md(tmp_path):
    ...
    assert "halal-graphify" in agents_md.read_text()
...
def test_agents_install_idempotent(tmp_path):
    ...
    _agents_install(tmp_path, "codex")
    content = (tmp_path / "AGENTS.md").read_text()
    assert content.count("## halal-graphify") == 1
...
def test_agents_install_appends_to_existing(tmp_path):
    ...
    assert "## halal-graphify" in content
...
def test_agents_uninstall_removes_section(tmp_path):
    ...
    agents_md = tmp_path / "AGENTS.md"
    # File deleted when it only contained halal-graphify section
...
def test_agents_uninstall_preserves_other_content(tmp_path):
    ...
    content = agents_md.read_text()
    assert "Do not break things." in content
    assert "## halal-graphify" not in content
...
def test_remove_marker_section_matches_exact_heading_only(tmp_path):
    """#2062: the strip helper must match halal-graphify's own `## halal-graphify` heading
    exactly, never a substring inside a user's `### halal-graphify` H3."""
    from halal_graphify.install import _remove_marker_section
    m = "## halal-graphify"
    ...
    assert _remove_marker_section("see the ## halal-graphify bullet\n", m) is None

    # A real H2 section alongside a user H3: remove only the H2 section.
    content = "# Doc\n\n### halal-graphify\n\nmy notes\n\n## halal-graphify\n\ngraphify stuff\n"
    out = _remove_marker_section(content, m)
    assert out is not None
    assert "### halal-graphify" in out and "my notes" in out
    assert not any(l.strip() == "## halal-graphify" for l in out.splitlines())
    assert "halal-graphify stuff" not in out
...
def test_agents_uninstall_preserves_user_h3_graphify_heading(tmp_path):
    """#2062 end-to-end: uninstall strips halal-graphify's own H2 section but leaves a
    user-authored `### halal-graphify` H3 (and everything else) byte-intact."""
    agents_md = tmp_path / "AGENTS.md"
    agents_md.write_text(
        "# My rules\n\n"
        "### halal-graphify\n\n"
        "My own notes on how I use halal-graphify. Keep this.\n\n"
        "## Other\n\nUnrelated content.\n"
    )
    _agents_install(tmp_path, "codex")  # appends a genuine `## halal-graphify` H2 section
    assert "## halal-graphify" in agents_md.read_text()

    _agents_uninstall(tmp_path)
    content = agents_md.read_text()
    assert "### halal-graphify" in content, "user's H3 heading was deleted (#2062)"
    assert "My own notes on how I use halal-graphify. Keep this." in content
    assert "## Other" in content and "Unrelated content." in content
    assert not any(l.strip() == "## halal-graphify" for l in content.splitlines())


def test_uninstall_untouched_when_only_user_h3_present(tmp_path, capsys):
    """#2062: a file with only a user `### halal-graphify` H3 (halal-graphify never installed)
...
def test_opencode_plugin_reminder_has_no_backticks(tmp_path):
    ...
    the very halal-graphify command we are only suggesting.
    """
    _agents_install(tmp_path, "opencode")
    plugin = tmp_path / ".opencode" / "plugins" / "graphify.js"
...
def test_kilo_install_writes_global_and_project_artifacts(tmp_path):
    ...
    assert (home_dir / ".config" / "kilo" / "skills" / "halal-graphify" / "SKILL.md").exists()
...
def test_kilo_uninstall_removes_plugin_registration_and_command(tmp_path):
    ...
    assert not (
        home_dir / ".config" / "kilo" / "skills" / "halal-graphify" / "SKILL.md"
    ).exists()
...
def test_cursor_install_writes_rule(tmp_path):
    """cursor install writes .cursor/rules/halal-graphify.mdc."""
    from halal_graphify.__main__ import _cursor_install

    _cursor_install(tmp_path)
    rule = tmp_path / ".cursor" / "rules" / "halal-graphify.mdc"
    assert rule.exists()
...
def test_cursor_uninstall_removes_rule(tmp_path):
    ...
    from halal_graphify.__main__ import _cursor_install, _cursor_uninstall
...
def test_cursor_uninstall_noop_if_not_installed(tmp_path):
    ...
    from halal_graphify.__main__ import _cursor_uninstall
...
def test_gemini_install_writes_gemini_md(tmp_path):
    from halal_graphify.__main__ import gemini_install
...
def test_gemini_uninstall_removes_section(tmp_path):
    from halal_graphify.__main__ import gemini_install, gemini_uninstall
...
def test_gemini_uninstall_noop_if_not_installed(tmp_path):
    from halal_graphify.__main__ import gemini_uninstall

    gemini_uninstall(tmp_path)  # should not raise


def test_amp_user_install_lands_in_config_agents(tmp_path, monkeypatch):
    """`halal-graphify amp install` (user scope) must drop the skill into an Amp search
    ...
    project.mkdir()
    monkeypatch.chdir(project)
    monkeypatch.setattr(sys, "argv", ["halal-graphify", "amp", "install"])
    with patch("halal_graphify.__main__.Path.home", return_value=home):
        main()

    correct = home / ".config" / "agents" / "skills" / "halal-graphify" / "SKILL.md"
    old = home / ".amp" / "skills" / "halal-graphify" / "SKILL.md"
...
def test_amp_install_cleans_legacy_amp_skills_dir(tmp_path, monkeypatch):
    """A pre-fix ~/.amp/skills/halal-graphify install is removed on the next install."""
    from halal_graphify.__main__ import main

    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
    legacy = home / ".amp" / "skills" / "halal-graphify"
    ...
    assert not legacy.exists(), "legacy ~/.amp/skills/halal-graphify should be cleaned up"
    assert (home / ".config" / "agents" / "skills" / "halal-graphify" / "SKILL.md").exists()


def test_amp_user_uninstall_removes_skill_and_agents(tmp_path, monkeypatch):
    """`halal-graphify amp uninstall` removes the user-scope skill and AGENTS.md section."""
    from halal_graphify.__main__ import main
    ...
    with patch("halal_graphify.__main__.Path.home", return_value=home):
        monkeypatch.setattr(sys, "argv", ["halal-graphify", "amp", "install"])
        main()
        skill = home / ".config" / "agents" / "skills" / "halal-graphify" / "SKILL.md"
        assert skill.exists()

        monkeypatch.setattr(sys, "argv", ["halal-graphify", "amp", "uninstall"])
        main()
...
def test_amp_project_install_lands_in_dot_agents(tmp_path, monkeypatch):
    ...
    project.mkdir()
    monkeypatch.chdir(project)
    monkeypatch.setattr(sys, "argv", ["halal-graphify", "amp", "install", "--project"])
    with patch("halal_graphify.__main__.Path.home", return_value=home):
        main()

    assert (project / ".agents" / "skills" / "halal-graphify" / "SKILL.md").exists()
    assert not (project / ".amp" / "skills" / "halal-graphify" / "SKILL.md").exists()
    assert (project / "AGENTS.md").exists()
    # User scope untouched.
    assert not (home / ".config" / "agents" / "skills" / "halal-graphify" / "SKILL.md").exists()


def test_uninstall_all_removes_amp_user_skill(tmp_path, monkeypatch):
    """The user-scope `halal-graphify uninstall` enumeration removes the amp skill."""
    from halal_graphify.__main__ import main
    ...
    with patch("halal_graphify.__main__.Path.home", return_value=home):
        ...
        monkeypatch.setattr(sys, "argv", ["halal-graphify", "uninstall"])
        main()
...
def test_hermes_skill_destination_windows_uses_localappdata():
    ...
    from halal_graphify.__main__ import _platform_skill_destination
    with patch("halal_graphify.__main__.platform.system", return_value="Windows"), \
         patch.dict(os.environ, {"LOCALAPPDATA": str(Path("/tmp/AppDataLocal"))}):
        dst = _platform_skill_destination("hermes", project=False)
    assert dst == Path("/tmp/AppDataLocal") / "hermes" / "skills" / "halal-graphify" / "SKILL.md", dst


def test_hermes_skill_destination_posix_uses_home():
    """Non-Windows hermes destination is unchanged (~/.hermes/skills)."""
    from halal_graphify.__main__ import _platform_skill_destination
    with patch("halal_graphify.__main__.platform.system", return_value="Linux"):
        dst = _platform_skill_destination("hermes", project=False)
    assert str(dst).endswith(".hermes/skills/halal-halal_graphify/SKILL.md"), dst


def _cli_dispatched_commands() -> set[str]:
    """Subcommand names the CLI actually dispatches.

    `halal-graphify`'s dispatcher is an `elif cmd == "..."` chain rather than a declarative
    ...
    """
    import re
    from halal_graphify import cli
...
def test_codex_hook_command_is_a_real_cli_subcommand(tmp_path):
    ...
    from halal_graphify.install import _install_codex_hook
    ...
    entries = [
        ...
        if "halal-graphify" in h.get("command", "")
    ]
    assert entries, "codex install must register a halal-graphify PreToolUse hook"
