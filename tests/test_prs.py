"""Tests for halal_graphify/prs.py."""
...
from halal_graphify.prs import (
    PRInfo,
    _classify,
    _gh,
    _parse_ci,
...
class TestFetchWorktrees:
    def test_normal_case_maps_branch_to_path(self):
        ...
        with patch("halal_graphify.prs.subprocess.run", return_value=mock_result):
    ...
    def test_subprocess_failure_returns_empty_dict(self):
        with patch(
            "halal_graphify.prs.subprocess.run",
...
class TestDetectDefaultBranch:
    ...
    def test_falls_back_to_git_symbolic_ref(self):
        ...
        with patch("halal_graphify.prs._gh", return_value=None), patch(
            "halal_graphify.prs.subprocess.run", return_value=mock_result
        ):
    ...
    def test_gh_returns_empty_dict_falls_back(self):
        ...
        with patch("halal_graphify.prs._gh", return_value={}), patch(
            "halal_graphify.prs.subprocess.run", return_value=mock_result
        ):
...
class TestSubprocessOutputEncoding:
    ...
    def test_detect_default_branch_decodes_output_as_utf8(self):
        ...
        with patch("halal_graphify.prs._gh", return_value=None), \
