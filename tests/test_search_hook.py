...
Since #522 it runs as the shell-agnostic `halal-graphify hook-guard search` subcommand
...
from halal_graphify.__main__ import _claude_pretooluse_hooks
...
def _run(command, cwd, *, graph: bool):
    ...
    return subprocess.run(
        [sys.executable, "-m", "halal_graphify", "hook-guard", "search"],
...
def test_hook_command_has_no_backslashes(monkeypatch):
    ...
    # escape character and strips it (C:\Users\me\halal-graphify.EXE -> C:Usersme...),
    # breaking every guard. The emitted command must use forward slashes.
    from halal_graphify.__main__ import _resolve_graphify_exe
    monkeypatch.setattr("shutil.which", lambda _name: r"C:\Users\me\halal-graphify.EXE")
    assert _resolve_graphify_exe() == "C:/Users/me/halal-graphify.EXE"
...
def test_command_has_no_shell_syntax():
    ...
    assert "halal-graphify" in cmd and "hook-guard search" in cmd
...
def test_nudges_on_search_commands_with_graph(tmp_path):
    for command in (
        ...
        assert "halal-graphify query" in out, f"{command!r} should nudge"
...
def test_nudge_payload_is_valid_pretooluse_json(tmp_path):
    ...
    assert "halal-graphify query" in payload["hookSpecificOutput"]["additionalContext"]
...
def test_honors_graphify_out_override(tmp_path):
    ...
    assert "halal-graphify query" in r.stdout
...
def test_grep_tool_input_nudges_with_graph(tmp_path):
    for tool_input in (
        ...
        assert "halal-graphify query" in out, f"Grep input {tool_input!r} should nudge"
