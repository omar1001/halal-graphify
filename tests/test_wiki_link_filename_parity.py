...
def _wiki(tmp_path, labels: dict[int, str], hub: list[dict] | None = None):
    G = nx.Graph()
    ...
    out = tmp_path / "wiki"
    to_wiki(G, communities, out, community_labels=labels, hub_nodes_data=hub or [])
    return out
...
def test_whole_wiki_has_no_dangling_link_with_callable_hub_nodes(tmp_path):
    G = nx.Graph()
    ...
    hub = [{"id": n, "label": G.nodes[n]["label"], "degree": G.degree(n)} for n in ids]
    out = tmp_path / "wiki"
