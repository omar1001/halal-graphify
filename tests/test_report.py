...
from halal_graphify.analyze import hub_nodes, surprising_connections
...
def make_inputs():
    ...
    return G, communities, cohesion, labels, hubs, surprises, detection, tokens

def test_report_contains_header():
    G, communities, cohesion, labels, hubs, surprises, detection, tokens = make_inputs()
    report = generate(G, communities, cohesion, labels, hubs, surprises, detection, tokens, "./project")
...
def test_report_contains_hub_nodes():
    G, communities, cohesion, labels, hubs, surprises, detection, tokens = make_inputs()
    report = generate(G, communities, cohesion, labels, hubs, surprises, detection, tokens, "./project")
    assert "## Hub Nodes" in report
...
def test_report_shows_raw_cohesion_scores():
    G, communities, cohesion, labels, hubs, surprises, detection, tokens = make_inputs()
    report = generate(G, communities, cohesion, labels, hubs, surprises, detection, tokens, "./project", min_community_size=1)
...
def test_portable_root_label():
    from halal_graphify.report import _portable_root_label
...
def test_report_work_memory_section_absent_without_overlay():
    ...
    G, communities, cohesion, labels, hubs, surprises, detection, tokens = make_inputs()
    before = generate(G, communities, cohesion, labels, hubs, surprises, detection,
                      tokens, "./project")
    assert "## Work-memory lessons" not in before
    # Explicit empty learning also omits the section.
    empty = generate(G, communities, cohesion, labels, hubs, surprises, detection,
...
def test_report_hubs_use_wikilinks_when_obsidian():
    ...
    G, communities, cohesion, labels, hubs, surprises, detection, tokens = make_inputs()
    labels = {cid: f"Widget {cid}" for cid in communities}
    report = generate(G, communities, cohesion, labels, hubs, surprises, detection, tokens, "./project", min_community_size=1, obsidian=True)
