"""Tests for halal_graphify/cache.py."""
import pytest
from pathlib import Path
from halal_graphify.cache import file_hash, cache_dir, load_cached, save_cached, cached_files, clear_cache, _body_content
...
def test_save_cached_relativizes_source_file(tmp_path):
    ...
    import json
    from halal_graphify.cache import save_cached, file_hash, cache_dir
...
def test_load_cached_absolutizes_source_file(tmp_path):
    ...
    from halal_graphify.cache import save_cached, load_cached
...
def test_load_cached_passes_through_legacy_absolute_source_file(tmp_path):
    """Cache entries written by an older halal-graphify (with absolute source_file
    inside) must still load correctly: the absolutize step is a no-op for
    already-absolute values."""
    import json
    from halal_graphify.cache import load_cached, file_hash, cache_dir
...
def test_cache_portable_across_roots(tmp_path):
    ...
    import json
    import shutil
    from halal_graphify.cache import save_cached, load_cached, file_hash, cache_dir
...
def _reset_stat_index():
    ...
    from halal_graphify import cache as _cache
...
def test_warm_cache_from_another_root_does_not_leak_that_root(tmp_path, monkeypatch):
    ...
    import halal_graphify.extract as ex
...
def test_cached_ids_round_trip_under_the_same_root(tmp_path):
    ...
    from halal_graphify.extract import _make_id
...
# AST cache entries are the output of halal-graphify's own extractor code, so they
# are only valid for the halal-graphify version that wrote them. Keying purely on
...
def test_ast_cache_invalidated_on_version_bump(tmp_path, monkeypatch):
    ...
    import halal_graphify.cache as cache_mod
    ...
    assert load_cached(f, root=tmp_path, kind="ast") is None, (
        "AST cache entry from a previous halal-graphify version must not be served"
    )
...
def test_legacy_unversioned_ast_entries_not_served(tmp_path):
    """Entries written by pre-versioning halal-graphify (flat cache/ or unversioned
    ...
    import json
    from halal_graphify.cache import file_hash, _GRAPHIFY_OUT
...
def test_file_hash_distinguishes_walked_symlink_paths_portably(
    ...
    from halal_graphify import cache as cache_mod
...
def test_semantic_cache_self_heals_legacy_symlink_collision(
    ...
    from halal_graphify.cache import check_semantic_cache, save_semantic_cache
...
def test_semantic_symlink_policy_uses_walked_identity(
    ...
    from halal_graphify.cache import load_cached, save_semantic_cache
...
def test_semantic_prune_removes_orphan_entries(tmp_path):
    ...
    current one."""
    from halal_graphify.cache import prune_semantic_cache
...
def test_semantic_prune_sweeps_both_namespaces_against_same_live_set(tmp_path):
    ...
    from halal_graphify.cache import prune_semantic_cache, save_semantic_cache
...
def test_prompt_fingerprint_stable_and_prompt_sensitive(tmp_path):
    ...
    from halal_graphify.cache import prompt_fingerprint
...
def test_semantic_cache_prompt_namespaced_layout(tmp_path):
    ...
    from halal_graphify.cache import prompt_fingerprint, save_semantic_cache
...
def test_edge_only_semantic_result_not_cached(tmp_path):
    ...
    from halal_graphify.cache import check_semantic_cache, load_cached, save_semantic_cache
...
def test_poisoned_edge_only_cache_entry_treated_as_miss(tmp_path):
    ...
    import json
    from halal_graphify.cache import cache_dir, file_hash, load_cached, prompt_fingerprint
...
def test_scope_semantic_result_drops_out_of_scope_groups(tmp_path):
    ...
    from halal_graphify.cache import scope_semantic_result
