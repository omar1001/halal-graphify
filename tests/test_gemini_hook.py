...
Since #522 it runs as `halal-graphify hook-guard gemini` (not a `python -c` one-liner
...
from halal_graphify.__main__ import _gemini_hook
...
def _run(cwd, *, graph: bool):
    ...
    return subprocess.run(
        [sys.executable, "-m", "halal_graphify", "hook-guard", "gemini"],
...
def test_matcher_and_command_shape():
    ...
    assert "halal-graphify" in cmd and "hook-guard gemini" in cmd
...
def test_honors_graphify_out_override(tmp_path):
    ...
    assert "halal-graphify query" in json.loads(r.stdout).get("additionalContext", "")
