...
import halal_graphify.cli as cli
...
def test_strict_first_read_denies_then_nudges(tmp_path, monkeypatch):
    ...
    assert "halal-graphify query" in json.loads(out1)["hookSpecificOutput"]["permissionDecisionReason"]
    # marker created
...
def test_install_hook_carries_strict_flag():
    from halal_graphify.install import _claude_pretooluse_hooks
