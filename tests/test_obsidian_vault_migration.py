...
notes while refusing to touch the user's. A vault created by an EARLIER halal-graphify
has no manifest, so `_owned` starts empty and every note halal-graphify itself wrote
last time reads as a user file: the re-export writes fresh notes beside the
stale ones, and warns that halal-graphify "did not create" files it did (#2863).

The fix adopts, once, the notes carrying halal-graphify's own signature:

  * node notes    - YAML frontmatter with a tag in the ``halal_graphify/`` namespace
  * community notes - no frontmatter at all, so matched on halal-graphify's own
                      filename prefix AND the Dataview query it writes

`.obsidian/graph.json` is deliberately NOT adopted: halal-graphify writes one, but so
...
from halal_graphify.export import to_obsidian

try:
    from halal_graphify.export import _adopt_pre_manifest_notes, _is_graphify_note
...
def pre_manifest_vault(tmp_path):
    """A vault as an older halal-graphify would have left it: its notes, no manifest."""
...
def test_a_note_merely_mentioning_graphify_is_not_adopted(pre_manifest_vault):
    ...
    mine.write_text("# Notes\n\nI use halal_graphify/document tags manually.\n", encoding="utf-8")
    _export(pre_manifest_vault, ["Alpha", "Beta", "Gamma"])
    assert "I use halal-graphify" in mine.read_text(encoding="utf-8")
...
def test_obsidian_config_is_not_adopted(pre_manifest_vault):
    """halal-graphify writes .obsidian/graph.json, but so does Obsidian. With no
