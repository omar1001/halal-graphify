...
import pytest
from halal_graphify.extract import (
...
def test_cpp_string_tests_punctuation_only_names_stay_distinct(tmp_path):
    ...
    r = extract_cpp(f)
    from halal_graphify.extract import _make_id, _file_stem
...
def test_metal_is_code_extension():
    from halal_graphify.detect import CODE_EXTENSIONS
...
from halal_graphify.extract import extract_elixir
...
# ── Objective-C ──────────────────────────────────────────────────────────────
from halal_graphify.extract import extract_objc
...
def test_objc_header_dispatch_routes_objc_not_c(tmp_path):
    ...
    from halal_graphify.extract import _get_extractor, extract_objc as _eo, extract_c as _ec
...
def test_ts_injected_field_ambiguous_type_emits_no_edge(tmp_path):
    ...
    the hub-node guard bails — no calls edge rather than a guess (#1316)."""
    from halal_graphify.extract import extract
    (tmp_path / "a" ).mkdir()
    (tmp_path / "b").mkdir()
...
from halal_graphify.extract import extract_markdown
...
def test_markdown_frontmatter_fallback_parses_flat_keys():
    ...
    from halal_graphify.extractors.markdown import _parse_frontmatter_fallback
...
def test_markdown_heading_id_is_stable_regardless_of_frontmatter():
    ...
    from halal_graphify.extractors.base import _make_id, _file_stem
    r = _md_extract("---\ntitle: Doc\ntags: [a, b]\n---\n\n# Overview\n\n## Details\n")
...
# ── Zig ───────────────────────────────────────────────────────────────────────
from halal_graphify.extract import extract_zig
