...
single-definition hub-node check — an ambiguous or uninferable receiver yields ZERO
...
def test_cpp_cross_file_member_call_connects_with_relative_paths(tmp_path):
    ...
    `halal-graphify extract .` usage), which is what exposes resolution gaps; an earlier
...
def test_cpp_hubnode_guard_ambiguous_and_unknown_receiver(tmp_path: Path):
...
def test_objc_hubnode_guard_ambiguous_selector(tmp_path: Path):
