"""Tests for halal_graphify.transcribe — video/audio transcription support."""
...
from halal_graphify.transcribe import (
    VIDEO_EXTENSIONS,
...
def test_build_whisper_prompt_no_nodes():
    """Empty hub_nodes returns fallback prompt."""
...
def test_build_whisper_prompt_returns_topic_string():
    """Returns a topic-based prompt from hub node labels — no LLM call."""
    hub_nodes = [{"label": "neural networks"}, {"label": "transformers"}, {"label": "attention"}]
    with patch.dict(os.environ, {}, clear=False):
        os.environ.pop("GRAPHIFY_WHISPER_PROMPT", None)
        prompt = build_whisper_prompt(hub_nodes)
...
def test_transcribe_force_reruns(tmp_path):
    ...
    with patch("halal_graphify.transcribe._get_whisper", return_value=lambda *a, **kw: fake_model):
...
def test_transcribe_missing_faster_whisper(tmp_path):
    ...
    with patch("halal_graphify.transcribe._get_whisper", side_effect=ImportError("faster-whisper not installed")):
...
def test_transcribe_all_skips_failed(tmp_path):
    ...
    with patch("halal_graphify.transcribe.transcribe", side_effect=RuntimeError("boom")):
