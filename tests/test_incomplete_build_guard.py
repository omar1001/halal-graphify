"""Tests for the incomplete-build shrink-guard on `halal-graphify extract`.
...
def _seed_to_json_recorder(monkeypatch, *, returns=True):
    ...
    monkeypatch.setattr("halal_graphify.export.to_json", _stub)
    return rec
...
def _arm_extract(monkeypatch, tmp_path, *, chunk_total, chunk_succeeded, extra_argv=()):
    ...
    monkeypatch.setattr("halal_graphify.llm.extract_corpus_parallel", _stub_corpus)
...
def _arm_no_cluster(monkeypatch, tmp_path, *, extra_argv=()):
    ...
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["halal-graphify", "extract", str(corpus), "--backend", "claude", "--no-cluster",
         "--out", str(out_dir), *extra_argv],
    )
