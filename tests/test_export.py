...
from halal_graphify.cluster import cluster
from halal_graphify.export import to_json, to_cypher, to_graphml, to_html, to_canvas, to_obsidian
...
def test_to_json_commit_fallback_uses_output_repo_not_cwd(tmp_path, monkeypatch):
    ...
    # be in — running `halal-graphify extract <target>` from another repo's root
...
def test_to_html_title_uses_portable_path_not_host_absolute():
    ...
    title = m.group(1)
    assert title.startswith("halal-graphify - ")
    label = title[len("halal-graphify - "):]
...
def test_html_document_title_helper_windows_and_relative():
    from halal_graphify.exporters.html import _html_document_title
...
def test_to_html_neighbor_links_have_no_inline_onclick_xss():
    ...
    (from a document or a scraped `halal-graphify add` URL) inject a live event handler
...
def test_obsidian_safe_stem_all_dots_label_falls_back_to_unnamed():
    ...
    from halal_graphify.export import _obsidian_safe_stem
...
# ── Existing-vault safety: halal-graphify must not clobber user notes / .obsidian (#1506) ──
...
def test_to_obsidian_preserves_existing_user_notes_and_obsidian_config():
    ...
    collides with a halal-graphify node name, nor their .obsidian/ graph settings."""
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # non-colliding halal-graphify note still written
...
def test_to_obsidian_rerun_updates_own_notes_but_not_user_files():
    """A re-run overwrites halal-graphify's own prior notes (via the manifest) but leaves a
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        assert (out / "Database.md").exists()  # halal-graphify re-wrote its own
...
def test_to_obsidian_rerun_prunes_removed_nodes():
    """#1896: re-exporting into the same vault must delete halal-graphify's own notes for
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        # surviving halal-graphify notes and the user's own note remain
        assert (out / "Database.md").exists() and (out / "Server.md").exists()
...
def test_backup_no_graph_json(tmp_path):
    ...
    from halal_graphify.export import backup_if_protected
...
def test_existing_graph_node_count(tmp_path):
    from halal_graphify.export import existing_graph_node_count, MALFORMED_GRAPH
    p = tmp_path / "graph.json"
...
def test_hyperedge_perimeter_uses_convex_hull_not_member_order():
    ...
    from halal_graphify.exporters.html import _hyperedge_script
