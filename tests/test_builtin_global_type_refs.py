...
`references[call]` edges and a false hub node. The cross-file CALL resolver
...
def test_builtin_date_type_ref_does_not_bind_to_user_DATE(tmp_path):
    ...
    assert deg <= 1, f"user DATE should not be a hub node; degree={deg}"
...
def test_builtin_static_call_does_not_bind_to_user_symbol(tmp_path):
    ...
    # binds to a same-spelled user `const DATE`, a false hub node. A typed param in
