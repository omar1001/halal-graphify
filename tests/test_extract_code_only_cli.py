"""`halal-graphify extract --code-only` indexes code without an LLM key (#1734).
...
def _run(repo: Path, *extra: str):
    ...
    return subprocess.run(
        [PYTHON, "-m", "halal_graphify", "extract", ".", *extra],
...
def test_extract_usage_advertises_code_only(tmp_path):
    ...
    by triggering the no-key error. `halal-graphify extract` with no path prints usage."""
    r = subprocess.run(
        [PYTHON, "-m", "halal_graphify", "extract"],
...
def test_no_gitignore_setting_persists_across_flagless_extract(tmp_path):
    ...
    `halal-graphify extract` must NOT clobber it back to honoring .gitignore (which
