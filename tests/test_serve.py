...
from halal_graphify.serve import (
    _strip_diacritics,
...
def test_score_nodes_multiword_exact_label_outranks_superset():
    ...
    Regression for the `halal-graphify path` "No path found" bug: every node sharing
...
def _force_full_scan(monkeypatch):
    ...
    monkeypatch.setattr("halal_graphify.serve._trigram_candidates", lambda *a, **k: None)
...
def test_query_terms_filters_only_short_english_terms(monkeypatch):
    import halal_graphify.serve as serve_mod
...
def test_load_graph_corrupted_json_prints_recovery_message(tmp_path, capsys):
    ...
    assert "Re-run /halal-graphify to rebuild" in err
...
def test_load_graph_rejects_oversized_file(monkeypatch, tmp_path, capsys):
    ...
    p = tmp_path / "graph.json"
    p.write_text(json.dumps(data))
    monkeypatch.setattr("halal_graphify.security._MAX_GRAPH_FILE_BYTES", 16)
    with pytest.raises(SystemExit):
        _load_graph(str(p))
    err = capsys.readouterr().err
...
def test_load_graph_accepts_under_cap(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("halal_graphify.security._MAX_GRAPH_FILE_BYTES", 10 * 1024 * 1024)
    G2 = _load_graph(str(p))
    assert G2.number_of_nodes() == G.number_of_nodes()
...
def test_query_graph_text_context_filter_aliases_resolve():
    import networkx as nx
    from halal_graphify.serve import _normalize_context_filters
...
def test_score_query_matches_legacy_under_full_scan_fallback(monkeypatch):
    ...
    monkeypatch.setattr(
        "halal_graphify.serve._trigram_candidates", lambda G, needles: None
    )
...
def test_query_graph_text_makes_exactly_one_score_query_call(monkeypatch):
    ...
    monkeypatch.setattr("halal_graphify.serve._score_query", counting_sq)
    monkeypatch.setattr("halal_graphify.serve._score_nodes", counting_sn)
...
def test_snake_case_identifier_still_matches_itself():
    ...
    G = nx.Graph()
    G.add_node("n1", label="_query_terms", source_file="halal_graphify/serve.py",
