...
from halal_graphify.analyze import hub_nodes, surprising_connections, _is_concept_node, graph_diff, _surprise_score, _file_category, _is_json_key_node, find_import_cycles, suggest_questions
...
def test_hub_nodes_returns_list():
    G = make_graph()
    result = hub_nodes(G, top_n=3)
    assert isinstance(result, list)
    assert len(result) <= 3


def test_hub_nodes_sorted_by_degree():
    G = make_graph()
    result = hub_nodes(G, top_n=10)
    degrees = [r["degree"] for r in result]
    assert degrees == sorted(degrees, reverse=True)


def test_hub_nodes_have_required_keys():
    G = make_graph()
    result = hub_nodes(G, top_n=1)
...
# --- npm dep-block key hub-node filtering tests ---
...
def test_hub_nodes_excludes_npm_dep_block_keys(dep_key: str) -> None:
    """npm package.json dep-block keys must be filtered from hub_nodes output.
    ...
    Asserts that hub_nodes() excludes the dep-block node even when it has the
    ...
    # Wire up enough edges so dep_node has high degree — it would be a hub-node
    # without the filter.
    for i in range(20):
        peer = f"pkg_{i}"
        G.add_node(
    ...
    assert "dep_node" not in result_ids, (
        f"hub_nodes() should filter npm dep-block key '{dep_key}' "
        f"but it appeared in the result: {result}"
    )
    assert "real_node" in result_ids, (
        f"hub_nodes() should include real-domain node 'AuthService' "
...
def test_hub_nodes_excludes_json_noise():
    """hub_nodes must not return generic JSON key nodes like 'name' or 'id'."""
    G = nx.Graph()
...
def test_hub_nodes_filter_is_case_insensitive():
...
def _make_file_node(path: str) -> tuple[str, dict]:
    """Create a graph node resembling real halal-graphify schema."""
    nid = _make_id(path)
