"""`halal-graphify merge-graphs` tolerates inputs that disagree on graph type (#1606).
...
def _run(args, cwd):
    return subprocess.run([PYTHON, "-m", "halal_graphify"] + args, cwd=cwd,
...
def test_distinct_repo_tags_unit(tmp_path):
    from halal_graphify.build import distinct_repo_tags
