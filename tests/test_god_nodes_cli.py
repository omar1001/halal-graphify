"""`halal-graphify hub-nodes` CLI subcommand (#2004 part 2).

hub_nodes has long been an analyzer + MCP tool + README-advertised capability
but was never wired as a CLI subcommand, so `halal-graphify hub_nodes` errored with
...
def test_hub_nodes_cli_text_output(monkeypatch, tmp_path, capsys):
    gp = _write_graph(tmp_path)
    _run(monkeypatch, ["halal-graphify", "hub-nodes", "--graph", str(gp)])
    out = capsys.readouterr().out
    assert "Hub nodes (most connected):" in out
    assert "Auth" in out
    assert "edges" in out
    assert "auth.py" not in out  # file node excluded from the ranking


def test_hub_nodes_cli_underscore_alias(monkeypatch, tmp_path, capsys):
    # The exact spelling from the issue title.
    gp = _write_graph(tmp_path)
    _run(monkeypatch, ["halal-graphify", "hub_nodes", "--graph", str(gp)])
    assert "Auth" in capsys.readouterr().out


def test_hub_nodes_cli_top_limits(monkeypatch, tmp_path, capsys):
    gp = _write_graph(tmp_path)
    _run(monkeypatch, ["halal-graphify", "hub-nodes", "--graph", str(gp), "--top", "1"])
    body = capsys.readouterr().out
    assert body.count(" edges") == 1


def test_hub_nodes_cli_json(monkeypatch, tmp_path, capsys):
    gp = _write_graph(tmp_path)
    _run(monkeypatch, ["halal-graphify", "hub-nodes", "--graph", str(gp), "--json"])
...
def test_hub_nodes_cli_missing_graph_errors(monkeypatch, tmp_path, capsys):
    with pytest.raises(SystemExit) as exc:
        _run(monkeypatch, ["halal-graphify", "hub-nodes", "--graph", str(tmp_path / "nope.json")])
