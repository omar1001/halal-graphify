...
`sed q`) is routine. halal-graphify used to keep writing after the reader disconnected,
...
def test_help_survives_reader_closing_pipe_early():
    """`halal-graphify --help | head -n1` must leave halal-graphify exiting 0, not 255."""
    producer = subprocess.Popen(
        [PYTHON, "-m", "halal_graphify", "--help"], stdout=subprocess.PIPE
    )
...
def test_small_buffered_output_survives_reader_that_reads_nothing():
    ...
    producer = subprocess.Popen(
        [PYTHON, "-m", "halal_graphify", "--version"], stdout=subprocess.PIPE
    )
    reader = subprocess.Popen(
