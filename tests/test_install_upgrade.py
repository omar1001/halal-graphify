...
Pre-fix, the installers wrote a "## halal-graphify" section with report-first
instructions and skipped writing if the marker was already present. So users
who installed halal-graphify and then upgraded to the fixed package still had the
...
- For cross-module "how does X relate to Y" questions, prefer `halal-graphify query "<question>"`, `halal-graphify path "<A>" "<B>"`, or `halal-graphify explain "<concept>"` over grep — these traverse the graph's EXTRACTED + INFERRED edges instead of scanning files
...
_OLD_CURSOR_RULE = """\
---
description: halal-graphify knowledge graph context
alwaysApply: true
---

This project has a halal-graphify knowledge graph at graphify-out/.

- Before answering architecture or codebase questions, read graphify-out/GRAPH_REPORT.md for hub nodes and community structure
- If graphify-out/wiki/index.md exists, navigate it instead of reading raw files
"""
...
halal-graphify: A knowledge graph of this project lives in `graphify-out/`. \
If `graphify-out/GRAPH_REPORT.md` exists, read it before answering architecture questions, \
tracing dependencies, or searching files — it contains hub nodes, community structure, \
and surprising connections the graph found.
"""


_OLD_HOOK_PAYLOAD_SNIPPET = "Read graphify-out/GRAPH_REPORT.md for hub nodes and community structure before searching raw files"
...
def _assert_query_first(text: str, ctx: str) -> None:
    assert "halal-graphify query" in text, (
        f"{ctx}: new 'halal-graphify query' guidance missing after upgrade"
    )
...
def test_claude_install_upgrades_stale_section(tmp_path, monkeypatch):
    ...
    `halal-graphify claude install` again after upgrading to a fixed package."""
...
def test_claude_install_upgrades_stale_hook_payload(tmp_path, monkeypatch):
    ...
    # Since #522 the nudge text lives in the `halal-graphify hook-guard` subcommand, not
    ...
    assert "hook-guard" in new_settings_text, (
        "new hook payload should route to the `halal-graphify hook-guard` subcommand"
    )
...
def test_cursor_install_upgrades_stale_rule(tmp_path, monkeypatch):
    """Same upgrade behavior for .cursor/rules/halal-graphify.mdc.
    The Cursor rule file is wholly graphify-owned; overwrite on upgrade."""
    monkeypatch.chdir(tmp_path)
    rule_path = tmp_path / ".cursor" / "rules" / "halal-graphify.mdc"
    ...
    assert "read graphify-out/GRAPH_REPORT.md for hub nodes and community structure" not in after
    _assert_query_first(after, ".cursor/rules/halal-graphify.mdc")
...
def test_kiro_install_ships_references_sidecar_and_version_stamp(tmp_path, monkeypatch):
    ...
    skill_dir = tmp_path / ".kiro" / "skills" / "halal-graphify"
