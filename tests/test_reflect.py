"""Tests for `halal-graphify reflect` and the work-memory reflection layer.

`halal-graphify reflect` reads the outcome-tagged Q&A docs that `halal-graphify save-result`
...
from halal_graphify.ingest import save_query_result
from halal_graphify.reflect import (
    aggregate_lessons,
    lessons_fresh,
    load_memory_docs,
    parse_memory_doc,
...
def _write_raw_doc(mem: Path, filename: str, date: str, *, outcome="dead_end",
    ...
    lines = ["---", 'type: "query"', f'date: "{date}"', f'question: "{question}"',
             'contributor: "halal-graphify"', f'outcome: "{outcome}"']
    if nodes:
...
from halal_graphify.reflect import (  # noqa: E402
