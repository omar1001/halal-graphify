...
def test_extraction_instructions_ride_in_user_turn(fake_claude):
    ...
    assert "halal-graphify semantic extraction agent" in sent
...
# `--json-schema`, halal-graphify constrains the output shape structurally so the
...
def test_json_schema_flag_added_when_cli_supports_it(monkeypatch, fake_claude):
    ...
    pins the top-level {nodes, edges} shape halal-graphify parses."""
