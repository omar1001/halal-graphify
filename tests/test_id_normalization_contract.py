...
Both callers now delegate to :mod:`halal_graphify.ids`, so they share one
...
from halal_graphify.build import _normalize_id
from halal_graphify.extract import _make_id
from halal_graphify.ids import make_id, normalize_id
...
def test_both_callers_share_one_implementation():
    ...
    underlying function object as halal_graphify.ids.normalize_id."""
    # build._normalize_id is imported directly from halal_graphify.ids.
    ...
    from halal_graphify.mcp_ingest import _make_id as _mcp_make_id
    from halal_graphify.symbol_resolution import _bash_make_id
