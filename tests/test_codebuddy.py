"""Tests for halal-graphify codebuddy install / uninstall commands."""
import json
...
def _skill_path_user(tmp_path):
    return tmp_path / ".codebuddy" / "skills" / "halal-graphify" / "SKILL.md"


def _skill_path_project(project_dir):
    return project_dir / ".codebuddy" / "skills" / "halal-graphify" / "SKILL.md"
...
# ---------------------------------------------------------------------------
# User-scope install (halal-graphify install --platform codebuddy)
# ---------------------------------------------------------------------------

def test_codebuddy_install_user_creates_skill_file(tmp_path):
    """User-scope install copies skill to ~/.codebuddy/skills/halal-halal_graphify/SKILL.md."""
...
def test_codebuddy_skill_file_contains_frontmatter(tmp_path):
    """Installed skill file must include halal-graphify YAML frontmatter."""
    _codebuddy_install_user(tmp_path)
    content = _skill_path_user(tmp_path).read_text()
    assert "name: halal-graphify" in content
    assert "description:" in content


def test_codebuddy_skill_file_references_graphify_query(tmp_path):
    """/halal-graphify skill must mention halal-graphify query (query-first policy)."""
    _codebuddy_install_user(tmp_path)
    content = _skill_path_user(tmp_path).read_text()
    assert "halal-graphify query" in content or "/halal-graphify query" in content


# ---------------------------------------------------------------------------
# Project-scope install (halal-graphify codebuddy install)
# ---------------------------------------------------------------------------

def test_codebuddy_install_project_writes_codebuddy_md(tmp_path):
    """Project-scope install writes CODEBUDDY.md with halal-graphify section."""
...
def test_codebuddy_install_hook_has_bash_matcher(tmp_path):
    ...
    assert any("halal-graphify" in str(h) for h in bash_hooks)
...
def test_codebuddy_install_hook_has_read_glob_matcher(tmp_path):
    ...
    assert any("halal-graphify" in str(h) for h in read_hooks)


def test_codebuddy_install_idempotent(tmp_path):
    """Re-install does not duplicate ## halal-graphify sections."""
    from halal_graphify.__main__ import codebuddy_install
    codebuddy_install(tmp_path)
    codebuddy_install(tmp_path)
    md = _codebuddy_md_path(tmp_path)
...
def test_codebuddy_install_upgrades_stale_section(tmp_path):
    """Re-install replaces an old halal-graphify section with the current template."""
    from halal_graphify.__main__ import codebuddy_install, _CODEBUDDY_MD_MARKER
...
def test_codebuddy_install_hint_git_add(tmp_path, capsys):
    ...
    try:
        ...
        with patch("halal_graphify.__main__.Path.home", return_value=home):
            sys.argv = ["halal-graphify", "codebuddy", "install"]
            main()
    finally:
        import os
        os.chdir(old_cwd)
...
def test_codebuddy_uninstall_removes_section(tmp_path):
    """Uninstall removes the ## halal-graphify section from CODEBUDDY.md."""
...
def test_codebuddy_uninstall_noop_if_no_section(tmp_path):
    """Uninstall should not error when CODEBUDDY.md exists but no halal-graphify section."""
    from halal_graphify.__main__ import codebuddy_uninstall
...
def test_codebuddy_uninstall_preserves_other_content(tmp_path):
    ...
    codebuddy_install(tmp_path)
    codebuddy_uninstall(tmp_path)
    # When halal-graphify section was appended, uninstall removes it and the file
...
def test_uninstall_all_removes_codebuddy_md(tmp_path, monkeypatch):
    """halal-graphify uninstall must clean up CODEBUDDY.md."""
    from halal_graphify.__main__ import main
    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
    ...
    with patch("halal_graphify.__main__.Path.home", return_value=home):
        monkeypatch.setattr(sys, "argv", ["halal-graphify", "codebuddy", "install"])
        main()
...
def test_uninstall_all_removes_codebuddy_hook(tmp_path, monkeypatch):
    """halal-graphify uninstall must clean up .codebuddy/settings.json hooks."""
    from halal_graphify.__main__ import main
    home = tmp_path / "home"
    project = tmp_path / "project"
    project.mkdir()
...
def test_codebuddy_in_platform_config():
    ...
    from halal_graphify.__main__ import _PLATFORM_CONFIG
...
def test_codebuddy_platform_skill_destination_user_scope(tmp_path):
    """User-scope destination must be ~/.codebuddy/skills/halal-halal_graphify/SKILL.md."""
    from halal_graphify.__main__ import _platform_skill_destination
    with patch("halal_graphify.__main__.Path.home", return_value=tmp_path):
        dst = _platform_skill_destination("codebuddy", project=False)
    assert dst == tmp_path / ".codebuddy" / "skills" / "halal-graphify" / "SKILL.md"


def test_codebuddy_platform_skill_destination_project_scope(tmp_path):
    """Project-scope destination must be <project>/.codebuddy/skills/halal-halal_graphify/SKILL.md."""
    from halal_graphify.__main__ import _platform_skill_destination
...
def test_codebuddy_in_main_help_text(capsys, monkeypatch):
    """`halal-graphify --help` must list codebuddy in the platform list and per-platform section."""
    from halal_graphify.__main__ import main
    monkeypatch.setattr(sys, "argv", ["halal-graphify", "--help"])
    main()
    captured = capsys.readouterr().out
    # codebuddy should appear in the top-level platform list
    assert "|codebuddy)" in captured or "codebuddy" in captured, (
        "codebuddy missing from `halal-graphify --help` platform list"
    )
...
def test_codebuddy_skill_file_exists_in_package():
    ...
    skill = Path(halal_graphify.__file__).parent / "skill.md"
    assert skill.exists(), "skill.md missing from package"


def test_codebuddy_installation_roundtrip(tmp_path):
    """Install then uninstall leaves no trace of halal-graphify CODEBUDDY.md or hook."""
    from halal_graphify.__main__ import codebuddy_install, codebuddy_uninstall
    # Pre-existing project file
    _codebuddy_md_path(tmp_path).write_text("# My project\n")
    # One section above halal-graphify to test cleanup
    codebuddy_install(tmp_path)
    codebuddy_uninstall(tmp_path)
