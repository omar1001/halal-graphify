...
2. A template lint over the generated ``halal_graphify/skill*.md`` bodies (and the
...
def _skill_bodies() -> list[Path]:
    paths = sorted(REPO_ROOT.glob("halal_graphify/skill*.md"))
    assert paths, "no generated halal_graphify/skill*.md found"
    return paths
