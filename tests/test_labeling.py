...
from halal_graphify.llm import label_communities, generate_community_labels
...
def test_label_communities_happy_path(monkeypatch):
    ...
    monkeypatch.setattr("halal_graphify.llm._call_llm", fake_call)
...
def test_label_cli_passes_model_override(tmp_path, monkeypatch):
    import halal_graphify.__main__ as cli
    ...
    def fake_generate(G, communities, *, backend=None, model=None, hubs=None,
    ...
    monkeypatch.setattr("halal_graphify.llm.generate_community_labels", fake_generate)
    monkeypatch.setattr("halal_graphify.export.to_html", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        sys,
        "argv",
        [
...
def test_label_cli_missing_only_preserves_existing_labels(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["halal-graphify", "label", str(tmp_path), "--missing-only", "--backend", "gemini", "--no-viz"],
    )
...
def test_label_communities_partial_reply_fills_placeholder(monkeypatch):
    G, communities = _graph()
    monkeypatch.setattr("halal_graphify.llm._call_llm",
...
def test_hubs_as_dicts_do_not_crash(monkeypatch):
    """hub_nodes() returns list[dict] with an 'id' key, not bare ids."""
    G, communities = _graph()
    monkeypatch.setattr("halal_graphify.llm._call_llm",
                        lambda p, *, backend, max_tokens=200: '{"0":"Orders","1":"Pay"}')
    hubs = [{"id": "order_repo", "label": "OrderRepository"}]
    labels = label_communities(G, communities, backend="gemini", hubs=hubs)
    assert labels == {0: "Orders", 1: "Pay"}
...
def test_label_communities_all_batches_fail_raises(monkeypatch):
    ...
    monkeypatch.setattr("halal_graphify.llm._call_llm", always_fail)
...
def test_label_communities_parallel_matches_sequential(monkeypatch):
    ...
    monkeypatch.setattr("halal_graphify.llm._label_batch_with_retry", fake_batch)
...
def test_cluster_commands_render_aggregated_html_above_viz_limit(
    ...
    monkeypatch.setattr(
        "halal_graphify.llm.generate_community_labels",
    ...
    argv = ["halal-graphify", command, str(tmp_path)]
...
def test_cluster_only_preserves_but_does_not_claim_unusable_aggregate(
    ...
    monkeypatch.setattr(cli, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(
        importlib.import_module("halal_graphify.cluster"),
        "cluster",
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["halal-graphify", "cluster-only", str(tmp_path), "--no-label"],
    )
...
def test_cluster_only_restores_html_after_unexpected_render_failure(
    ...
    monkeypatch.setattr(cli, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(
        importlib.import_module("halal_graphify.export"),
        "to_html",
        fail_render,
    )
    monkeypatch.setattr(
...
def test_cluster_only_marks_html_stale_before_report_generation(
    ...
    monkeypatch.setenv("GRAPHIFY_VIZ_NODE_LIMIT", "3")
    monkeypatch.setattr(cli, "_check_skill_version", lambda _: None)
    monkeypatch.setattr("halal_graphify.report.generate", interrupt_report)
    monkeypatch.setattr(
        sys,
        "argv",
        ["halal-graphify", "cluster-only", str(tmp_path), "--no-label"],
...
def test_cluster_only_refused_graph_write_preserves_existing_stale_marker(
    ...
    monkeypatch.setattr(cli, "_check_skill_version", lambda _: None)
    monkeypatch.setattr("halal_graphify.export.to_json", lambda *args, **kwargs: False)
    monkeypatch.setattr(
        sys,
        "argv",
        ["halal-graphify", "cluster-only", str(tmp_path), "--no-label"],
...
def test_cluster_only_no_label_does_not_persist_placeholders(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cli, "_check_skill_version", lambda _: None)
    monkeypatch.setattr("halal_graphify.export.to_html", lambda *a, **k: None)
    # No-backend fallback shape: returns placeholders, which must not clobber hubs.
    monkeypatch.setattr("halal_graphify.llm.generate_community_labels",
                        lambda G, comms, **k: ({cid: f"Community {cid}" for cid in comms}, "none"))

    monkeypatch.setattr(sys, "argv", ["halal-graphify", "cluster-only", str(tmp_path), "--no-label", "--no-viz"])
    cli.main()
    ...
    monkeypatch.setattr(sys, "argv", ["halal-graphify", "cluster-only", str(tmp_path), "--no-viz"])
    cli.main()
...
def test_cluster_only_heals_persisted_placeholder_but_reuses_genuine(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("halal_graphify.llm.generate_community_labels", _fail_generate)
...
def test_label_prompt_lines_use_bare_cid_keys():
    ...
    from halal_graphify.llm import _community_label_lines
...
def test_label_cli_drops_sentinel_and_bare_key_echoes(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        sys, "argv",
        ["halal-graphify", "label", str(tmp_path), "--backend", "gemini", "--no-viz"],
    )
