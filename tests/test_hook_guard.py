"""Rigorous edge-case coverage for the `halal-graphify hook-guard` subcommand (#522).
...
from halal_graphify import __main__ as m
...
def _invoke(kind, payload, tmp_path, monkeypatch, *, graph=True, out_name="graphify-out"):
    monkeypatch.setattr("halal_graphify.paths.GRAPHIFY_OUT", out_name)
    monkeypatch.setattr("halal_graphify.paths.GRAPHIFY_OUT_NAME", out_name)
...
def test_search_top_level_command_without_tool_input(tmp_path, monkeypatch):
    ...
    assert "halal-graphify query" in out
...
def test_read_nudges(tool_input, tmp_path, monkeypatch):
    ...
    assert "halal-graphify query" in out, f"{tool_input!r} should nudge"
...
def test_search_out_path_error_is_swallowed(tmp_path, monkeypatch):
    ...
    def _boom(*a, **k):
        raise OSError("boom")
    monkeypatch.setattr("halal_graphify.paths.out_path", _boom)
...
def test_gemini_allow_with_nudge(tmp_path, monkeypatch):
    ...
    assert "halal-graphify query" in payload["additionalContext"]
...
def _cli(args, tmp_path, stdin=""):
    return subprocess.run(
        [sys.executable, "-m", "halal_graphify", *args],
...
def test_read_nudge_em_dash_survives_utf8(tmp_path):
    ...
    (tmp_path / "graphify-out" / "graph.json").write_text("{}", encoding="utf-8")
    r = subprocess.run(
        [sys.executable, "-m", "halal_graphify", "hook-guard", "read"],
