...
import pytest
from halal_graphify.hooks import install, uninstall, status, _hooks_dir, _HOOK_MARKER, _CHECKOUT_MARKER
...
def test_install_embeds_pinned_interpreter(tmp_path):
    ...
    halal-graphify launcher on PATH (uv tool / pipx isolation, #1127).

    When halal-graphify is installed via `uv tool install halal-graphify` or `pipx install
    halal-graphify`, the interpreter lives in an isolated venv and the launcher is in
    ~/.local/bin.  GUI git clients and CI runners often run with a minimal PATH
    that omits that directory, so `command -v halal-graphify` fails, the python3/python
    fallbacks cannot import halal_graphify (wrong venv), and the hook silently exits 0.
...
def test_hook_check_no_additionalContext(tmp_path):
    """halal-graphify hook-check must not emit additionalContext — Codex Desktop rejects it."""
    import sys
    ...
    result = subprocess.run(
        [sys.executable, "-m", "halal_graphify", "hook-check"],
        cwd=tmp_path,
        capture_output=True,
        text=True,
    )
...
from halal_graphify.hooks import (  # noqa: E402
    _HOOK_SCRIPT,
    _CHECKOUT_SCRIPT,
...
def test_windows_hookspath_rejected_no_junk_dir_on_posix(tmp_path, monkeypatch, winpath):
    ...
    monkeypatch.setattr("halal_graphify.hooks.os.name", "posix")
    repo = _make_git_repo(tmp_path)
...
def test_probes_use_find_spec_not_full_import():
    """`python -c "import halal_graphify"` executes the FULL package import — 10s+ on a
    ...
    from halal_graphify.hooks import _PYTHON_DETECT
    assert '-c "import halal_graphify"' not in _PYTHON_DETECT, (
...
def test_shebang_read_is_null_byte_safe():
    """On Windows, `command -v halal-graphify` can return the launcher path WITHOUT its
...
def test_probe_prefers_sibling_python_exe_on_windows_layouts():
    """pip on Windows puts Scripts/halal-graphify(.exe) beside ..\\python.exe (or
...
def _worktree_guard_snippet() -> str:
    from halal_graphify.hooks import _WORKTREE_GUARD
...
def test_install_registers_merge_driver(tmp_path):
    ...
    assert any(
        "graph.json" in line and "merge=halal-graphify" in line
...
def test_install_merge_driver_idempotent(tmp_path):
    ...
    matches = [l for l in lines if "merge=halal-graphify" in l]
...
def test_install_preserves_existing_gitattributes(tmp_path):
    ...
    assert "merge=halal-graphify" in content


def test_uninstall_removes_merge_driver_keeps_other_attrs(tmp_path):
    """uninstall() must unset merge.graphify.* and remove only the halal-graphify
    ...
    content = (repo / ".gitattributes").read_text(encoding="utf-8")
    assert "*.png binary" in content
    assert "merge=halal-graphify" not in content


@pytest.mark.parametrize("exe", [
    r"C:\Users\First Last\AppData\Roaming\uv\tools\halal-graphify\Scripts\python.exe",
    r"C:\Program Files\Python312\python.exe",
    "/home/first last/.local/share/uv/tools/halal-halal_graphify/bin/python",
])
def test_pinned_python_accepts_paths_containing_spaces(exe, monkeypatch):
    ...
    from halal_graphify.hooks import _pinned_python
...
def test_merge_driver_quotes_interpreter_with_spaces(tmp_path, monkeypatch):
    ...
    from halal_graphify.hooks import install

    exe = r"C:\Users\First Last\AppData\Roaming\uv\tools\halal-graphify\Scripts\python.exe"
    repo = _make_git_repo(tmp_path)
    monkeypatch.setattr(_sys, "executable", exe)
    install(repo)
    ...
    assert driver.endswith("-m halal_graphify merge-driver %O %A %B")
