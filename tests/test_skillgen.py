...
skillgen renders halal-graphify's committed skill artifacts from human-edited
...
def test_no_version_or_timestamp_in_output():
    ...
    from halal_graphify.__main__ import __version__
...
def _claude_artifacts():
    ...
    core = next(a for a in arts if a.path == "halal_graphify/skill.md")
    refs = {a.path.rsplit("/", 1)[-1]: a.content for a in arts if a.path != "halal_graphify/skill.md"}
...
def test_lean_core_has_no_reference_only_content():
    ...
    assert "from halal_graphify.build import build_merge" not in core
    assert "halal-graphify cluster-only ." not in core
    ...
    assert "halal-graphify export wiki" not in core
    assert "halal-graphify export neo4j" not in core
    # The add / watch / hook flows live only in their references.
    assert "from halal_graphify.ingest import ingest" not in core
    assert "halal-graphify hook install" not in core
    assert "python3 -m halal_graphify.watch" not in core
...
def test_lean_core_runs_default_pipeline_with_zero_references():
    ...
    for needed in (
        "### Step 1 - Ensure halal-graphify is installed",
        "### Step 2 - Detect files",
...
def test_extraction_states_no_api_key_required_for_every_host():
    ...
    for a in bodies:
        assert "halal-graphify needs no API key" in a.content, a.path
        ...
        if tip in a.content:
            assert a.content.index("halal-graphify needs no API key") < a.content.index(tip), \
...
def test_references_contain_no_core_pipeline_content():
    ...
    core_only_markers = (
        "from halal_graphify.cluster import cluster, score_all",
        "### Step 4 - Build graph, cluster, analyze, generate outputs",
        "### Step 5 - Label communities",
        "## Honesty Rules",
    )
...
def test_query_heading_is_homed_in_core_stub_only():
    ...
    assert "## For /halal-graphify query" in core_headings
    assert "## For /halal-graphify query" not in query_headings
    # The deeper query content moved into the reference.
    assert "## For /halal-graphify path" in query_headings
    assert "## For /halal-graphify explain" in query_headings
    assert "## For /halal-graphify path" not in core_headings
...
UNIFIED_DESCRIPTION = (
    ...
    "should be treated as a halal-graphify query first. Turns any input (code, docs, "
    "papers, images, videos) into a persistent knowledge graph with hub nodes, "
...
def test_descriptions_are_unified():
    ...
    for key, p in platforms.items():
        ...
        assert "Provides persistent graph with hub nodes" not in body, f"[{key}] kept old wording"
        assert "treat the question as a /halal-graphify query." not in body, f"[{key}] kept old wording"
        assert "clustered communities" not in body, f"[{key}] kept old wording"


def test_windows_frontmatter_name_and_shell_and_extra():
    """windows: name must be `halal-graphify` (folder-name rule, #1635), powershell
    ...
    # (halal-graphify); a `graphify-windows` name broke skill discovery (#1635).
...
def test_every_platform_query_has_expansion_and_fallback():
    ...
    for key in ("claude", "codex", "windows", "opencode"):
        ...
        assert "## For /halal-graphify path" in q
        assert "## For /halal-graphify explain" in q
...
def test_monoliths_render_inline_single_file_no_references():
    ...
    for key in ("aider", "devin"):
        ...
        assert arts[0].path == f"halal_graphify/skill-{key}.md"
...
def test_generated_runbooks_pass_root_to_save_manifest():
    ...
    """
    targets = [
        REPO_ROOT / "halal_graphify" / "skill.md",
        REPO_ROOT / "halal_graphify" / "skill-aider.md",
        REPO_ROOT / "halal_graphify" / "skill-devin.md",
    ]
    targets += sorted((REPO_ROOT / "halal_graphify" / "skills").glob("*/references/update.md"))
    checked = 0
    for path in targets:
...
def test_always_on_renders_six_blocks():
    ...
    assert paths == [
        "halal_graphify/always_on/agents-md.md",
        "halal_graphify/always_on/antigravity-rules.md",
        "halal_graphify/always_on/claude-md.md",
        "halal_graphify/always_on/gemini-md.md",
        "halal_graphify/always_on/kiro-steering.md",
        "halal_graphify/always_on/vscode-instructions.md",
    ]
...
def test_always_on_included_in_full_render_not_per_platform():
    ...
    assert "halal_graphify/always_on/claude-md.md" in full
    assert "halal_graphify/always_on/claude-md.md" not in claude_only
...
def test_always_on_roundtrip_is_byte_faithful():
    ...
    halal_graphify.__main__, so the packaged markdown must round-trip exactly or those
    ...
    rendered_agents = next(
        ...
        if a.path == "halal_graphify/always_on/agents-md.md"
    )
    old_instruction = (
        "When the user types `/halal-graphify`, invoke the `skill` tool with "
        '`skill: "halal-graphify"` before doing anything else.'
    )
    new_instruction = (
        "When the user types `/halal-graphify`, use the installed halal-graphify skill or instructions "
    ...
    assert 'skill: "halal-graphify"' not in rendered_agents
...
def test_extracted_constants_equal_the_packaged_always_on_files():
    ...
    from halal_graphify import __main__ as mainmod
...
def test_always_on_files_are_guarded_by_check(tmp_path):
    ...
    mutated = [
        ...
        if a.path == "halal_graphify/always_on/claude-md.md"
        else a
        for a in arts
    ]
...
def test_audit_reads_each_host_against_its_own_v8_body():
    ...
    """
    assert gen._v8_baseline_ref("claude") == "47042beb05d1f6dd2186c0c499ae2840ce604ead:halal_graphify/skill.md"
    assert gen._v8_baseline_ref("trae") == "47042beb05d1f6dd2186c0c499ae2840ce604ead:halal_graphify/skill-trae.md"
    assert gen._v8_baseline_ref("vscode") == "47042beb05d1f6dd2186c0c499ae2840ce604ead:halal_graphify/skill-vscode.md"
...
def test_trae_renders_native_agents_md_integration_not_claude():
    """trae wires `halal-graphify trae install` -> AGENTS.md, never `halal-graphify claude install`."""
    ...
    assert "halal-graphify trae install" in hooks
    assert "halal-graphify trae-cn install" in hooks
    assert "writes a `## halal-graphify` section to the local `AGENTS.md`" in hooks
    # The claude-flavored install command must NOT appear for trae.
    assert "halal-graphify claude install" not in hooks
    ...
    assert "wire halal-graphify into a project's AGENTS.md" in core
...
def test_trae_hooks_reference_includes_the_pretooluse_note():
    ...
    assert "Run `/halal-graphify --update` manually after code changes" in hooks
...
def test_claude_flavored_hosts_keep_their_hooks_text_unchanged():
    ...
    for key in ("claude", "droid", "codex", "windows", "kilo", "vscode"):
        core, refs = _platform_artifacts(key)
        hooks = refs["hooks.md"]
        assert "halal-graphify claude install" in hooks, f"[{key}] lost the claude install command"
...
def test_amp_renders_native_agents_md_integration_v8_faithfully():
    """amp wires `halal-graphify amp install` -> AGENTS.md exactly as its v8 body had it.
    ...
    assert "make halal-graphify always-on in Amp sessions" in hooks
    assert "instructs Amp to check the graph" in hooks
    # amp's single-line install/uninstall, no trae-cn alt comments.
    assert "halal-graphify amp install" in hooks
    assert "halal-graphify amp uninstall  # remove the section" in hooks
    assert "halal-graphify trae install" not in hooks
    assert "halal-graphify trae-cn" not in hooks
    assert "or: halal-graphify" not in hooks
...
def test_amp_audit_coverage_passes_against_its_own_v8():
    ...
    against a lean split. The audit reads origin/v8:halal_graphify/skill-amp.md and
    confirms every heading single-homes in amp's core + references.
    """
    platforms = gen.load_platforms()
    assert gen._v8_baseline_ref("amp") == "47042beb05d1f6dd2186c0c499ae2840ce604ead:halal_graphify/skill-amp.md"
...
def test_agents_renders_its_own_agents_md_hooks_wording():
    ...
    `halal-graphify agents install` and is worded for an unspecified host.
    """
    core, refs = _platform_artifacts("agents")
    hooks = refs["hooks.md"]
    assert "## For native AGENTS.md integration" in hooks
    assert "## For native AGENTS.md integration (Trae)" not in hooks
    assert "make halal-graphify always-on in your agent sessions" in hooks
    assert "halal-graphify agents install" in hooks
    assert "halal-graphify agents uninstall  # remove the section" in hooks
    # No amp/trae/claude wording leaks into the agents render.
    assert "halal-graphify amp install" not in hooks
    assert "halal-graphify trae" not in hooks
    assert "halal-graphify claude install" not in hooks
...
def test_agents_audit_baseline_is_amps_v8_body():
    ...
    platforms = gen.load_platforms()
    assert gen._v8_baseline_ref("agents") == "47042beb05d1f6dd2186c0c499ae2840ce604ead:halal_graphify/skill-amp.md"
