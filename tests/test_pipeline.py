...
from halal_graphify.extract import collect_files, extract
...
from halal_graphify.export import to_json, to_html, to_obsidian
...
def run_pipeline(tmp_path: Path) -> dict:
    ...
    hubs = hub_nodes(G)
    assert len(hubs) > 0
    assert all("id" in g and "degree" in g for g in hubs)
    ...
    report = generate(G, communities, cohesion, labels, hubs, surprises, detection, tokens, str(FIXTURES), suggested_questions=questions)
...
def test_pipeline_report_mentions_top_hub_node(tmp_path):
    result = run_pipeline(tmp_path)
    top_hub = result["hubs"][0]["label"]
    assert top_hub in result["report"]
