...
from halal_graphify.detect import classify_file, count_words, detect, detect_incremental, save_manifest, FileType, _looks_like_paper, _is_ignored, _load_graphifyignore, _is_sensitive
...
def test_detect_skips_noise_dot_dirs():
    ...
    for files in result["files"].values():
        for f in files:
            # halal-graphify's own cache is always skipped
...
def test_classify_video_extensions():
    ...
    from halal_graphify.detect import FileType
...
def test_detect_converts_google_workspace_shortcuts_when_enabled(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("halal_graphify.detect.convert_google_workspace_file", fake_convert)
...
def test_collect_files_keeps_coverage_code_namespace(tmp_path):
    ...
    from halal_graphify.extract import collect_files
...
def test_git_info_exclude_ranks_below_gitignore_negation(tmp_path):
    ...
    from halal_graphify.detect import _load_graphifyignore, _is_ignored
    (tmp_path / ".git" / "info").mkdir(parents=True)
...
def test_negation_cannot_rescue_file_under_excluded_dir(tmp_path):
    ...
    from halal_graphify.detect import _is_ignored, _load_graphifyignore
...
def test_negation_does_not_disable_directory_pruning(tmp_path, monkeypatch):
    ...
    """
    import os
    import halal_graphify.detect as det
...
def test_is_ignored_cache_evaluates_each_dir_once():
    ...
    """
    from halal_graphify.detect import _is_ignored
...
def test_save_manifest_skips_semantic_hash_for_files_without_cache(tmp_path):
    ...
    import json
    from halal_graphify.cache import save_cached
...
def test_save_manifest_relativizes_keys_when_root_given(tmp_path):
    ...
    import json
    from halal_graphify.detect import save_manifest, load_manifest
...
def test_load_manifest_absolutizes_relative_keys(tmp_path):
    ...
    import json
    from halal_graphify.detect import load_manifest
...
def test_detect_incremental_portable_across_paths(tmp_path):
    ...
    import json
    from halal_graphify.detect import save_manifest, detect_incremental
...
def test_graphifyinclude_is_inert_and_not_unclassified(tmp_path, capsys):
    ...
    assert any("main.py" in f for f in result["files"]["code"])
    # one-time stderr note, matching the [halal-graphify] warning convention
    err = capsys.readouterr().err
    assert err.count("[halal-graphify] WARNING: .graphifyinclude is no longer supported") == 1
...
def test_save_manifest_ast_kind_noop_then_change(tmp_path):
    """#2838's literal path: `halal-graphify update` calls save_manifest with kind='ast'.
...
def test_sensitive_filter_indexes_topic_prose_and_source(path):
    from halal_graphify.detect import _is_sensitive
