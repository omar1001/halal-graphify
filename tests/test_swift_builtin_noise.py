"""Swift/Foundation/SwiftUI builtins must not become hub nodes or bind to user symbols.
...
`NSLock`, `View`, `Data`, `Sendable`, ...) ranked as hub nodes and the Swift
...
from halal_graphify.analyze import hub_nodes
...
def test_hub_nodes_excludes_swift_builtin_labels(builtin_label: str) -> None:
    """Swift framework symbols must be filtered from hub_nodes output.
    ...
    assert "builtin_node" not in result_ids, (
        f"hub_nodes() should filter Swift builtin '{builtin_label}' "
        f"but it appeared in the result: {result}"
    )
    assert "real_node" in result_ids, (
        f"hub_nodes() should include project symbol 'AudioStreamer' "
