...
the member-call resolver's single-definition hub-node guard, and produced NO edge —
...
def test_objc_same_named_categories_in_different_directories_stay_distinct(tmp_path: Path):
    ...
    them; a `[Thing act]` receiver stays ambiguous and yields no edge (hub-node guard).
    """
    base = tmp_path / "src"
    for d in ("a", "b"):
