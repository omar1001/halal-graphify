...
The real claude bundle now ships in the package (halal_graphify/skills/claude/), so
...
PKG_DIR = Path(halal_graphify.__file__).parent
...
def test_install_stages_references_sidecar(tmp_path, fake_bundle):
    ...
    skill_dir = tmp_path / ".claude" / "skills" / "halal-graphify"
...
def test_gemini_install_references_all_resolve(tmp_path):
    ...
    skill = tmp_path / ".gemini" / "skills" / "halal-graphify" / "SKILL.md"
    assert skill.exists()
...
def test_pyproject_declares_references_globs():
    ...
    The references sidecar ships under halal_graphify/skills/<host>/references/ and the
    always-on injection blocks under halal_graphify/always_on/. The earlier
    skills/*/SKILL.md glob matched nothing (no halal_graphify/skills/<host>/SKILL.md
    exists; the skill bodies are halal_graphify/skill*.md, listed explicitly), so it was
    ...
    pkg_data = data["tool"]["setuptools"]["package-data"]["halal_graphify"]
...
def test_built_wheel_ships_the_full_skill_payload():
    ...
    assert "halal_graphify/skills/claude/references/extraction-spec.md" in names
    assert "halal_graphify/skills/trae/references/hooks.md" in names
    # amp is now a split host too; its bundle must ship like every other.
    assert "halal_graphify/skills/amp/references/hooks.md" in names
...
def test_monolith_install_clears_orphan_references(tmp_path, fake_bundle):
    ...
    skill_dst = tmp_path / ".aider" / "halal-graphify" / "SKILL.md"
...
def test_amp_user_install_carries_references(tmp_path, monkeypatch):
    ...
    amp's real bundle now ships in the package (halal_graphify/skills/amp/), so the
    ...
    with patch("halal_graphify.__main__.Path.home", return_value=home):
        ...
        main()
        skill_dir = home / ".config" / "agents" / "skills" / "halal-graphify"
        assert (skill_dir / "SKILL.md").exists()
