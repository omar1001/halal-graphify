...
Every platform halal-graphify knows about installs its SKILL.md at a specific real
...
def test_amp_user_install_at_corrected_agents_path(tmp_path, monkeypatch):
    ...
    with patch("halal_graphify.__main__.Path.home", return_value=home):
        ...
        assert dst == home / ".config" / "agents" / "skills" / "halal-graphify" / "SKILL.md"
        assert dst.exists()
...
def test_amp_project_install_at_agents_path(tmp_path, monkeypatch):
    ...
    project_dir.mkdir()
    monkeypatch.chdir(project_dir)
    with patch("halal_graphify.__main__.Path.home", return_value=tmp_path / "home"):
        dst = mainmod._copy_skill_file("amp", project=True, project_dir=project_dir)
        assert dst == project_dir / ".agents" / "skills" / "halal-graphify" / "SKILL.md"
        assert dst.exists()
...
def test_vscode_install_uninstall_roundtrip(tmp_path, monkeypatch):
    """VS Code Copilot Chat round trip at ~/.copilot/skills/halal-graphify + instructions file."""
    home = tmp_path / "home"
    project_dir = tmp_path / "proj"
    home.mkdir()
    project_dir.mkdir()
    ...
    with patch("halal_graphify.__main__.Path.home", return_value=home):
        ...
        skill = home / ".copilot" / "skills" / "halal-graphify" / "SKILL.md"
        instructions = project_dir / ".github" / "copilot-instructions.md"
        assert skill.exists()
        assert instructions.exists()
        assert "## halal-graphify" in instructions.read_text(encoding="utf-8")
        ...
        # The halal-graphify section is stripped from the instructions file.
        if instructions.exists():
            assert "## halal-graphify" not in instructions.read_text(encoding="utf-8")
...
def test_install_entrypoint_roundtrip_for_progressive_and_monolith(tmp_path):
    ...
    """
    for platform, rel in (
        ("claude", Path(".claude") / "skills" / "halal-graphify"),
        ("aider", Path(".aider") / "halal-graphify"),
    ):
...
def test_failed_copytree_leaves_no_partial_references(tmp_path, fake_progressive_bundle):
    ...
    with patch("halal_graphify.__main__.shutil.copytree", side_effect=boom):
