...
These strings live in halal_graphify/__main__.py and are written into project-local
files (CLAUDE.md, AGENTS.md, GEMINI.md, .cursor/rules/, .kiro/steering/, etc.)
or into in-process hook payloads. Earlier versions of halal-graphify told every
...
from halal_graphify.__main__ import (
    _SEARCH_NUDGE,
    _READ_NUDGE,
    _skill_registration,
    _CLAUDE_MD_SECTION,
...
def test_every_install_surface_recommends_graphify_query():
    """All ten install surfaces must point the assistant at `halal-graphify query`
    ...
    for name, text in _INSTALL_TEXTS.items():
        if "halal-graphify query" not in text:
            missing.append(name)
    assert not missing, (
        f"these install surfaces no longer mention `halal-graphify query`: {missing}. "
...
def test_agents_section_does_not_skip_dirty_graph_output():
    ...
    assert "not a reason to skip halal-graphify" in _AGENTS_MD_SECTION


def test_agents_section_uses_generic_graphify_instruction():
    assert "`skill` tool" not in _AGENTS_MD_SECTION
    assert 'skill: "halal-graphify"' not in _AGENTS_MD_SECTION
    assert "use the installed halal-graphify skill" in _AGENTS_MD_SECTION


def test_skill_registration_uses_host_generic_instruction():
    reg = _skill_registration()
    assert 'skill: "halal-graphify"' not in reg
    assert "Skill tool" not in reg
    assert "use the installed halal-graphify skill or instructions" in reg
