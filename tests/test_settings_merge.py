...
from halal_graphify.install import (
...
def test_claude_install_preserves_existing_settings(tmp_path):
    """#2167 core case: every key halal-graphify does not own must survive install."""
    seeded = {
    ...
    # top-level keys halal-graphify does not own are untouched
    assert result["mcpServers"] == seeded["mcpServers"]
    assert result["enabledPlugins"] == seeded["enabledPlugins"]
    assert result["theme"] == "dark"
    # hooks sections halal-graphify does not manage are untouched
    assert result["hooks"]["PostToolUse"] == seeded["hooks"]["PostToolUse"]
    # the user's own PreToolUse entry survives alongside halal-graphify's
    pre_tool = result["hooks"]["PreToolUse"]
    assert seeded["hooks"]["PreToolUse"][0] in pre_tool
    graphify_hooks = [h for h in pre_tool if "halal-graphify" in str(h)]
...
def test_bom_settings_are_merged_not_clobbered(tmp_path, installer):
    ...
    assert any("halal-graphify" in str(h) for h in result["hooks"][section])
...
def test_non_dict_hook_entry_is_preserved_not_fatal(tmp_path, installer):
    ...
    assert any("halal-graphify" in str(h) for h in entries)
