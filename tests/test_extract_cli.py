"""Tests for `halal-graphify extract` CLI dispatch path in halal_graphify.__main__."""
...
import halal_graphify.__main__ as mainmod
...
def test_extract_exits_nonzero_when_ast_extraction_raises(
    ...
    import halal_graphify.extract as extractmod
    ...
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["halal-graphify", "extract", str(corpus), "--code-only",
         "--out", str(out_dir)],
    )
    ...
    assert (
        "[halal-graphify extract] AST extraction failed: worker pool failed"
...
def test_extract_allow_partial_continues_past_ast_failure(
    ...
    monkeypatch.setattr(
        "halal_graphify.llm.extract_corpus_parallel", _one_chunk_succeeded
    )
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(
        mainmod.sys,
        "argv",
        ["halal-graphify", "extract", str(corpus), "--backend", "claude",
...
def test_extract_exits_nonzero_when_all_semantic_chunks_fail(
    ...
    The bug this guards: `pip install halal-graphify` doesn't pull in `anthropic`,
    so `halal-graphify extract --backend claude` would print per-chunk errors and
    ...
    monkeypatch.setattr(
        "halal_graphify.llm.extract_corpus_parallel", _all_chunks_failed
    )
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(
        mainmod.sys,
...
def test_extract_succeeds_when_at_least_one_chunk_completes(
    ...
    monkeypatch.setattr(
        "halal_graphify.cache.save_semantic_cache", _capture_semantic_cache
    )
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(
        mainmod.sys,
...
def test_incremental_partial_run_preserves_untouched_semantic_hash(
    ...
    monkeypatch.setattr(
        "halal_graphify.llm.extract_corpus_parallel", _stamp_everything_sent
    )
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
...
def test_truncated_doc_semantic_hash_is_cleared_for_requeue(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("halal_graphify.llm.extract_corpus_parallel", _extract)
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
...
def test_manifest_stamps_freshly_extracted_semantic_docs(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("halal_graphify.llm.extract_corpus_parallel", _fresh_relative)
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["halal-graphify", "extract", str(corpus), "--backend", "claude",
...
def test_stamped_manifest_files_normalizes_both_sides(tmp_path):
    ...
    from halal_graphify.cli import _stamped_manifest_files
...
def test_manifest_stamps_hyperedge_only_docs(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("halal_graphify.llm.extract_corpus_parallel", _hyperedge_only)
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["halal-graphify", "extract", str(corpus), "--backend", "claude",
...
def test_extract_mode_deep_dispatches_over_warm_cache(monkeypatch, tmp_path):
    ...
    calls: list[dict] = []
    monkeypatch.setattr("halal_graphify.llm.extract_corpus_parallel",
    ...
    base = ["halal-graphify", "extract", str(corpus), "--backend", "claude",
            "--no-cluster"]
...
def test_cache_check_mode_deep_reads_deep_namespace(monkeypatch, tmp_path, capsys):
    ...
    from halal_graphify.cache import save_semantic_cache
    ...
    _run_extract(monkeypatch, ["halal-graphify", "cache-check", str(files_from),
                               "--root", str(tmp_path)])
...
def test_extract_codeonly_succeeds_without_api_key(monkeypatch, tmp_path):
    ...
    Regression: halal-graphify extract validated a backend upfront and exited 1 with
    ...
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["halal-graphify", "extract", str(corpus), "--out", str(out_dir)],
    )
...
def test_missing_manifest_code_only_preserves_semantic_layer(monkeypatch, tmp_path):
    """#1925: `halal-graphify extract --code-only` with a MISSING manifest.json must
    ...
    _run_extract(monkeypatch, ["halal-graphify", "extract", str(corpus),
...
def test_extract_out_keeps_project_root_clean(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["halal-graphify", "extract", ".", "--out", str(external)],
    )
...
def test_extract_without_key_still_errors_when_docs_present(
    ...
    monkeypatch.setattr("halal_graphify.llm.detect_backend", lambda: None)
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["halal-graphify", "extract", str(corpus), "--out", str(out_dir)],
...
def test_extract_timing_flag_emits_stage_timings(monkeypatch, tmp_path, capsys):
    """--timing prints per-stage `[halal-graphify timing]` lines to stderr (#1490); omitting
    ...
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["halal-graphify", "extract", str(code), "--no-cluster", "--out", str(tmp_path / "o1"), "--timing"],
    )
    with pytest.raises(SystemExit) as exc:
        mainmod.main()
    assert exc.value.code == 0
    err = capsys.readouterr().err
    assert "[halal-graphify timing] detect:" in err
    assert "[halal-graphify timing] total:" in err

    # without --timing => no timing lines
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["halal-graphify", "extract", str(code), "--no-cluster", "--out", str(tmp_path / "o2")],
    )
    with pytest.raises(SystemExit) as exc2:
        mainmod.main()
    assert exc2.value.code == 0
    assert "halal-graphify timing" not in capsys.readouterr().err
...
def test_pathless_postgres_extract_initializes_empty_detection(
    ...
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    monkeypatch.setattr("halal_graphify.pg_introspect.introspect_postgres", _introspect)
...
def test_incremental_extract_prunes_newly_excluded_file_not_in_manifest(
    ...
    _run_extract(
        monkeypatch,
        ["halal-graphify", "extract", str(project), "--out", str(out_dir)],
    )
...
def test_no_cluster_incremental_prunes_newly_excluded_file(
    ...
    monkeypatch.setattr(
        mainmod.sys, "argv",
        ["halal-graphify", "extract", str(project), "--no-cluster", "--out", str(out_dir)],
    )
    with pytest.raises(SystemExit) as exc:
        mainmod.main()
    assert exc.value.code == 0
...
def test_failed_extra_is_retried_and_recovers(monkeypatch, tmp_path, capsys):
    ...
    graph_path = out_dir / "graphify-out" / "graph.json"
    manifest_path = out_dir / "graphify-out" / "manifest.json"
    argv = ["halal-graphify", "extract", str(project), "--out", str(out_dir)]
...
def test_cache_check_prompt_file_scopes_hits_to_that_prompt(monkeypatch, tmp_path, capsys):
    ...
    base = ["halal-graphify", "cache-check", str(files_from), "--root", str(tmp_path)]
...
def test_edge_only_semantic_extraction_not_stamped_and_retried(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("halal_graphify.llm.extract_corpus_parallel", _mock_llm)
    argv = ["halal-graphify", "extract", str(corpus), "--backend", "claude",
            "--no-cluster", "--out", str(out_dir)]
    ...
    from halal_graphify.cache import file_hash
...
def test_stale_poisoned_manifest_semantic_source_healed(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr("halal_graphify.llm.extract_corpus_parallel", _mock_llm)
    argv = ["halal-graphify", "extract", str(project), "--backend", "claude",
            "--no-cluster", "--out", str(out_dir)]
...
def test_incremental_stray_attribution_preserves_undispatched_file(
    ...
    monkeypatch.setattr(
        "halal_graphify.llm.extract_corpus_parallel", _seed_extraction
    )
    monkeypatch.setattr(mainmod, "_check_skill_version", lambda _: None)
    argv = ["halal-graphify", "extract", str(project), "--out", str(out_dir)]
    _run_extract(monkeypatch, argv)
    ...
    monkeypatch.setattr(
        "halal_graphify.llm.extract_corpus_parallel", _straying_extraction
    )
    _run_extract(monkeypatch, argv)
