...
Registered into halal_graphify.resolver_registry and run by extract() after id
...
def resolve_ruby_member_calls(
    ...
    (hub-node guard) so an ambiguous class name resolves to nothing rather than a
    wrong edge.
    """
    ...
    # hub-node guard. Ambiguous at any step -> bail, no wrong edge.
    ...
    for rc in _ruby_raw_calls(per_file):
        ...
        # receiver's class under the single-owning-class hub-node guard.
