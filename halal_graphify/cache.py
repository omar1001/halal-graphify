...
# absolute path ("/shared/graphify-out"). Single source of truth in halal_graphify.paths
...
try:
    ...
    _EXTRACTOR_VERSION = _pkg_version("halal-graphify")
except Exception:
...
def _cleanup_stale_ast_entries(ast_base: Path, current_dir: Path) -> None:
    """Remove AST cache entries left behind by other halal-graphify versions.
...
def prompt_fingerprint(prompt: "str | Path") -> str:
    ...
    its system prompt, :func:`halal_graphify.llm._extraction_system`) or a Path to the
...
# `halal-graphify extract --force` / `halal-graphify update --force` (or GRAPHIFY_FORCE=1)
...
def _stat_sig_fresh(entry: object, st: "os.stat_result") -> bool:
    ...
    Entries written by an older halal-graphify carry no ``indexed_at_ns``; they are
...
def _stat_key_to_relative(key: str, anchor: Path) -> str:
    ...
    Local duplicate of :func:`halal_graphify.detect._to_relative_for_storage` —
...
def _stat_key_to_absolute(key: str, anchor: Path) -> str:
    ...
    written by an older halal-graphify remains readable.
    """
    p = Path(key)
    if p.is_absolute():
        return str(p)
...
def _relativize_source_files_in(payload: dict, root: Path) -> None:
    ...
    Mirror of :func:`halal_graphify.watch._relativize_source_files` so cached
    ...
    :func:`halal_graphify.detect._to_relative_for_storage`.
    """
    try:
...
def _normalize_source_file_value(src: "str | Path", root_resolved: Path) -> str:
    ...
    ``halal-graphify update`` runs with a machine-specific ``source_file`` (#2197).
...
def _id_anchor(path_str: str, rel_str: str) -> str:
    ...
    """
    from halal_graphify.ids import normalize_id  # ids imports only re/unicodedata: no cycle
...
def _portability_anchors(path: "str | Path", root: "str | Path") -> tuple[list[str], str, list[str], str]:
    ...
    """
    from halal_graphify.ids import normalize_id
...
def cache_dir(root: Path = Path("."), kind: str = "ast",
    ...
    by halal-graphify version and cache-key schema because they depend on extractor
...
def load_cached(path: Path, root: Path = Path("."), kind: str = "ast",
    ...
    AST entries written by other halal-graphify versions — including the legacy
