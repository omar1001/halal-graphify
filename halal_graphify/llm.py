...
def _resolve_max_retry_depth(default: int = 3) -> int:
    ...
    with no way for a `halal-graphify extract` operator to lower it — or set it to 0 —
...
def _sanitize_fragment(parsed: dict) -> dict:
    ...
    if hyperedges:
        from halal_graphify.build import _coerce_hyperedge_member_refs
...
def _extract_with_adaptive_retry(
    ...
    *,
    ...
    try:
        ...
        for _delay in (_HOLLOW_BACKOFF_S if max_depth > 0 else ()):
            if result.get("finish_reason") != "hollow":
                break
            print(
                f"[halal-graphify] retrying the same chunk of {len(chunk)} in {_delay:g}s "
        ...
        if len(chunk) <= 1:
            ...
            if halves is not None:
                print(
                    f"[halal-graphify] slice of {unit_path(chunk[0])} {reason} at "
            ...
            print(
                f"[halal-graphify] single-file chunk {unit_path(chunk[0])} {fail_desc} "
        ...
        if _depth >= max_depth:
            ...
            print(
                f"[halal-graphify] chunk of {len(chunk)} {persist_desc} at "
                f"recursion depth {_depth} (max {max_depth}) — dropping",
                file=sys.stderr,
            )
            return {"nodes": [], "edges": [], "hyperedges": [], "input_tokens": 0, "output_tokens": 0, "model": model, "finish_reason": "stop"}
        print(
            f"[halal-graphify] chunk of {len(chunk)} {reason} at depth "
    ...
    if result.get("finish_reason") == "hollow":
        ...
        print(
            f"[halal-graphify] chunk of {len(chunk)} still hollow after "
