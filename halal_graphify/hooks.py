...
def install(path: Path = Path(".")) -> str:
    ...
    # Pin the current interpreter so the hook works even when the halal-graphify
...
def status(path: Path = Path(".")) -> str:
    ...
    def _check(name: str, marker: str) -> str:
        ...
        return "installed" if marker in p.read_text(encoding="utf-8") else "not installed (hook exists but halal-graphify not found)"
