# git hook integration - install/uninstall halal-graphify post-commit and post-checkout hooks
...
# Python interpreter that ran `halal-graphify hook install`.  For uv-tool and pipx
...
# when the halal-graphify launcher is not on PATH (common in GUI clients and CI).
#
# Probes check availability with importlib.util.find_spec instead of importing
# the package: a probe that imports halal-graphify wholesale executes the full package
...
_GFY_PROBE="import importlib.util, sys; sys.exit(0 if importlib.util.find_spec('halal-graphify') else 1)"
GRAPHIFY_PYTHON=""
...
# Third probe: resolve via the halal-graphify launcher on PATH.
if [ -z "$GRAPHIFY_PYTHON" ]; then
    GRAPHIFY_BIN=$(command -v halal-graphify 2>/dev/null)
    if [ -n "$GRAPHIFY_BIN" ]; then
        # Windows pip layout: Scripts/halal-graphify(.exe) sits beside ..\\python.exe
...
# recommended method) puts halal-graphify in an isolated venv that no ambient
...
# co-installed tool without halal-graphify never satisfies it.
if [ -z "$GRAPHIFY_PYTHON" ]; then
    for _GFY_TOOLS in \
        "${UV_TOOL_DIR:-}" \
...
if [ -z "$GRAPHIFY_PYTHON" ]; then
    if command -v python3 >/dev/null 2>&1 && python3 -c "$_GFY_PROBE" 2>/dev/null; then
        ...
        echo "[halal-graphify hook] could not locate a Python with halal-graphify installed. Add the halal-graphify bin dir to PATH or re-run 'halal-graphify hook install' from the env where halal-graphify lives." >&2
        exit 0
    fi
fi
"""
...
print(f'[halal-graphify hook] {len(changed)} file(s) changed - rebuilding graph...')

try:
    from halal_graphify.watch import _rebuild_code, _apply_resource_limits
    _apply_resource_limits()
    _timeout = int(os.environ.get('GRAPHIFY_REBUILD_TIMEOUT', '600'))
    if _timeout > 0:
        if hasattr(signal, 'SIGALRM'):
            signal.signal(signal.SIGALRM, lambda *_: (_ for _ in ()).throw(TimeoutError(f'halal-graphify rebuild exceeded {_timeout}s')))
            signal.alarm(_timeout)
        else:
            def _bail():
                print(f'[halal-graphify hook] halal-graphify rebuild exceeded {_timeout}s', flush=True)
                os._exit(1)
    ...
    try:
        ...
        if _md.is_dir() and any(_md.glob('*.md')):
            from halal_graphify.reflect import reflect as _reflect
    ...
    print(f'[halal-graphify hook] {exc}')
    sys.exit(1)
except Exception as exc:
    print(f'[halal-graphify hook] Rebuild failed: {exc}')
    sys.exit(1)
"""
...
try:
    ...
    if _timeout > 0:
        if hasattr(signal, 'SIGALRM'):
            ...
            def _bail():
                print(f'[halal-graphify] halal-graphify rebuild exceeded {_timeout}s', flush=True)
                os._exit(1)
            _watchdog = threading.Timer(_timeout, _bail)
            _watchdog.daemon = True
            _watchdog.start()
    ...
    print(f'[halal-graphify] {exc}')
    sys.exit(1)
except Exception as exc:
    print(f'[halal-graphify] Rebuild failed: {exc}')
    sys.exit(1)
"""
...
# still returned 0, so the graph just went stale with no signal. halal-graphify already
...
# Installed by: halal-graphify hook install
...
echo "[halal-graphify hook] launching background rebuild (log: $_GRAPHIFY_LOG)"
...
mkdir -p "$(dirname "$_GRAPHIFY_LOG")"
export GRAPHIFY_REBUILD_LOG="$_GRAPHIFY_LOG"
echo "[halal-graphify] Branch switched - launching background rebuild (log: $_GRAPHIFY_LOG)"
...
def _hooks_dir(root: Path) -> Path:
    ...
    try:
        ...
        if res.returncode != 0:
            ...
            print(
                f"[halal-graphify hooks] git could not resolve the hooks path for "
...
def _install_hook(
    ...
    an existing halal-graphify block in-place."""
...
def _uninstall_hook(hooks_dir: Path, name: str, marker: str, marker_end: str) -> str:
    """Remove halal-graphify section from a git hook using start/end markers."""
    hook_path = hooks_dir / name
    if not hook_path.exists():
        return f"no {name} hook found - nothing to remove."
    content = hook_path.read_text(encoding="utf-8")
    if marker not in content:
        return f"halal-graphify hook not found in {name} - nothing to remove."
    new_content = re.sub(
    ...
    return f"halal-graphify removed from {name} at {hook_path} (other hook content preserved)"
...
def _pinned_python() -> str:
    ...
    means callers must fall back to the `halal-graphify` launcher on PATH — safe
    degradation.
    """
...
def _merge_attr_line() -> str:
    """The .gitattributes line assigning the halal-graphify merge driver to graph.json.

    The graph lives under the configured output directory (halal_graphify.paths,
    ...
    """
    from halal_graphify.paths import GRAPHIFY_OUT
    out = GRAPHIFY_OUT
    if not out or Path(out).is_absolute() or "\\" in out:
        out = "graphify-out"
    return f"{out.rstrip('/')}/graph.json merge=halal-graphify"


def _has_merge_attr(content: str) -> bool:
    """True if a (non-comment) `<...>graph.json ... merge=halal-graphify` line exists."""
    for raw in content.splitlines():
        line = raw.strip()
        if not line or line.startswith("#"):
            continue
        fields = line.split()
        if fields and fields[0].endswith("graph.json") and "merge=halal-graphify" in fields[1:]:
            return True
    return False
...
def _register_merge_driver(root: Path) -> str:
    ...
    README and CHANGELOG 0.7.0 document `halal-graphify merge-driver` as being set up
    ...
    same way the hook scripts pin it, so the driver works even when the halal-graphify
    ...
    if pinned:
        ...
        driver = f'"{pinned}" -m halal_graphify merge-driver %O %A %B'
    else:
        driver = "halal-graphify merge-driver %O %A %B"
    try:
        for key, value in (
            ("merge.graphify.name", "halal-graphify graph.json union merge"),
...
def install(path: Path = Path(".")) -> str:
    """Install halal-graphify post-commit and post-checkout hooks in the nearest git repo."""
    root = _git_root(path)
    if root is None:
...
def uninstall(path: Path = Path(".")) -> str:
    """Remove halal-graphify post-commit and post-checkout hooks."""
    root = _git_root(path)
    if root is None:
        raise RuntimeError(f"No git repository found at or above {path.resolve()}")
...
def status(path: Path = Path(".")) -> str:
    """Check if halal-graphify hooks are installed."""
    root = _git_root(path)
    if root is None:
    ...
    def _check(name: str, marker: str) -> str:
        ...
        if marker not in text:
            return "not installed (hook exists but halal-graphify not found)"
