...
# Hyperedge member lists are canonically keyed `nodes` (see halal_graphify/llm.py
    ...
    for ref in members:
        if isinstance(ref, dict):
            ...
            if inner in (None, "") or not _hashable(inner):
                print(
                    f"[halal-graphify] WARNING: hyperedge "
...
def _fold_node_aliases(node: dict) -> None:
    ...
    label-keyed merge/dedup — a permanent ghost that ``halal-graphify update``
...
def _infer_merge_root(graph_path: Path) -> str | None:
    ...
    scan root halal-graphify records at build/watch time (#686/#1423) — then falls back to
    ...
    applied only when ``graph.json``'s own directory actually looks like a halal-graphify
...
def _semantic_id_remap(nodes: list, root: str | None) -> dict:
    ...
    from halal_graphify.extractors.base import _file_stem  # local: avoid import cost at module load
...
def graph_has_legacy_ids(nodes: list, root: str | Path | None = None, sample: int = 300) -> bool:
    ...
    from halal_graphify.extractors.base import _file_stem
...
def _doc_twin_remap(nodes: list) -> dict[str, str]:
    ...
    the same document. A ``halal-graphify update`` after a semantic build leaves both,
...
def build_from_json(extraction: dict, *, directed: bool = False, root: str | Path | None = None) -> nx.Graph:
    ...
    for node in extraction.get("nodes", []):
        ...
        if "source" in node and "source_file" not in node:
            ...
            print(
                f"[halal-graphify] WARNING: node '{node_id}' uses field 'source' instead of "
        ...
        # halal-graphify versions that didn't always populate file_type) don't
    ...
    if real_errors:
        ...
        print(
            f"[halal-graphify] Extraction warning ({len(real_errors)} issues): {breakdown}",
            file=sys.stderr,
        )
    ...
    for node in extraction.get("nodes", []):
        ...
        if isinstance(node, dict):
            ...
            try:
                ...
                print(
                    f"[halal-graphify] WARNING: skipping node with non-hashable id "
    ...
    from halal_graphify.extractors.base import _file_stem as _fs
    ...
    for edge in sorted(
        ...
        try:
            ...
            print(
                f"[halal-graphify] WARNING: skipping edge with non-hashable endpoint "
        ...
        # `calls` and `uses` after everything. On halal-graphify's own corpus that
    ...
    if hyperedges:
        ...
        for he in hyperedges:
            ...
            if isinstance(he, dict) and isinstance(he.get("nodes"), list):
                ...
                if not valid_members:
                    print(
                        f"[halal-graphify] WARNING: dropping hyperedge "
        ...
        if kept_hyperedges:
            ...
            G.graph["hyperedges"] = []
            print(
                f"[halal-graphify] WARNING: all {len(hyperedges)} hyperedge(s) were "
...
def deduplicate_by_label(nodes: list[dict], edges: list[dict]) -> tuple[list[dict], list[dict]]:
    ...
    print(f"[halal-graphify] Deduplicated {len(remap)} duplicate node(s) by label.", file=sys.stderr)
...
def build_merge(
    ...
    *,
    ...
    if new_sources:
        ...
        if replaced_n:
            print(
                f"[halal-graphify] Replaced {replaced_n} node(s) from re-extracted "
                f"source file(s).",
                file=sys.stderr,
            )
    ...
    if existing_hyperedges:
        ...
        for he in existing_hyperedges:
            ...
            carried.append(he)
        if carried:
            from halal_graphify.export import attach_hyperedges
    ...
    if prune_sources:
        ...
        if n_nodes:
            print(
                f"[halal-graphify] Pruned {n_nodes} node(s) from {n_files} deleted or "
                f"excluded source file(s).",
                file=sys.stderr,
            )
        if edges_to_remove:
            print(
                f"[halal-graphify] Pruned {len(edges_to_remove)} edge(s) from deleted or "
                f"excluded source file(s).",
                file=sys.stderr,
            )
        ...
        if not n_nodes and not edges_to_remove:
            if (prune_set or prune_abs) and not _matched_prune_entries:
                ...
                print(
                    f"[halal-graphify] WARNING: {len(prune_sources)} prune source(s) "
                ...
                print(
                    f"[halal-graphify] {len(prune_sources)} source file(s) deleted or "
    ...
    if had_graph and not dedup:
        ...
        if unexplained:
            raise ValueError(
                f"halal-graphify: build_merge would drop {len(unexplained)} node(s) "
