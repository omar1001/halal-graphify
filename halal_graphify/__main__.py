"""halal-graphify CLI - `halal-graphify install` sets up the Claude Code skill."""
...
try:
    ...
    __version__ = _pkg_version("halal-graphify")
except Exception:
    __version__ = "unknown"
...
# Defined once in halal_graphify.paths so the security/callflow path guards honour the
# same override (#1423).
from halal_graphify.paths import GRAPHIFY_OUT as _GRAPHIFY_OUT

# Install/uninstall subsystem moved to halal_graphify/install.py; re-exported here so
# `from halal_graphify.__main__ import <name>` keeps working unchanged.
from halal_graphify.install import (  # noqa: E402,F401
...
from halal_graphify.cli import (  # noqa: E402,F401
    dispatch_command,
    _StageTimer,
    _clone_repo,
    _default_graph_path,
...
def __getattr__(name: str) -> str:
    ...
    # brick the CLI on `import halal_graphify.__main__` (#1121 follow-up).
...
def _check_skill_version(skill_dst: Path) -> None:
    """Warn if the installed skill is from an older halal-graphify version."""
    ...
    try:
        ...
        return
    if not skill_exists:
        print("  warning: skill dir exists but SKILL.md is missing. Run 'halal-graphify install' to repair.", file=sys.stderr)
        return
    ...
    if "references/" in body and not (skill_dst.parent / "references").exists():
        print("  warning: skill references/ sidecar is missing. Run 'halal-graphify install' to repair.", file=sys.stderr)
    try:
    ...
    if installed != __version__:
        if _version_tuple(installed) > _version_tuple(__version__):
            # The skill on disk is NEWER than the running package. `halal-graphify install`
            ...
            print(
                f"  warning: skill is from halal_graphify {installed}, but the package is "
                f"{__version__} (older). Upgrade the package "
                f"(e.g. 'uv tool upgrade halal-graphify' or 'pip install -U halal-graphify'); "
                f"running 'halal-graphify install' would downgrade the skill.",
                file=sys.stderr,
            )
        else:
            print(f"  warning: skill is from halal_graphify {installed}, package is {__version__}. Run 'halal-graphify install' to update.", file=sys.stderr)
...
# `halal-graphify hook-guard` subcommand (see _run_hook_guard). The previous hooks
...
# silently vanished — users had to invoke /halal-graphify by hand (#522). Moving the
...
# The always-on instruction blocks are packaged markdown under halal_graphify/always_on/,
...
# when a graph exists. Emitted by `halal-graphify hook-guard gemini`. The old hook was a
...
_KIRO_STEERING_MARKER = "halal-graphify: A knowledge graph of this project"
        ...
        "PreToolUse": [
            {
                ...
                "hooks": [
                    {
                        "type": "command",
                        # Use the halal-graphify CLI itself so the hook is shell-agnostic:
                        ...
                        "command": "halal-graphify hook-check",
                    }
                ],
            }
        ]
...
def _run_cli() -> None:
    ...
    if len(sys.argv) >= 2 and sys.argv[1] in ("-v", "--version", "version"):
        print(f"halal-graphify {__version__}")
        return

    if len(sys.argv) < 2 or sys.argv[1] in ("-h", "--help", "-?"):
        print("Usage: halal-graphify <command>")
        print()
        print("Commands:")
        ...
        print("  uninstall               remove halal-graphify from all detected platforms in one shot")
        ...
        print("  clone <github-url>      clone a GitHub repo locally and print its path for /halal-graphify")
        ...
        print("    --graph <path>          path to graph.json (default graphify-out/graph.json)")
        print("  hub-nodes               list the most connected nodes (architectural hubs)")
        ...
        print("  cursor install          write .cursor/rules/halal-graphify.mdc (Cursor)")
        print("  cursor uninstall        remove .cursor/rules/halal-graphify.mdc")
        print("  claude install          write halal-graphify section to CLAUDE.md + PreToolUse hook (Claude Code)")
        print("  claude uninstall        remove halal-graphify section from CLAUDE.md + PreToolUse hook")
        print("  codebuddy install       write halal-graphify section to CODEBUDDY.md + PreToolUse hook (CodeBuddy)")
        print("  codebuddy uninstall     remove halal-graphify section from CODEBUDDY.md + PreToolUse hook")
        print("  codex install           write halal-graphify section to AGENTS.md (Codex)")
        print("  codex uninstall         remove halal-graphify section from AGENTS.md")
        print(
            "  opencode install        write halal-graphify section to AGENTS.md + tool.execute.before plugin (OpenCode)"
        )
        print(
            "  opencode uninstall      remove halal-graphify section from AGENTS.md + plugin"
        )
        print(
        ...
        print("  aider install           write halal-graphify section to AGENTS.md (Aider)")
        print("  aider uninstall         remove halal-graphify section from AGENTS.md")
        print(
            "  copilot install         copy halal-graphify skill to ~/.copilot/skills (GitHub Copilot CLI)"
        )
        print("  copilot uninstall       remove halal-graphify skill from ~/.copilot/skills")
        print(
        ...
        print(
            "  claw install            write halal-graphify section to AGENTS.md (OpenClaw)"
        )
        print("  claw uninstall          remove halal-graphify section from AGENTS.md")
        print(
            "  droid install           write halal-graphify section to AGENTS.md (Factory Droid)"
        )
        print("  droid uninstall        remove halal-graphify section from AGENTS.md")
        print("  trae install            write halal-graphify section to AGENTS.md (Trae)")
        print("  trae uninstall         remove halal-graphify section from AGENTS.md")
        print("  trae-cn install         write halal-graphify section to AGENTS.md (Trae CN)")
        print("  trae-cn uninstall      remove halal-graphify section from AGENTS.md")
        print(
        ...
        print(
            "  hermes install          write skill to ~/.hermes/skills/halal-halal_graphify/ (Hermes)"
        )
        print("  hermes uninstall        remove skill from ~/.hermes/skills/halal-halal_graphify/")
        print(
            "  kiro install            write skill to .kiro/skills/halal-halal_graphify/ + steering file (Kiro IDE/CLI)"
        )
        print("  kiro uninstall          remove skill + steering file")
        print("  pi install              write skill to ~/.pi/agent/skills/halal-halal_graphify/ (Pi coding agent)")
        print("  pi uninstall            remove skill from ~/.pi/agent/skills/halal-halal_graphify/")
        print("  devin install           write skill to ~/.config/devin/skills/halal-halal_graphify/ (Devin CLI)")
        print("  devin uninstall         remove skill from ~/.config/devin/skills/halal-halal_graphify/")
        print()
        return
    ...
    if cmd not in _FREE_TEXT_CMDS and any(a in {"-h", "--help", "-?"} for a in sys.argv[2:]):
        print(f"Run 'halal-graphify --help' for full usage.")
        return
