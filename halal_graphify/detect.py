...
from halal_graphify.google_workspace import (
    GOOGLE_WORKSPACE_EXTENSIONS,
    convert_google_workspace_file,
    google_workspace_enabled,
)
from halal_graphify.paths import GRAPHIFY_OUT, out_path
...
# attacker-controllable (halal-graphify runs on cloned/shared folders), and .docx/.xlsx
...
def classify_file(path: Path) -> FileType | None:
    ...
    from halal_graphify.manifest_ingest import is_package_manifest_path
...
def xlsx_extract_structure(path: Path) -> dict:
    ...
    Returns a nodes/edges dict compatible with the halal-graphify extract pipeline.
...
def count_words(path: Path) -> int:
    try:
        ...
        # cannot help, and `halal-graphify update` hangs with no output. os.stat
...
_SKIP_DIRS = {
    ...
    ".graphify",  # halal-graphify's own extraction cache — never index self-generated data
...
def _git_info_exclude(vcs_root: Path) -> Path | None:
    ...
    a directory without any ``.gitignore`` entry. halal-graphify only read
...
def _read_ignore_text(path: Path) -> str:
    ...
    for enc in (fallback, "latin-1"):
        ...
        if key not in _warned_ignore_encodings:
            ...
            print(
                f"[halal-graphify] WARNING: {path} is not valid UTF-8; read it as "
...
def detect(root: Path, *, follow_symlinks: bool | None = None, google_workspace: bool | None = None, extra_excludes: list[str] | None = None, cache_root: Path | None = None, gitignore: bool = True) -> dict:
    ...
    if (root / ".graphifyinclude").is_file():
        import sys as _sys
        print(
            "[halal-graphify] WARNING: .graphifyinclude is no longer supported "
    ...
    def _on_walk_error(err: OSError) -> None:
        ...
        print(
            f"[halal-graphify] WARNING: could not scan {target} ({err}); "
        ...
        if ftype:
            ...
            if p.suffix.lower() in OFFICE_EXTENSIONS:
                ...
                if md_path:
                    ...
                    skipped_sensitive.append(str(p) + " [office conversion failed - pip install halal-graphify[office]]")
                continue
...
def _to_relative_for_storage(key: str, root: Path) -> str:
    ...
    fallback in :func:`halal_graphify.watch._relativize_source_files` so the
...
def save_manifest(
    ...
    *,
    ...
    kind="ast"      — written by `halal-graphify update` (AST-only rebuild). Stamps
                      ast_hash; preserves an existing semantic_hash only when
                      the file content is unchanged (mtime + hash match).
    kind="semantic" — written by `halal-graphify extract` after semantic extraction.
...
def detect_incremental(
    ...
    *,
    ...
    kind="semantic" (default for extract): a file is "changed" when its
        ...
        semantic extraction pass. Use this for `halal-graphify extract` so that
        files touched by `halal-graphify update` (AST-only) are re-extracted
        semantically.
    kind="ast": a file is "changed" when its ast_hash is missing or its
        content has changed. Use this for `halal-graphify update`.
