"""halal-graphify prs — graph-aware PR dashboard.
...
Usage:
  halal-graphify prs                   # dashboard of all open PRs
  halal-graphify prs <number>          # deep dive on one PR
  halal-graphify prs --triage          # Opus ranks your review queue
  halal-graphify prs --worktrees       # show worktree → branch → PR mapping
  halal-graphify prs --conflicts       # PRs sharing graph communities (merge-order risk)
  halal-graphify prs --base <branch>   # filter to PRs targeting this base (default: v8)
"""
...
def render_dashboard(prs: list[PRInfo], base: str = "v8", show_wrong_base: bool = False) -> None:
    ...
    print()
    print(bold(f"  halal-graphify prs  ·  base: {base}  ·  {len(actionable)} PRs"))
    print()
...
def _resolve_triage_backend() -> tuple[str, str]:
    ...
    from halal_graphify.llm import BACKENDS, _get_backend_api_key, _default_model_for_backend
...
def triage_with_opus(prs: list[PRInfo], base: str) -> None:
    try:
        from halal_graphify.llm import BACKENDS, _get_backend_api_key
    except ImportError:
        print(red("  halal_graphify.llm not available - cannot run triage."), file=sys.stderr)
        sys.exit(1)
