...
`halal-graphify reflect` reads the Q&A memory docs that `halal-graphify save-result` files back
...
because ``halal-graphify export wiki`` deletes every ``wiki/*.md`` on each run — a lessons file
...
from halal_graphify.ingest import OUTCOMES
...
def _load_node_community(graph_path: Path, analysis_path: Path,
    ...
    Mirrors how `halal-graphify export wiki` reads graph.json + .graphify_analysis.json +
...
def render_lessons_md(agg: dict[str, Any]) -> str:
    ...
    out: list[str] = [
        "# Lessons",
        "",
        f"_Auto-generated by `halal-graphify reflect` from {agg['total']} session "
...
def lessons_fresh(out_path: Path, memory_dir: Path,
    ...
    Lets ``halal-graphify reflect --if-stale`` skip a redundant run — e.g. when the git
