"""halal-graphify command dispatch — every non-install subcommand.
...
module. The path-redirect (`halal-graphify <path>` -> extract) re-enters via a lazy
...
import sys
import time
from halal_graphify.paths import GRAPHIFY_OUT as _GRAPHIFY_OUT
...
_SEARCH_NUDGE = json.dumps({
    "hookSpecificOutput": {
        ...
        "additionalContext": (
            ...
            '`halal-graphify query "<question>"` before grepping raw files. Only grep '
            'after halal-graphify has oriented you, or to modify/debug specific lines.'
        ),
    }
...
_READ_NUDGE = json.dumps({
    "hookSpecificOutput": {
        "hookEventName": "PreToolUse",
        "additionalContext": (
            'MANDATORY: graphify-out/graph.json exists. You MUST run halal-graphify '
            'before reading source files. Use: `halal-graphify query "<question>"` '
            '(scoped subgraph), `halal-graphify explain "<concept>"`, or '
            '`halal-graphify path "<A>" "<B>"`. Only read raw files after halal-graphify has '
...
_READ_NUDGE_STALE = json.dumps({
    "hookSpecificOutput": {
        ...
        "additionalContext": (
            ...
            'changed after the last build). Prefer `halal-graphify query "<question>"` for '
            'orientation, and run `halal-graphify update` to refresh the graph. Reading the '
...
_READ_DENY = json.dumps({
    "hookSpecificOutput": {
        ...
        "permissionDecisionReason": (
            'halal-graphify strict mode: this project has a fresh knowledge graph that covers '
            'this file. Run `halal-graphify query "<your question>"` (or `halal-graphify explain` / '
            '`halal-graphify path`) FIRST to orient yourself, then re-issue this Read — it '
...
_GEMINI_NUDGE_TEXT = (
    'halal-graphify: knowledge graph at graphify-out/. For focused questions, run '
    '`halal-graphify query "<question>"` (scoped subgraph, usually much smaller than '
...
def _stamped_manifest_files(
    ...
    Mirrors the #1890 path normalization in halal_graphify.llm.
...
def _stale_graph_sources(
    ...
    """
    from halal_graphify.paths import nfc
    try:
    ...
    def _provably_excluded(c: Path) -> bool:
        ...
        for s in spellings:
            ...
            if any(s.startswith(pref) for pref in excluded_prefixes):
                return True
        try:
            from halal_graphify.detect import _is_sensitive as _det_sensitive
            if _det_sensitive(c):
                return True
        except Exception:
            pass
    ...
    if kept_alive:
        print(
            f"[halal-graphify] fail-closed: kept node(s) from {len(kept_alive)} "
...
def _zero_node_stamped_code_sources(
    ...
    from halal_graphify.extract import _get_extractor
    healed: list[str] = []
...
def _prune_graph_json_sources(graph_path: Path, stale_sources: list[str]) -> int:
    ...
    from halal_graphify.export import backup_if_protected as _backup
    _backup(graph_path.parent)
    from halal_graphify.paths import write_json_atomic
...
class _StageTimer:
    ...
    Monotonic (perf_counter), diagnostic-only: emits ``[halal-graphify timing] <stage>:
    ...
    def mark(self, stage: str) -> None:
        now = self._now()
        if self.enabled:
            print(f"[halal-graphify timing] {stage}: {now - self._last:.1f}s", file=sys.stderr)
        self._last = now

    def total(self) -> None:
        if self.enabled:
            print(f"[halal-graphify timing] total: {self._now() - self.start:.1f}s", file=sys.stderr)
def _enforce_graph_size_cap_or_exit(gp: Path) -> None:
    """Reject oversized graph files before parsing (CLI exit-on-fail flavor).

    Delegates to ``halal_graphify.security.check_graph_file_size_cap`` and turns the
    ...
    """
    from halal_graphify.security import check_graph_file_size_cap
    try:
...
def _touch_query_stamp(graph_path: "Path") -> None:
    """Record that halal-graphify oriented the agent recently, next to the queried graph.
    The strict guard suppresses its block while this stamp is fresh. Fail-silent."""
    try:
        from halal_graphify.paths import write_text_atomic
...
def _query_stamp_fresh() -> bool:
    ...
    from halal_graphify.paths import out_path
    try:
...
def _run_hook_guard(kind: str, strict: bool = False) -> None:
    ...
    exists, nudges the agent to use halal-graphify instead of grepping/reading raw files.
    ...
    in-project, fresh code per session is DENIED with a redirect to `halal-graphify query`
    ...
    """
    from halal_graphify.paths import out_path, GRAPHIFY_OUT_NAME
...
def _reenter_main() -> None:
    from halal_graphify.__main__ import main
    main()


def dispatch_command(cmd: str) -> None:
    if cmd == "provider":
        from halal_graphify.llm import _custom_providers_path, BACKENDS
        ...
        if subcmd == "list":
            ...
            if not name:
                print("Usage: halal-graphify provider show <name>", file=sys.stderr)
                sys.exit(1)
            existing = {}
            if global_path.is_file():
                try:
            ...
            if not name:
                print("Usage: halal-graphify provider add <name> --base-url URL --default-model MODEL --env-key KEY", file=sys.stderr)
                sys.exit(1)
            if name in BACKENDS:
            ...
            if not base_url or not default_model or not env_key:
                ...
                sys.exit(1)
            from halal_graphify.llm import provider_base_url_ok
            ...
            print(f"Provider '{name}' added. Use with: halal-graphify extract . --backend {name}")

        elif subcmd == "remove":
            name = sys.argv[3] if len(sys.argv) > 3 else ""
            if not name:
                print("Usage: halal-graphify provider remove <name>", file=sys.stderr)
                sys.exit(1)
            existing = {}
            if global_path.is_file():
                try:
            ...
            print("Usage: halal-graphify provider [add|list|show|remove]", file=sys.stderr)
            if subcmd:
                sys.exit(1)
    elif cmd == "prs":
        from halal_graphify.prs import cmd_prs
        cmd_prs(sys.argv[2:])
    elif cmd == "hook":
        from halal_graphify.hooks import (
        ...
        if subcmd == "install":
            ...
            print("Usage: halal-graphify hook [install|uninstall|status]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "query":
        if len(sys.argv) < 3:
            print("Usage: halal-graphify query \"<question>\" [--dfs] [--context C] [--budget N] [--graph path]", file=sys.stderr)
            sys.exit(1)
        from halal_graphify.serve import _query_graph_text
        from halal_graphify.security import sanitize_label
        from networkx.readwrite import json_graph
        from halal_graphify import querylog
        ...
        try:
            ...
            # per-edge below (mirrors halal_graphify/build.py's _src/_tgt pattern)
            ...
            try:
                from halal_graphify.build import graph_has_legacy_ids as _legacy
                if _legacy(_raw.get("nodes", [])):
                    print(
                        "[halal-graphify] note: this graph uses the pre-#1504 node-ID scheme; "
                        "rebuild with `halal-graphify extract --force` to get path-qualified IDs "
        ...
        if len(sys.argv) < 3:
            print("Usage: halal-graphify affected \"<node-or-label>\" [--relation R] [--depth N] [--graph path]", file=sys.stderr)
            sys.exit(1)
        from halal_graphify.affected import DEFAULT_AFFECTED_RELATIONS, format_affected, load_graph
        query = sys.argv[2]
        graph_path = _default_graph_path()
        depth = 2
        relations: list[str] = []
        ...
        from halal_graphify.paths import GRAPHIFY_OUT_NAME
    ...
    elif cmd in ("hub-nodes", "hub_nodes"):
        # hub_nodes has long been an analyzer (analyze.py), an MCP tool, and a
        # README-advertised capability, but never a CLI subcommand — `halal-graphify
        # hub_nodes` fell through to "unknown command" (#2004). Wire it as a
        # read-only graph query, mirroring `affected`.
        from halal_graphify.affected import load_graph
        from halal_graphify.analyze import hub_nodes as _hub_nodes
        from halal_graphify.security import sanitize_label as _sanitize_label
        graph_path = _default_graph_path()
        top_n = 10
        ...
        try:
            ...
            print(f"error: could not load graph: {exc}", file=sys.stderr)
            sys.exit(1)
        hubs = _hub_nodes(G, top_n=top_n)
        if as_json:
            print(json.dumps(hubs, indent=2))
        else:
            print("Hub nodes (most connected):")
            for rank, n in enumerate(hubs, 1):
                print(f"  {rank}. {_sanitize_label(str(n['label']))} - {n['degree']} edges")
    elif cmd == "save-result":
        # halal-graphify save-result --question Q --answer A [--type T] [--nodes N1 N2 ...]
        #                      [--outcome useful|dead_end|corrected] [--correction TEXT]
        import argparse as _ap

        p = _ap.ArgumentParser(prog="halal-graphify save-result")
        ...
        from halal_graphify.ingest import save_query_result as _sqr
        ...
        p = _ap.ArgumentParser(prog="halal-graphify reflect")
        p.add_argument("--memory-dir", default=str(Path(_GRAPHIFY_OUT) / "memory"))
        p.add_argument(
            "--out",
        ...
        opts = p.parse_args(sys.argv[2:])
        from halal_graphify.reflect import reflect as _reflect, lessons_fresh as _lessons_fresh
        ...
        if len(sys.argv) < 4:
            print(
                'Usage: halal-graphify path "<source>" "<target>" [--graph path] '
                "[--directed|--undirected]",
                file=sys.stderr,
            )
            sys.exit(1)
        from halal_graphify.serve import _pick_scored_endpoint, _score_nodes
        from networkx.readwrite import json_graph
        import networkx as _nx
        ...
        segments = []
        from halal_graphify.build import edge_datas
        ...
        if len(sys.argv) < 3:
            print('Usage: halal-graphify explain "<node>" [--graph path]', file=sys.stderr)
            sys.exit(1)
        from halal_graphify.serve import _find_node, find_node_ambiguity
        from networkx.readwrite import json_graph
        ...
        # Work-memory overlay: a derived experiential hint from `halal-graphify reflect`,
        ...
        try:
            from halal_graphify.reflect import load_learning_overlay as _llo
            from halal_graphify.security import sanitize_label as _sl
            _overlay = _llo(gp)
        ...
        from halal_graphify.build import edge_data
        ...
        if subcmd != "multigraph":
            print(
                "Usage: halal-graphify diagnose multigraph "
        ...
        from halal_graphify.diagnostics import (
            diagnose_file,
        ...
        if len(sys.argv) < 3:
            print(
                "Usage: halal-graphify add <url> [--author Name] [--contributor Name] [--dir ./raw]",
                file=sys.stderr,
            )
            sys.exit(1)
        from halal_graphify.ingest import ingest as _ingest
        ...
        try:
            ...
            print("Run /halal-graphify --update in your AI assistant to update the graph.")
        except Exception as exc:
            print(f"error: {exc}", file=sys.stderr)
            sys.exit(1)
        ...
        if not watch_path.exists():
            ...
            sys.exit(1)
        from halal_graphify.watch import watch as _watch
        ...
        if not graph_json.exists():
            print(
                f"error: no graph found at {graph_json} — run /halal-graphify first",
                file=sys.stderr,
            )
            sys.exit(1)
        from networkx.readwrite import json_graph as _jg
        from halal_graphify.build import build_from_json
        from halal_graphify.cluster import cluster, score_all, remap_communities_to_previous
        from halal_graphify.analyze import (
            hub_nodes,
            surprising_connections,
            suggest_questions,
        )
        from halal_graphify.report import generate
        from halal_graphify.export import to_json, to_html
        ...
        from halal_graphify.security import check_graph_file_size_cap as _check_cap
        try:
        ...
        if labels_path.exists() and not force_relabel:
            ...
            if _ignored_label_flags:
                print(
                    f"[halal-graphify] warning: {'/'.join(_ignored_label_flags)} ignored: "
                    f"reusing saved labels at {labels_path}. "
                    f"Run `halal-graphify label` to relabel, or delete the labels file.",
                    file=sys.stderr,
                )
            ...
            # is told to `halal-graphify label` for fresh LLM names. Unchanged communities keep
            ...
            from halal_graphify.cluster import community_member_sigs, label_communities_by_hub
            ...
            # No labels file yet (or `halal-graphify label` forced a refresh). When run
            ...
            from halal_graphify.cluster import label_communities_by_hub
            from halal_graphify.llm import generate_community_labels
            ...
            generated_labels, _ = generate_community_labels(
                G, label_communities_input, backend=label_backend, model=label_model, hubs=hubs,
        ...
        if not _commit:
            from halal_graphify.watch import _git_head as _gh
        ...
        from halal_graphify.export import backup_if_protected as _backup
        from halal_graphify.exporters.html import _HTML_STALE_MARKER
        _backup(out)
        ...
        from halal_graphify.report import load_learning_for_report as _llfr
        report = generate(G, communities, cohesion, labels, hubs, surprises,
        ...
        if not placeholder_only:
            from halal_graphify.paths import write_json_atomic as _wja
            ...
            # (see reuse above).
            from halal_graphify.cluster import community_member_sigs as _cms
        ...
        if no_viz:
            ...
            try:
                from halal_graphify.exporters.html import _viz_node_limit
        ...
        if not watch_path.exists():
            print(f"error: path not found: {watch_path}", file=sys.stderr)
            sys.exit(1)
        from halal_graphify.watch import _rebuild_code

        print(f"Re-extracting code files in {watch_path} (no LLM needed)...")
        # Interactive CLI: block on the per-repo lock rather than skip, so the
        # user sees their explicit `halal-graphify update` complete instead of
        ...
        if ok:
            print("Code graph updated. For doc/paper/image changes run /halal-graphify --update in your AI assistant.")
            if not (
        ...
        # toward halal-graphify when a fresh in-project graph exists; always exits 0. In
        ...
        if len(sys.argv) < 3:
            print("Usage: halal-graphify check-update <path>", file=sys.stderr)
            sys.exit(1)
        from halal_graphify.watch import check_update
        ...
        from halal_graphify.tree_html import write_tree_html, DEFAULT_MAX_CHILDREN
        ...
        while i_arg < len(args):
            ...
            if a == "--graph" and i_arg + 1 < len(args):
                ...
                print("Usage: halal-graphify tree [--graph PATH] [--output HTML]")
        ...
        # Usage: halal-graphify merge-driver %O %A %B  (set in .git/config merge driver)
        if len(sys.argv) < 5:
            print("Usage: halal-graphify merge-driver <base> <current> <other>", file=sys.stderr)
            sys.exit(1)
        ...
        def _load_graph(p: str):
            ...
            from halal_graphify.paths import load_node_link_graph as _lnlg
            return _lnlg(data), data
        try:
            G_cur, _ = _load_graph(_current_path)
            G_oth, _ = _load_graph(_other_path)
        except Exception as exc:
            print(f"[halal-graphify merge-driver] error loading graphs: {exc}", file=sys.stderr)
        ...
        if merged.number_of_nodes() > _MERGE_MAX_NODES:
            print(
                f"[halal-graphify merge-driver] merged graph has {merged.number_of_nodes()} nodes, "
        ...
        # halal-graphify merge-graphs graph1.json graph2.json ... --out merged.json
        args = sys.argv[2:]
        ...
        if len(graph_paths) < 2:
            print(
                "Usage: halal-graphify merge-graphs <graph1.json> <graph2.json> [...] [--out merged.json]",
                file=sys.stderr,
            )
            sys.exit(1)
        import networkx as _nx
        from networkx.readwrite import json_graph as _jg
        from halal_graphify.build import prefix_graph_for_global as _prefix, distinct_repo_tags as _repo_tags
        graphs = []
        for gp in graph_paths:
            ...
            # Normalize edges/links key before loading — halal-graphify writes "links"
        ...
        # (the halal-graphify default) by converting MultiGraphs with nx.Graph().
        ...
        from halal_graphify.cross_repo_types import link_shared_type_declarations as _link_shared
        ...
        if collected_hyperedges:
            from halal_graphify.export import attach_hyperedges as _attach
        ...
        if len(sys.argv) < 3:
            print(
                "Usage: halal-graphify clone <github-url> [--branch <branch>] [--out <dir>]",
                file=sys.stderr,
            )
            sys.exit(1)
        url = sys.argv[2]
        ...
        if subcmd not in ("html", "callflow-html", "obsidian", "wiki", "svg", "graphml", "neo4j", "falkordb"):
            print("Usage: halal-graphify export <format>", file=sys.stderr)
        ...
        while i < len(args):
            ...
            if a == "--graph" and i + 1 < len(args):
                ...
                print("Usage: halal-graphify export callflow-html [GRAPH|DIR] [--graph PATH] [--labels PATH]")
        ...
        if not graph_path.exists():
            print(f"error: graph not found: {graph_path}. Run /halal-graphify <path> first.", file=sys.stderr)
            sys.exit(1)

        if subcmd == "callflow-html":
            from halal_graphify.callflow_html import write_callflow_html as _write_callflow_html
        ...
        from networkx.readwrite import json_graph as _jg
        from halal_graphify.build import build_from_json as _bfj
        from halal_graphify.security import check_graph_file_size_cap as _check_cap
        ...
        # that happens, `halal-graphify export html` previously bailed with
        ...
        if subcmd == "html":
            from halal_graphify.export import to_html as _to_html
            if no_viz:
            ...
            from halal_graphify.export import to_obsidian as _to_obsidian, to_canvas as _to_canvas
            ...
            if push_uri:
                ...
                from halal_graphify.export import to_cypher as _to_cypher
            ...
            if push_uri:
                from halal_graphify.export import push_to_falkordb as _push
                result = _push(G, uri=push_uri, user=push_user,
                               password=push_password, communities=communities)
                ...
                print(f"cypher.txt written ({out_dir}/cypher.txt) - statements are OpenCypher. "
                      ...
                      f"import), so load a graph with: halal-graphify export falkordb --push "
                      f"falkordb://localhost:6379")

    elif cmd == "benchmark":
        from halal_graphify.benchmark import run_benchmark, print_benchmark
        ...
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        from halal_graphify.global_graph import (
        ...
        if subcmd == "add":
            # halal-graphify global add <graph.json> [--as <tag>]
            args = sys.argv[3:]
            source = None
            tag = None
            i = 0
            while i < len(args):
                if args[i] == "--as" and i + 1 < len(args):
                    ...
                    i += 1
            if not source:
                print("Usage: halal-graphify global add <graph.json> [--as <repo-tag>]", file=sys.stderr)
                sys.exit(1)
            ...
            if not tag:
                print("Usage: halal-graphify global remove <repo-tag>", file=sys.stderr); sys.exit(1)
            try:
            ...
            repos = _global_list()
            if not repos:
                print("Global graph is empty. Use 'halal-graphify global add' to add a project.")
            else:
            ...
            print("Usage: halal-graphify global [add|remove|list|path]", file=sys.stderr); sys.exit(1)
        ...
        if len(sys.argv) < 3:
            print(
                "Usage: halal-graphify extract <path> [--backend gemini|kimi|claude|openai|deepseek|ollama] "
        ...
        # --force parity with `halal-graphify update`: the flag or GRAPHIFY_FORCE=1
        ...
        while i < len(args):
            ...
            if a == "--backend" and i + 1 < len(args):
                ...
                # before, and `halal-graphify tree` already documents --output, so the
        ...
        if deep_mode:
            print("[halal-graphify extract] deep mode enabled: richer semantic extraction")
        ...
        from halal_graphify.watch import (
        ...
        # flag-less `halal-graphify extract` must NOT clobber it back to True, which
        ...
        from halal_graphify.detect import (
            detect as _detect,
        ...
        if force and code_only and existing_graph_path.exists():
            ...
            print(
                "[halal-graphify extract] --force --code-only: full AST re-scan, "
                "existing semantic layer preserved (no semantic pass this run)"
            )
        if force:
            print("[halal-graphify extract] --force: full re-scan, semantic cache reads skipped")
        elif incremental_mode and not manifest_path.exists():
            print(
                "[halal-graphify extract] manifest.json missing; using existing "
        ...
        if not has_path:
            ...
            print(f"[halal-graphify extract] incremental scan of {target}")
            ...
            if _healed_sources:
                print(
                    f"[halal-graphify extract] re-queuing {len(_healed_sources)} "
            ...
            if _healed_sem_sources:
                print(
                    f"[halal-graphify extract] re-queuing {len(_healed_sem_sources)} "
            ...
            print(f"[halal-graphify extract] scanning {target}")
            detection = _detect(
                target,
                google_workspace=google_workspace or None,
                extra_excludes=_effective_excludes or None,
        ...
        if code_only and semantic_files:
            print(
                f"[halal-graphify extract] --code-only: skipping {len(semantic_files)} "
        ...
        if deep_mode and incremental_mode and not code_only:
            ...
            if len(_deep_all) != len(semantic_files):
                print(
                    f"[halal-graphify extract] deep mode: widening semantic pass from "
        ...
        if incremental_mode:
            ...
            print(
                f"[halal-graphify extract] {len(code_files)} code, {len(doc_files)} docs, "
            ...
            print(
                f"[halal-graphify extract] found {len(code_files)} code, "
        ...
        # are no longer invisible in halal-graphify's own output (#1692).
        ...
        if _unclassified:
            ...
            print(
                f"[halal-graphify extract] {len(_unclassified)} file(s) not classified "
        ...
        if _sec:
            ...
            print(
                f"[halal-graphify extract] {len(_sec)} file(s) skipped as potentially sensitive "
        ...
        from halal_graphify.llm import (
        ...
        if needs_llm:
            if backend is None:
                ...
                sys.exit(1)
            if backend == "ollama":
                from halal_graphify.llm import _validate_ollama_base_url
        ...
        if code_files:
            from halal_graphify.extract import extract as _ast_extract
            ...
            # #2437/#2438 (the `halal-graphify update` twin of watch's #2406 fix): an
            ...
            if incremental_mode and existing_graph_path.exists():
                ...
                try:
                    from halal_graphify.build import _is_ast_tier as _ctx_is_ast_tier
                    from halal_graphify.security import (
            ...
            print(f"[halal-graphify extract] AST extraction on {len(code_files)} code files...")
            try:
                ast_result = _ast_extract(code_files, **ast_kwargs)
            except Exception as exc:
                print(f"[halal-graphify extract] AST extraction failed: {exc}", file=sys.stderr)
        ...
        from halal_graphify.cache import (
        ...
        from halal_graphify.llm import _extraction_system as _sem_prompt_for
        ...
        if semantic_files:
            ...
            if sem_cache_hits:
                print(f"[halal-graphify extract] semantic cache: {sem_cache_hits} hit / {sem_cache_misses} miss")

            if uncached_paths:
                print(f"[halal-graphify extract] semantic extraction on {len(uncached_paths)} files via {backend}...")
                corpus_kwargs: dict = {
                    "backend": backend,
                    "model": model,
                    "root": target,
                ...
                def _progress(idx: int, total: int, _result: dict) -> None:
                    ...
                    print(
                        f"[halal-graphify extract] chunk {idx + 1}/{total} done",
                        flush=True,
                    )
                ...
                try:
                    ...
                    print(
                        f"[halal-graphify extract] semantic extraction failed: {exc}",
                        file=sys.stderr,
                    )
                ...
                if uncached_paths and _chunk_stats["succeeded"] == 0:
                    print(
                        f"[halal-graphify extract] error: all semantic chunks failed "
                ...
                if _dropped_files:
                    print(
                        f"[halal-graphify extract] dropped {_dropped_items} out-of-scope "
                ...
                try:
                    ...
                    print(f"[halal-graphify extract] warning: could not write semantic cache: {exc}", file=sys.stderr)
        ...
        try:
            from halal_graphify.cache import file_hash as _file_hash
            ...
            print(f"[halal-graphify extract] warning: could not prune semantic cache: {exc}", file=sys.stderr)
        stages.mark("semantic extract")

        pg_result: dict = {"nodes": [], "edges": []}
        if cli_postgres_dsn is not None:
            from halal_graphify.pg_introspect import introspect_postgres
            print(f"[halal-graphify extract] introspecting PostgreSQL schema...")
            try:
                pg_result = introspect_postgres(cli_postgres_dsn)
            except (ConnectionError, ImportError) as exc:
                print(f"error: {exc}", file=sys.stderr)
                sys.exit(1)
            print(f"[halal-graphify extract] PostgreSQL: {len(pg_result['nodes'])} nodes, "
                  f"{len(pg_result['edges'])} edges")

        cargo_result: dict = {"nodes": [], "edges": []}
        if cli_cargo:
            from halal_graphify.cargo_introspect import introspect_cargo
            print("[halal-graphify extract] introspecting Cargo workspace...")
            try:
                cargo_result = introspect_cargo(target)
            except (ConnectionError, ImportError, OSError) as exc:
                print(f"error: {exc}", file=sys.stderr)
                sys.exit(1)
            print(f"[halal-graphify extract] Cargo: {len(cargo_result['nodes'])} nodes, "
        ...
        if no_cluster:
            ...
            from halal_graphify.build import dedupe_edges as _dedupe_edges, dedupe_nodes as _dedupe_nodes
            from halal_graphify.export import (
            ...
            if (
                ...
                if graph_stale_sources:
                    ...
                    if _n_pruned:
                        print(
                            f"[halal-graphify extract] pruned {_n_pruned} node(s) from "
                            f"{len(graph_stale_sources)} source file(s) no longer "
                            "in the scan (deleted or excluded)."
                        )
                print(
                    "[halal-graphify extract] no incremental changes detected "
                ...
                try:
                    ...
                    print(f"[halal-graphify extract] warning: could not write manifest: {exc}", file=sys.stderr)
                stages.total()
                sys.exit(0)
            ...
            if incremental_mode:
                ...
                from halal_graphify.build import merge_raw_extraction as _merge_raw_extraction
            ...
            from halal_graphify.build import disambiguate_file_labels_in_nodes as _disamb_labels
            ...
            if _extraction_incomplete and not cli_allow_partial:
                from halal_graphify.export import MALFORMED_GRAPH as _MALFORMED_GRAPH
                ...
                if _malformed or _shrinks:
                    ...
                    print(
                        "[halal-graphify extract] error: extraction was incomplete (an AST/"
            ...
            from halal_graphify.paths import write_json_atomic as _write_json_atomic
            ...
            try:
                ...
                print(f"[halal-graphify extract] warning: could not write manifest: {exc}", file=sys.stderr)
            if global_merge:
                from halal_graphify.global_graph import global_add as _global_add
                _tag = global_repo_tag or target.name
                try:
                    result = _global_add(graphify_out / "graph.json", _tag)
                    if result["skipped"]:
                        print(f"[halal-graphify global] '{_tag}' unchanged since last add - skipped.")
                    else:
                        print(f"[halal-graphify global] '{_tag}' merged into global graph "
                              f"(+{result['nodes_added']} nodes, -{result['nodes_removed']} pruned).")
                except Exception as exc:
                    print(f"[halal-graphify global] warning: failed to merge into global graph: {exc}", file=sys.stderr)
            stages.total()
            sys.exit(0)

        # Build graph + cluster + score + write.
        from halal_graphify.build import (
            build as _build,
            build_from_json as _build_from_json,
            build_merge as _build_merge,
        )
        from halal_graphify.cluster import cluster as _cluster, score_all as _score_all
        from halal_graphify.export import to_json as _to_json
        from halal_graphify.analyze import hub_nodes as _hub_nodes, surprising_connections as _surprising
        ...
        if incremental_mode:
            ...
            try:
                ...
                print(f"[halal-graphify extract] {exc}", file=sys.stderr)
                sys.exit(1)
        else:
        ...
        if G.number_of_nodes() == 0:
            print(
                "[halal-graphify extract] graph is empty — extraction produced no nodes. "
        ...
        # command was invoked from, so `halal-graphify extract <target>` run from
        ...
        # extract path.
        from halal_graphify.watch import _git_head as _gh_target
        ...
        if not _wrote:
            ...
            # re-attempts.
            print(
                "[halal-graphify extract] error: extraction was incomplete (an AST/semantic "
        ...
        print(f"[halal-graphify extract] wrote {analysis_path}")
        if incremental_mode:
            _excl_note = f", {len(excluded_files)} excluded" if excluded_files else ""
            print(
                f"[halal-graphify extract] incremental summary: "
            ...
            print(f"[halal-graphify extract] semantic cache: {sem_cache_hits} cached, {sem_cache_misses} re-extracted")
        if merged["input_tokens"] or merged["output_tokens"]:
            print(
                f"[halal-graphify extract] tokens: "
                f"{merged['input_tokens']:,} in / "
        ...
        print(
            "[halal-graphify extract] next: run "
            f"`halal-graphify cluster-only {graphify_out.parent}` "
            "to generate GRAPH_REPORT.md and name communities"
        )
        stages.total()

    elif cmd == "cache-check":
        # halal-graphify cache-check <files_from> [--root <dir>] [--mode <m> | --deep]
        #                       [--prompt-file <path>]
        ...
        from halal_graphify.cache import check_semantic_cache
        if len(sys.argv) < 3:
            print("Usage: halal-graphify cache-check <files_from> [--root <dir>] "
        ...
        # halal-graphify merge-chunks <chunk_glob_or_files...> --out <path>
        ...
        if len(sys.argv) < 3:
            print("Usage: halal-graphify merge-chunks <chunk_files...> --out <path>", file=sys.stderr)
            sys.exit(1)
        ...
        from halal_graphify.semantic_cleanup import load_validated_semantic_fragment
        for cf in chunk_files:
            chunk, _chunk_errs = load_validated_semantic_fragment(Path(cf))
            if _chunk_errs:
                print(
                    f"[halal-graphify merge-chunks] warning: skipping invalid chunk {cf}: "
        ...
        if not valid_chunks:
            print(
                f"[halal-graphify merge-chunks] error: no valid chunks to merge; "
        ...
        # halal-graphify merge-semantic --cached <path> --new <path> --out <path>
        ...
        if len(sys.argv) < 3:
            print("Usage: halal-graphify merge-semantic --cached <path> --new <path> --out <path>", file=sys.stderr)
            sys.exit(1)
        ...
        # User ran `halal-graphify <path>` directly — treat as `halal-graphify extract <path>`.
        # Common when following the PowerShell note in README (`halal-graphify .`) or
        ...
        print("Run 'halal-graphify --help' for usage.", file=sys.stderr)
        sys.exit(1)
