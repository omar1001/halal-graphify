...
from halal_graphify.analyze import _node_community_map
from halal_graphify.build import edge_data
from halal_graphify.paths import stem_filename_budget

from halal_graphify.exporters.graphdb import push_to_falkordb, push_to_neo4j  # noqa: E402,F401
        ...
        for name in _BACKUP_ARTIFACTS:
            ...
            if src.exists():
                try:
                    ...
                    pass
        if copied:
            print(f"[halal-graphify] backed up {reason} graph ({copied} files) -> {backup_dir.name}/")
        return backup_dir
    except Exception as exc:
        import sys
        print(f"[halal-graphify] warning: backup failed ({exc}) - continuing with overwrite", file=sys.stderr)
        return None
...
def _yaml_str(s: str) -> str:
    ...
    See `halal_graphify.ingest._yaml_str` for the full rationale; duplicated here to
...
from halal_graphify.exporters.base import COMMUNITY_COLORS  # noqa: E402,F401

from halal_graphify.exporters.html import to_html  # noqa: E402,F401
...
def to_json(G: nx.Graph, communities: dict[int, list[str]], output_path: str, *, force: bool = False, built_at_commit: str | None = None, community_labels: dict[int, str] | None = None) -> bool:
        ...
        if not oversized:
            ...
            if not raw.strip():
                ...
                try:
                    ...
                    import sys as _sys
                    print(
                        f"[halal-graphify] WARNING: existing {existing_path} could not be "
            ...
            if new_n < existing_n:
                import sys as _sys
                print(
                    f"[halal-graphify] WARNING: new graph has {new_n} nodes but existing "
                    ...
                    f"Run a full rebuild (/halal-graphify .) to be safe, or pass force=True "
    ...
    if "hyperedges" not in getattr(G, "graph", {}):
        ...
        if _prev_hyperedges:
            print(
                f"[halal-graphify] WARNING: graph carries no hyperedge metadata but "
...
def to_cypher(G: nx.Graph, output_path: str) -> None:
    lines = ["// Neo4j Cypher import - generated by /halal-graphify", ""]
...
# A frontmatter tag entry in halal-graphify's own namespace, e.g. "  - halal_graphify/document".
_GRAPHIFY_TAG_RE = re.compile(r"^\s*-\s+halal_graphify/\S")
...
# Community notes carry no frontmatter; halal-graphify identifies its own by the
...
def _is_graphify_note(path: Path) -> bool:
    """Whether a vault note carries halal-graphify's own frontmatter signature.

    Every note halal-graphify writes opens with a YAML frontmatter block tagging it in
    the ``halal_graphify/`` namespace::

        ---
        source_file: "d0.md"
        tags:
          - halal_graphify/document
          - halal_graphify/EXTRACTED
        ---

    Only that block is inspected, and only a tag entry inside it counts — a
    user's note that merely mentions halal-graphify in its prose is not adopted.

    Community overview notes are recognised separately: they carry no
    frontmatter at all, so they are identified by halal-graphify's own filename prefix
    ...
    for line in head.splitlines()[1:]:
        if line.strip() == "---":
            return False  # frontmatter closed without a halal-graphify tag
...
def _adopt_pre_manifest_notes(out: Path) -> set[str]:
    """Names of notes in *out* that halal-graphify itself wrote before manifests existed.

    Deliberately limited to top-level ``*.md``: those are the only files halal-graphify
    can identify as its own from their content. ``.obsidian/graph.json`` is NOT
    adopted — halal-graphify writes one, but so does Obsidian, and with no manifest
...
def to_obsidian(
    ...
    # .obsidian/ config. Track the files halal-graphify owns in a manifest; a pre-existing
    ...
    if not _manifest_existed:
        ...
        # halal-graphify owns, so every note it wrote last time reads as the user's and
        # is skipped. The re-export then writes fresh notes BESIDE the stale ones
        # and the vault carries two generations, with a warning claiming halal-graphify
        # "did not create" files it did (#2863). Adopt the notes that carry
        # halal-graphify's own frontmatter, once, so the manifest starts out honest.
    ...
    def _owned_write(rel_name: str, content: str) -> bool:
        ...
        halal-graphify didn't create. Returns True if written."""
        target = out / rel_name
    ...
    # Map file_type → halal-graphify tag
    _FTYPE_TAG = {
        "code": "halal_graphify/code",
        "document": "halal_graphify/document",
        "paper": "halal_graphify/paper",
        "image": "halal_graphify/image",
    }
    ...
    for node_id, data in G.nodes(data=True):
        ...
        ftype_tag = _FTYPE_TAG.get(ftype, f"halal_graphify/{ftype}" if ftype else "halal_graphify/document")
        dom_conf = _dominant_confidence(node_id)
        conf_tag = f"halal_graphify/{dom_conf}"
    ...
    # view — but never clobber an existing .obsidian/graph.json halal-graphify doesn't own
    ...
    # manifest says halal-graphify owns are candidates, and anything written or skipped
    ...
    if pruned:
        print(
            f"[halal-graphify] pruned {pruned} note(s) for nodes no longer in the graph",
            file=sys.stderr,
        )

    # Persist the manifest of files halal-graphify owns, so a re-run can safely update its
    ...
    if _skipped:
        ...
        print(
            f"[halal-graphify] WARNING: skipped {len(_skipped)} pre-existing file(s) halal-graphify "
