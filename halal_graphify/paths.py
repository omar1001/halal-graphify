"""Single source of truth for the halal-graphify output-directory name.
...
def _atomic_replace(path: "str | Path", write_fn) -> None:
    ...
    try:
        ...
        try:
            ...
            # back to copy-then-delete, matching halal_graphify.cache's atomic writer.
            import shutil
...
def _path_proximity_winner(call_site_file: str, candidate_files: dict[str, str]) -> str | None:
    ...
    hub-node guard still holds when proximity is genuinely ambiguous.
    """
...
def disambiguate_ambiguous_candidates(
    ...
    Shared hub-node tie-breaker (#1553) used by both the inline cross-file call
    ...
    survives; otherwise ``None`` (caller keeps the hub-node guard / ``continue``).
...
def load_node_link_graph(path_or_data):
    """Load a halal-graphify graph.json into a networkx graph, accepting both writers.
    ...
    if not isinstance(data, dict):
        p = Path(data)
        from halal_graphify.security import check_graph_file_size_cap  # lazy: security imports paths
