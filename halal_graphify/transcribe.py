...
from halal_graphify.paths import out_path as _out_path
...
def _get_whisper():
    try:
        ...
        raise ImportError(
            ...
            "Run: pip install 'halal-graphify[video]'"
        ) from exc
...
def download_audio(url: str, output_dir: Path) -> Path:
    ...
    """
    from halal_graphify.security import validate_url
...
def build_whisper_prompt(hub_nodes: list[dict]) -> str:
    """Build a domain hint for Whisper from hub nodes extracted from the corpus.

    Formats the top hub node labels into a topic string for Whisper.
    ...
    labels = [n.get("label", "") for n in hub_nodes[:10] if n.get("label")]
    if not labels:
        return _FALLBACK_PROMPT
...
def transcribe(
    ...
    initial_prompt: domain hint for Whisper (built from corpus hub nodes).
...
def transcribe_all(
    ...
    initial_prompt is shared across all files — built once from corpus hub nodes.
    """
    if not video_files:
        return []
