...
from halal_graphify.ids import make_id as _shared_make_id
...
def resolve_cross_file_raw_calls(
    ...
    known_pairs = existing_edge_pairs(all_edges)
    # nid -> source_file, for the shared hub-node tie-breakers (#1553) so a
    ...
    for raw_call in iter_raw_calls(per_file):
        ...
        if len(candidates) == 1:
            ...
            # candidate survives, else preserve the hub-node guard and skip.
...
def _bash_make_id(*parts: str) -> str:
    ...
    Previously an inline copy to dodge an import cycle; ``halal_graphify.ids`` is
    dependency-free, so it can be imported directly.
    """
    return _shared_make_id(*parts)


from halal_graphify.extractors.base import _file_stem as _bash_file_stem  # canonical recipe (no import cycle: base imports only halal_graphify.ids)
