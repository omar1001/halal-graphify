...
``halal_graphify.resolver_registry``), with the full merged node/edge corpus
...
def resolve_pascal_inherited_calls(
    ...
    ``inherits`` chain (hub-node guard, same principle as
    ...
    for e in all_edges:
        if e.get("relation") == "inherits":
            ...
            # hub-node guard below (`len(candidates) == 1`) measuring real
