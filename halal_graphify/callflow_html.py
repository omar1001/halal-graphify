#!/usr/bin/env python3
"""
callflow_html.py — Generate call-flow architecture HTML from halal_graphify knowledge graph outputs.
...
Usage:
  python3 -m halal_graphify export callflow-html
  python3 -m halal_graphify export callflow-html /path/to/project/graphify-out/graph.json
  python3 -m halal_graphify export callflow-html --graph /path/to/graph.json --output docs/architecture.html
"""
...
from halal_graphify.paths import GRAPHIFY_OUT, GRAPHIFY_OUT_NAME
...
def normalize_node(raw: dict, index: int) -> dict:
    """Normalize a halal-graphify node across common graph.json schema variants."""
    node = dict(raw)
...
def normalize_edge(raw: dict, index: int) -> dict | None:
    """Normalize halal-graphify edges while preserving original fields."""
    edge = dict(raw)
...
def _node_link_payload(data: dict) -> tuple[list, list] | None:
    """Read current halal-graphify graph.json via NetworkX's node-link parser."""
...
def resolve_graphify_paths(args) -> dict:
    """Resolve project root, halal-graphify output dir, and optional files."""
...
SECTION_ARCHETYPES = [
    ...
    (
        ...
        "Analysis & Clustering",
        {
            "cluster", "community", "leiden", "cohesion", "analyze", "hub",
...
def node_importance(node: dict) -> float:
    """Use halal-graphify centrality fields when available."""
...
def generate_header(sections: list, meta: dict, lang: str) -> str:
    ...
    if lang.startswith("zh"):
        ...
        subtitle = (
            f"由 halal-graphify 知识图谱生成：{meta.get('node_count', '?')} 个节点、"
        ...
        subtitle = (
            f"Generated from halal_graphify knowledge graph: {meta.get('node_count', '?')} nodes, "
...
def generate_section_cards(sec: dict, nodes: list, section_edges: list, lang: str) -> str:
    ...
    note = pick_text(
        lang,
        f"本节由 halal-graphify 社区聚类生成。关系概况：{relation_text}。图表优先展示高置信、跨节点调用或使用关系，完整节点清单位于表格中。",
        f"This section comes from halal_graphify community clustering. Relationship summary: {relation_text}. The diagram prioritizes high-confidence calls or usage relationships; the table keeps the broader node inventory.",
    )
...
def _report_highlights(report_text: str, lang: str) -> str:
    ...
    for line in lines:
        ...
        if stripped.startswith("## "):
            ...
            in_hubs = stripped.startswith("## Hub Nodes")
            continue
        if in_summary and stripped.startswith("- "):
            keep.append(stripped[2:])
        elif in_hubs and re.match(r"^\d+\.", stripped):
...
def write_callflow_html(
    ...
    *,
    ...
    """Generate call-flow architecture HTML from halal_graphify output files."""
    ...
    if not paths["graph"].exists():
        raise FileNotFoundError(
            f"halal-graphify output not found: {paths['graph']}. "
            "Run halal-graphify first or pass --graph /path/to/graph.json."
        )
...
""")
  ...
  <p>Generated: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')} · halal-graphify callflow-html</p>
</div>
""")
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate call-flow architecture HTML from halal_graphify knowledge graph outputs"
    )
    parser.add_argument("project", nargs="?", default=None, help="Project root or halal-graphify output directory")
    parser.add_argument("--graphify-out", default=None, help="Path to halal-graphify output directory")
