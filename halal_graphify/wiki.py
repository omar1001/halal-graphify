...
# and resolves fine on every filesystem halal-graphify targets; stripping it would
...
def _md_link(label: str, resolver: dict[str, str]) -> str:
    ...
    hub nodes get article files — render as plain text instead of a dead link
...
def _community_article(
    ...
    lines += ["---", "", f"*Part of the halal-graphify knowledge wiki. See {_md_link('index', resolver)} to navigate.*"]
    return "\n".join(lines)


def _hub_node_article(G: nx.Graph, nid: str, labels: dict[int, str], node_community: dict[str, int] | None = None, resolver: dict[str, str] | None = None) -> str:
    resolver = resolver or {}
    d = G.nodes[nid]
    node_label = d.get("label", nid)
    src = d.get("source_file", "")
...
def _index_md(
    ...
    lines: list[str] = [
        ...
        "",
        "> Auto-generated by halal-graphify. Start here — read community articles for context, then drill into hub nodes for detail.",
        "",
    ...
    lines += [
        "---",
        "",
        "*Generated by [halal-graphify](https://github.com/safishamsi/graphify)*",
    ]
    return "\n".join(lines)
...
def to_wiki(
    ...
    if not communities:
        raise ValueError(
            ...
            "Run `halal-graphify extract .` or `halal-graphify cluster-only .` first."
        )
    ...
    if not communities:
        raise ValueError(
            ...
            "Re-run `halal-graphify extract .` to regenerate .graphify_analysis.json."
        )
    ...
    # the communities dict), so _cross_community_links and _hub_node_article need this.
    ...
    # _md_link renders them as plain text. Communities are slugged before hub nodes
    ...
    for nid, slug in hub_articles:
        article = _hub_node_article(G, nid, labels, node_community, resolver)
        (out / f"{slug}.md").write_text(article, encoding="utf-8")
        count += 1

    # Index
    (out / "index.md").write_text(
        _index_md(communities, labels, hub_nodes_data, G.number_of_nodes(), G.number_of_edges(), resolver),
        encoding="utf-8",
    )
