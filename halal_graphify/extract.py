...
# --- migrated to halal_graphify/extractors/ (see halal_graphify/extractors/MIGRATION.md) ---
from halal_graphify.extractors.base import (  # noqa: F401
    _LANGUAGE_BUILTIN_GLOBALS,
    _file_stem,
    _make_id,
    _read_text,
)
from halal_graphify.extractors.apex import extract_apex  # noqa: F401
from halal_graphify.extractors.bash import extract_bash  # noqa: F401
from halal_graphify.extractors.blade import extract_blade  # noqa: F401
from halal_graphify.extractors.csharp import (
    CsharpNameResolver,
    _resolve_cross_file_csharp_imports,
    _resolve_csharp_type_references,
)
from halal_graphify.extractors.dart import extract_dart  # noqa: F401
from halal_graphify.extractors.dm import extract_dm, extract_dmf, extract_dmi, extract_dmm  # noqa: F401
from halal_graphify.extractors.elixir import extract_elixir  # noqa: F401
from halal_graphify.extractors.fortran import _cpp_preprocess, extract_fortran  # noqa: F401
from halal_graphify.extractors.go import _GO_PREDECLARED_FUNCS, extract_go  # noqa: F401
from halal_graphify.extractors.json_config import extract_json  # noqa: F401
from halal_graphify.extractors.commonlisp import extract_commonlisp  # noqa: F401
from halal_graphify.extractors.markdown import extract_markdown, _MD_LINK_INDEX_CACHE  # noqa: F401
from halal_graphify.extractors.ocaml import extract_ocaml  # noqa: F401
from halal_graphify.extractors.pascal_forms import extract_delphi_form, extract_lazarus_form  # noqa: F401
from halal_graphify.extractors.powershell import extract_powershell, extract_powershell_manifest  # noqa: F401
from halal_graphify.extractors.razor import extract_razor  # noqa: F401
from halal_graphify.extractors.rust import extract_rust  # noqa: F401
from halal_graphify.extractors.sln import extract_sln  # noqa: F401
from halal_graphify.extractors.sql import extract_sql  # noqa: F401
...
from halal_graphify.extractors.models import LanguageConfig, _JS_CACHE_BYPASS_SUFFIXES, _NamespaceExportFact, _StarExportFact, _SymbolAliasFact, _SymbolDeclarationFact, _SymbolExportFact, _SymbolImportFact, _SymbolResolutionFacts, _SymbolUseFact, _WORKSPACE_PACKAGE_CACHE  # noqa: E402,F401

from halal_graphify.extractors.resolution import (  # noqa: E402,F401
...
# Without this filter they become hub-nodes accumulating spurious edges from
...
def _resolve_swift_member_calls(
    ...
    method name (``update``) collides across the corpus and inflates hub-nodes
    ...
    # len != 1 is the hub-node guard: an ambiguous type name bails.
    ...
    for rc in all_raw_calls:
        ...
        if len(type_defs) != 1:  # ambiguous or absent -> bail (hub-node guard)
            continue
...
def _resolve_python_member_calls(
    ...
    The shared cross-file call pass drops every ``is_member_call`` because a bare
    method name (``log``) collides across the corpus and inflates hub-nodes
    ...
    with a single-definition hub-node guard.
    ...
    # class label -> owning class node ids (len != 1 is the hub-node guard), and
    ...
    def _emit_call(caller: str, target_nid: "str | None", rc: dict) -> None:
        ...
        # definition (each arm applies a single-definition hub-node guard).
        all_edges.append({
            "source": caller,
            "target": target_nid,
            "relation": "calls",
    ...
    for rc in all_raw_calls:
        ...
        if receiver[:1].isupper():
            ...
            if len(class_nids) != 1:  # absent or ambiguous -> bail (hub-node guard)
                continue
...
def _resolve_typescript_member_calls(
    ...
    ``findById`` collides across the corpus (hub-node guard).  TS constructors with
    ...
    for rc in all_raw_calls:
        ...
        # phantom `references[call]` edges and a false hub node (#1726). The
...
def _resolve_cpp_member_calls(
    ...
    name (``bar``) collides across the corpus and inflates hub-nodes (#543/#1219).
    ...
    to exactly ONE definition (the hub-node guard).
...
def _resolve_csharp_member_calls(
    ...
    match (the hub-node guard). An untypable/ambiguous receiver is skipped — never
    a guess.
    ...
    for rc in all_raw_calls:
        ...
        if receiver == "this":
            ...
            if not type_name:
                continue
            type_nid = _resolve_type_name_nid(type_name, caller_node, src_file)
            if not type_nid:  # ambiguous or absent -> bail (hub-node guard)
                continue
            type_qualified = False
...
def _resolve_objc_member_calls(
    ...
    type resolves to exactly ONE definition (the hub-node guard).
    ...
    protocol's declaration and, when a real class existed, trip the hub-node guard.
...
def _resolve_kotlin_qualified_calls(
    ...
    for rc in raw:
        ...
        if len(candidates) != 1:  # zero or ambiguous -> no edge (hub-node guard)
            continue
        tgt = candidates[0]
        if tgt == caller or (caller, tgt) in existing_pairs:
            continue
...
# registry (framework lives in halal_graphify.resolver_registry). A new language plugs in
...
# halal_graphify.ruby_resolution; registered here as a second consumer of the framework.
register_language_resolver(
...
# scope. Lives in halal_graphify.pascal_resolution; registered here as a consumer
...
# graph unreachable from the call site. Lives in halal_graphify.csharp_dispatch;
...
# Extensions halal-graphify creates document file nodes for. A link to one of these
...
def _check_tree_sitter_version() -> None:
    ...
    if LANGUAGE_VERSION < 14:
        ...
        raise RuntimeError(
            ...
            f"halal-graphify requires tree-sitter >= 0.23.0 (Language API v2). "
...
def _xaml_csharp_class_nodes(path: Path) -> dict[str, list[dict]]:
    from halal_graphify.detect import _is_ignored, _is_noise_dir, _load_graphifyignore
...
def _get_extractor(path: Path) -> Any | None:
    ...
    if not suffix:
        from halal_graphify.detect import _shebang_interpreter
...
def extract(
    ...
    *,
    ...
    # was never observed again for the life of the process. `halal-graphify watch`
    ...
    from halal_graphify.detect import CODE_EXTENSIONS as _CODE_EXTS
    ...
    if _no_extractor:
        ...
        print(
            f"  warning: {_tot} file(s) are classified as code but halal-graphify has no AST "
    ...
    for _ext, _n in sorted(_missing_dep_count.items(), key=lambda kv: (-kv[1], kv[0])):
        ...
        if _extra and _DEP_MISSING_MARKER in _err_text:
            ...
            _hint = f' Install it with: pip install "halal-graphify[{_extra}]"'
    ...
    # those inflates hub_nodes ranking with spurious cross-file edges.
    ...
    for n in resolution_nodes:
        ...
        if normalised:
            ...
            # single shell variable the #1 hub-node (#1581). Only case-insensitive
        ...
        if len(candidates) == 1:
            ...
            if len(symbol_matches) == 1:
                ...
                if len(module_matches) == 1:
                    ...
                    # real call graph, #1553), apply the shared hub-node
    ...
    # single-definition hub-node guard. Registered in halal_graphify.resolver_registry so
...
def collect_files(target: Path, *, follow_symlinks: bool = False, root: Path | None = None) -> list[Path]:
    ...
    from halal_graphify.detect import _resolves_under_root
    if target.is_file():
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python -m halal_graphify.extract <file_or_dir> ...", file=sys.stderr)
        sys.exit(1)
