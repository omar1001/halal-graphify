"""Graph analysis: hub nodes (most connected), surprising connections (cross-community), suggested questions."""
...
# graphs. Excluded from hub-node ranking so they don't displace real abstractions
...
_BUILTIN_NOISE_LABELS = frozenset({
    ...
    # otherwise dominate hub-node rankings on Swift codebases (#2147)
...
def _is_file_node(G: nx.Graph, node_id: str) -> bool:
    ...
    from hub nodes, surprising connections, and knowledge gap reporting.
    """
    ...
    if source_file:
        from halal_graphify.build import _is_file_node_label
...
def hub_nodes(G: nx.Graph, top_n: int = 10) -> list[dict]:
...
from halal_graphify.detect import CODE_EXTENSIONS, DOC_EXTENSIONS, PAPER_EXTENSIONS, IMAGE_EXTENSIONS
...
def _cross_file_surprises(G: nx.Graph, communities: dict[int, list[str]], top_n: int) -> list[dict]:
    ...
    - Peripheral→hub (low-degree node reaching a hub node)
...
def _cross_community_surprises(
    ...
    # Without this, a single high-betweenness hub node dominates all results.
...
def suggest_questions(
    ...
    Based on: AMBIGUOUS edges, bridge nodes, underexplored hub nodes, isolated nodes.
    ...
    # 3. Hub nodes with many INFERRED edges → verification questions
    degree = dict(G.degree())
    top_nodes = sorted(
