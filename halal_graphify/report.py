...
def _portable_root_label(root: str) -> str:
    ...
    filesystem, and makes `halal-graphify update .`, `halal-graphify update ./proj`, and
    `halal-graphify update /abs/path/proj` all label the header `proj`. Only the
...
def load_learning_for_report(graph_path) -> dict | None:
    ...
    try:
        gp = _Path(graph_path)
        from halal_graphify.reflect import load_learning_overlay, load_memory_docs, aggregate_lessons
...
def generate(
    ...
    if built_at_commit:
        lines += [
            ...
            "- Run `halal-graphify update .` after code changes (no API cost).",
        ]
    ...
    for i, node in enumerate(hub_node_list, 1):
    ...
    if gap_count > 0 or amb_pct > 20:
        ...
        if thin_communities:
            lines.append(f"- **{len(thin_communities)} thin communities (<{min_community_size} nodes) omitted from report** — run `halal-graphify query` to explore isolated nodes.")
        if amb_pct > 20:
