...
from halal_graphify._minhash import MinHash, MinHashLSH
    ...
    for loser in losers:
        ...
        if lose_file == keep_file:
            if _norm(lose_label) != _norm(keep_label):
                print(
                    f"[halal-graphify] note: node '{nid}' was extracted twice from "
            ...
            print(
                f"[halal-graphify] WARNING: node '{nid}' is minted by two different files — "
                ...
                f"dropped node is lost. To keep them distinct, run 'halal-graphify extract' "
                f"per subfolder and merge with 'halal-graphify merge-graphs'.",
                file=sys.stderr,
            )
...
def _llm_tiebreak(
    ...
    *,
    ...
    try:
        from halal_graphify.llm import BACKENDS, _format_backend_env_keys, _get_backend_api_key
        if backend not in BACKENDS:
            print(f"[halal-graphify] --dedup-llm: unknown backend {backend!r}, skipping LLM tiebreaker.", flush=True)
            return
        if not _get_backend_api_key(backend):
            env_keys = _format_backend_env_keys(backend)
            print(f"[halal-graphify] --dedup-llm: {env_keys} not set, skipping LLM tiebreaker.", flush=True)
            return
    except ImportError:
        return
    ...
    try:
        ...
        # didn't exist in `halal_graphify.llm` at all, so `--dedup-llm` was a no-op.
        # Surface the import failure so future regressions are visible.
        print(
            f"[halal-graphify] --dedup-llm: cannot import _call_llm ({exc}); skipping LLM tiebreaker.",
            flush=True,
        )
        return
