"""halal-graphify install/uninstall subsystem.

The per-platform skill/hook installers and uninstallers, extracted verbatim from
halal_graphify/__main__.py so the CLI dispatcher (`main`) stays readable. Import path is
unchanged: `halal_graphify.__main__` re-exports every name defined here, so
`from halal_graphify.__main__ import claude_install` (etc.) keeps working.

Lives at halal_graphify/install.py (same package dir as __main__) on purpose: the
installers resolve packaged assets via `Path(__file__).parent / "always_on"` and
`/ "skills"`, which only works from within the halal_graphify/ directory.
"""
...
def _always_on(basename: str) -> str:
    """Read a packaged always-on instruction block from halal_graphify/always_on/.
...
def _platform_skill_destination(platform_name: str, *, project: bool = False, project_dir: Path | None = None) -> Path:
    ...
    if platform_name == "gemini":
        if project:
            return (project_dir or Path(".")) / ".gemini" / "skills" / "halal-graphify" / "SKILL.md"
        if platform.system() == "Windows":
            return Path.home() / ".agents" / "skills" / "halal-graphify" / "SKILL.md"
        return Path.home() / ".gemini" / "skills" / "halal-graphify" / "SKILL.md"

    if platform_name == "opencode":
        if project:
            return (project_dir or Path(".")) / ".opencode" / "skills" / "halal-graphify" / "SKILL.md"
        return Path.home() / ".config" / "opencode" / "skills" / "halal-graphify" / "SKILL.md"

    if platform_name == "hermes":
        if project:
            return (project_dir or Path(".")) / ".hermes" / "skills" / "halal-graphify" / "SKILL.md"
        # On Windows, Hermes scans %LOCALAPPDATA%\hermes\skills, not ~/.hermes (#1403).
        if platform.system() == "Windows":
            local_appdata = Path(os.environ.get("LOCALAPPDATA") or (Path.home() / "AppData" / "Local"))
            return local_appdata / "hermes" / "skills" / "halal-graphify" / "SKILL.md"
        return Path.home() / ".hermes" / "skills" / "halal-graphify" / "SKILL.md"

    if platform_name == "devin":
        if project:
            return (project_dir or Path(".")) / ".devin" / "skills" / "halal-graphify" / "SKILL.md"
        return Path.home() / ".config" / "devin" / "skills" / "halal-graphify" / "SKILL.md"

    if platform_name == "amp":
        if project:
            return (project_dir or Path(".")) / ".agents" / "skills" / "halal-graphify" / "SKILL.md"
        return Path.home() / ".config" / "agents" / "skills" / "halal-graphify" / "SKILL.md"
    ...
    if platform_name in ("antigravity", "antigravity-windows"):
        ...
        return Path.home() / ".gemini" / "config" / "skills" / "halal-graphify" / "SKILL.md"
    ...
    if platform_name in ("claude", "windows") and os.environ.get("CLAUDE_CONFIG_DIR"):
        return Path(os.environ["CLAUDE_CONFIG_DIR"]) / "skills" / "halal-graphify" / "SKILL.md"
...
def _packaged_skill_refs_dir(platform_name: str) -> Path | None:
    ...
    ``halal_graphify/skills/<bundle>/references/``. Reuse keys (e.g. trae-cn) point at
    their twin's bundle.
    ...
    ``halal_graphify/skills/<bundle>/`` is not in this build has not gone progressive
...
def _copy_skill_file(platform_name: str, *, project: bool = False, project_dir: Path | None = None) -> Path:
    ...
    if not skill_src.exists():
        print(f"error: {skill_file} not found in package - reinstall halal-graphify", file=sys.stderr)
        sys.exit(1)
    ...
    if refs_src is not None and not refs_src.exists():
        ...
        print(
            ...
            f"({refs_src}) - reinstall halal-graphify",
            file=sys.stderr,
        )
        sys.exit(1)
...
def _remove_claude_skill_registration(project_dir: Path) -> None:
    ...
    # Match the exact H1 `# halal-graphify` registration heading, never a substring of a
    # user's `## halal-graphify`/`### halal-graphify` (#2062). Section runs to the next H1.
    cleaned = _remove_marker_section(content, "# halal-graphify", boundary_prefix="# ")
    if cleaned is None:
        return
    if cleaned:
        claude_md.write_text(cleaned + "\n", encoding="utf-8")
        print(f"  CLAUDE.md        ->  halal-graphify skill registration removed from {claude_md}")
    else:
        claude_md.unlink()
...
def _claude_pretooluse_hooks(strict: bool = False) -> "list[dict]":
    """halal-graphify's Claude/Codebuddy PreToolUse hooks, resolved at install time.

    The command invokes `halal-graphify hook-guard <search|read>` via the absolute exe
    ...
    "Bash|Grep" and "Read|Glob" and the command always contains "halal-graphify", so the
...
def _skill_registration(skill_path: str = "~/.claude/skills/halal-halal_graphify/SKILL.md") -> str:
    return (
        "\n# halal-graphify\n"
        f"- **halal-graphify** (`{skill_path}`) "
        "- any input to knowledge graph. Trigger: `/halal-graphify`\n"
        "When the user types `/halal-graphify`, use the installed halal-graphify skill "
        "or instructions before doing anything else.\n"
    )
_PLATFORM_CONFIG: dict[str, dict] = {
    "claude": {
        "skill_file": "skill.md",
        "skill_dst": Path(".claude") / "skills" / "halal-graphify" / "SKILL.md",
        "claude_md": True,
        "skill_refs": "claude",
    },
    "codex": {
        "skill_file": "skill-codex.md",
        "skill_dst": Path(".codex") / "skills" / "halal-graphify" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "codex",
    },
    "opencode": {
        "skill_file": "skill-opencode.md",
        "skill_dst": Path(".config") / "opencode" / "skills" / "halal-graphify" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "opencode",
    },
    "kilo": {
        "skill_file": "skill-kilo.md",
        "skill_dst": Path(".config") / "kilo" / "skills" / "halal-graphify" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "kilo",
    },
    "aider": {
        ...
        "skill_dst": Path(".aider") / "halal-graphify" / "SKILL.md",
        "claude_md": False,
    },
    "copilot": {
        "skill_file": "skill-copilot.md",
        "skill_dst": Path(".copilot") / "skills" / "halal-graphify" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "copilot",
    },
    "claw": {
        "skill_file": "skill-claw.md",
        "skill_dst": Path(".openclaw") / "skills" / "halal-graphify" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "claw",
    },
    "droid": {
        "skill_file": "skill-droid.md",
        "skill_dst": Path(".factory") / "skills" / "halal-graphify" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "droid",
    },
    "trae": {
        "skill_file": "skill-trae.md",
        "skill_dst": Path(".trae") / "skills" / "halal-graphify" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "trae",
    },
    "trae-cn": {
        ...
        "skill_file": "skill-trae.md",
        "skill_dst": Path(".trae-cn") / "skills" / "halal-graphify" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "trae",
    },
    "hermes": {
        ...
        "skill_file": "skill-claw.md",
        "skill_dst": Path(".hermes") / "skills" / "halal-graphify" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "claw",
    },
    "kiro": {
        "skill_file": "skill-kiro.md",
        "skill_dst": Path(".kiro") / "skills" / "halal-graphify" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "kiro",
    },
    "pi": {
        "skill_file": "skill-pi.md",
        "skill_dst": Path(".pi") / "agent" / "skills" / "halal-graphify" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "pi",
    },
    "codebuddy": {
        ...
        "skill_file": "skill.md",
        "skill_dst": Path(".codebuddy") / "skills" / "halal-graphify" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "claude",
    },
    "antigravity": {
        ...
        "skill_file": "skill.md",
        "skill_dst": Path(".agents") / "skills" / "halal-graphify" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "claude",
    },
    "antigravity-windows": {
    ...
    "kimi": {
        # Reuses claude's split bundle (shares skill.md).
        "skill_file": "skill.md",
        "skill_dst": Path(".kimi") / "skills" / "halal-graphify" / "SKILL.md",
        "claude_md": False,
        "skill_refs": "claude",
    },
    "amp": {
    ...
    "devin": {
        ...
        # User scope: ~/.config/devin/skills/halal-halal_graphify/SKILL.md
        # Project scope: .devin/skills/halal-halal_graphify/SKILL.md (overridden in _platform_skill_destination)
        "skill_dst": Path(".config") / "devin" / "skills" / "halal-graphify" / "SKILL.md",
        "claude_md": False,
    },
}
...
def _replace_or_append_section(content: str, marker: str, new_section: str) -> str:
    ...
    halal-graphify`` inside a bullet or an inline reference used to anchor the replace
    ...
    headings exist, the last one is used, since halal-graphify's section is appended.
    """
    lines = content.split("\n")
...
def _remove_marker_section(content: str, marker: str, boundary_prefix: str = "## ") -> "str | None":
    ...
    regex ``## halal-graphify`` was unanchored, so it matched inside a user's
    ``### halal-graphify`` heading and deleted hand-written content (#2062) — the same
...
def _print_banner() -> None:
    """Amber brain banner on halal-graphify install. TTY-only, never raises."""
...
def install(platform: str = "claude", *, project: bool = False, project_dir: Path | None = None) -> None:
    ...
    if platform == "kilo":
        # Kilo Code also supports a native /halal-graphify command file.
        command_src = Path(__file__).parent / "command-kilo.md"
        if not command_src.exists():
            print(
                f"error: command-kilo.md not found in package - reinstall halal-graphify",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    if platform == "codebuddy":
        ...
        registration = _skill_registration("~/.codebuddy/skills/halal-halal_graphify/SKILL.md")
...
def _print_install_usage() -> None:
    ...
    print("Usage: halal-graphify install [--project] [--strict] [--platform P|P]")
    print(f"Platforms: {platforms}")
    print("  --strict  block the first raw file read per session until one "
          "`halal-graphify query` runs (Claude Code project hook only; needs --project)")
_CLAUDE_MD_MARKER = "## halal-graphify"
_CODEBUDDY_MD_MARKER = "## halal-graphify"
_AGENTS_MD_MARKER = "## halal-graphify"
_GEMINI_MD_MARKER = "## halal-graphify"
def _gemini_hook() -> dict:
    """Gemini CLI BeforeTool hook, resolved to a shell-agnostic `halal-graphify` call."""
    exe = _resolve_graphify_exe()
    if " " in exe and not exe.startswith('"'):
        exe = f'"{exe}"'
    return {
...
def gemini_install(project_dir: Path | None = None, *, project: bool = False) -> None:
    ...
    if target.exists() and new_content == target.read_text(encoding="utf-8"):
        print(f"halal-graphify already configured in {target.resolve()} (no change)")
    else:
        target.write_text(new_content, encoding="utf-8")
        print(f"halal-graphify section written to {target.resolve()}")
...
def _refuse_to_modify(settings_path: Path) -> "NoReturn":
    ...
    print(
        f"[halal-graphify] refusing to modify {settings_path}: not valid JSON "
...
def _install_gemini_hook(project_dir: Path) -> None:
    ...
    if not isinstance(before_tool, list):
        _refuse_to_modify(settings_path)
    hooks["BeforeTool"] = [
        h for h in before_tool if "halal-graphify" not in str(h)
    ]
...
def _uninstall_gemini_hook(project_dir: Path) -> None:
    ...
    filtered = [h for h in before_tool if "halal-graphify" not in str(h)]
...
def gemini_uninstall(project_dir: Path | None = None, *, project: bool = False, remove_user_skill: bool | None = None) -> None:
    """Remove the halal-graphify section from GEMINI.md, uninstall hook, and remove skill file.
    ...
    if cleaned is None:
        print("halal-graphify section not found in GEMINI.md - nothing to do")
        return
    if cleaned:
        target.write_text(cleaned + "\n", encoding="utf-8")
        print(f"halal-graphify section removed from {target.resolve()}")
    else:
        target.unlink()
        print(f"GEMINI.md was empty after removal - deleted {target.resolve()}")
    _uninstall_gemini_hook(project_dir)
_VSCODE_INSTRUCTIONS_MARKER = "## halal-graphify"
def vscode_install(project_dir: Path | None = None) -> None:
    """Install halal-graphify skill for VS Code Copilot Chat + write .github/copilot-instructions.md."""
    skill_src = Path(__file__).parent / "skill-vscode.md"
    refs_bundle = "vscode"
    if not skill_src.exists():
        skill_src = Path(__file__).parent / "skill-copilot.md"
        refs_bundle = "copilot"
    skill_dst = Path.home() / ".copilot" / "skills" / "halal-graphify" / "SKILL.md"
    skill_dst.parent.mkdir(parents=True, exist_ok=True)
    tmp_dst = skill_dst.with_suffix(skill_dst.suffix + ".tmp")
    try:
        shutil.copy(skill_src, tmp_dst)
    ...
    if instructions.exists():
        ...
        if new_content == content:
            ...
            print(f"  {instructions}  ->  halal-graphify section {'updated' if _VSCODE_INSTRUCTIONS_MARKER in content else 'added'}")
    else:
    ...
    print()
    print(
        "VS Code Copilot Chat configured. Type /halal-graphify in the chat panel to build the graph."
    )
    print("Note: for GitHub Copilot CLI (terminal), use: halal-graphify copilot install")
def vscode_uninstall(project_dir: Path | None = None) -> None:
    """Remove halal-graphify VS Code Copilot Chat skill and .github/copilot-instructions.md section."""
    skill_dst = Path.home() / ".copilot" / "skills" / "halal-graphify" / "SKILL.md"
    if skill_dst.exists():
        skill_dst.unlink()
        print(f"  skill removed    ->  {skill_dst}")
    ...
    if cleaned:
        ...
        print(f"  halal-graphify section removed from {instructions}")
    else:
        instructions.unlink()
...
# Workflow: halal-graphify

Follow the halal-graphify skill to run the full pipeline.

If no path argument is given, use `.` (current directory).
"""
def _kiro_install(project_dir: Path) -> None:
    """Write halal-graphify skill + steering file for Kiro IDE/CLI."""
    project_dir = project_dir or Path(".")
    ...
    print("Use /halal-graphify to build or update the graph.")
def _kiro_uninstall(project_dir: Path) -> None:
    """Remove halal-graphify skill + steering file for Kiro."""
    project_dir = project_dir or Path(".")
    removed = []
...
def _antigravity_finalize(skill_dst: Path, project_dir: Path) -> None:
    ...
    if rules_path.exists():
        ...
        if _always_on("antigravity-rules").strip() != existing.strip():
            ...
            print(f"halal-graphify rule updated at {rules_path.resolve()}")
        else:
            print(f"halal-graphify rule already configured at {rules_path.resolve()} (no change)")
    else:
        rules_path.write_text(_always_on("antigravity-rules"), encoding="utf-8")
        print(f"halal-graphify rule written to {rules_path.resolve()}")
    ...
    if wf_path.exists():
        ...
        if _ANTIGRAVITY_WORKFLOW.strip() != existing.strip():
            ...
            print(f"halal-graphify workflow updated at {wf_path.resolve()}")
        else:
            print(f"halal-graphify workflow already configured at {wf_path.resolve()} (no change)")
    else:
        wf_path.write_text(_ANTIGRAVITY_WORKFLOW, encoding="utf-8")
        print(f"halal-graphify workflow written to {wf_path.resolve()}")
def _antigravity_install(project_dir: Path) -> None:
    """Install halal-graphify for Google Antigravity (global skill + .agents/rules + .agents/workflows)."""
    # Copy the skill to ~/.gemini/config/skills/halal-halal_graphify/SKILL.md (global), then
    ...
    print("codebase questions. Run /halal-graphify first to build the graph.")
    print()
    print(
    ...
    print('    "command": "uv",')
    print(
        '    "args": ["run", "--with", "halal-graphify", "--with", "mcp", "-m", "halal_graphify.serve", "${workspace.path}/graphify-out/graph.json"]'
    )
    print("  }")
def _antigravity_uninstall(project_dir: Path, *, project: bool = False) -> None:
    """Remove halal-graphify Antigravity rules, workflow, and skill files."""
    # Remove rules file
    rules_path = project_dir / _ANTIGRAVITY_RULES_PATH
    if rules_path.exists():
        rules_path.unlink()
        print(f"halal-graphify rule removed from {rules_path.resolve()}")
    else:
        print("No halal-graphify Antigravity rule found - nothing to do")

    # Remove workflow file
    wf_path = project_dir / _ANTIGRAVITY_WORKFLOW_PATH
    if wf_path.exists():
        wf_path.unlink()
        print(f"halal-graphify workflow removed from {wf_path.resolve()}")

    # Remove skill file
    skill_dst = _platform_skill_destination("antigravity", project=project, project_dir=project_dir)
    if skill_dst.exists():
        skill_dst.unlink()
        print(f"halal-graphify skill removed from {skill_dst}")
    version_file = skill_dst.parent / ".graphify_version"
    if version_file.exists():
        version_file.unlink()
    refs_dir = skill_dst.parent / "references"
    ...
    for d in (
        ...
        try:
            ...
            break
_CURSOR_RULE_PATH = Path(".cursor") / "rules" / "halal-graphify.mdc"
_CURSOR_RULE = """\
---
...
**MANDATORY: Before using Read, Grep, Glob, or Bash to explore the codebase, you MUST run halal-graphify first:**
- `halal-graphify query "<question>"` — scoped subgraph for any codebase or architecture question
- `halal-graphify path "<A>" "<B>"` — dependency path between two symbols
- `halal-graphify explain "<concept>"` — all nodes related to a concept

This applies to YOU and to every subagent you spawn. Include this rule explicitly in every subagent prompt that involves code exploration. Do not skip halal-graphify because files are "already known" or because you are executing a plan — the graph surfaces cross-file dependencies and INFERRED edges that grep and Read cannot find.

Only use Read/Grep/Glob directly when:
1. halal-graphify has already oriented you and you need to modify or debug specific lines
...
- After modifying code files, run `halal-graphify update .` to keep the graph current (AST-only, no API cost)
"""
def _cursor_install(project_dir: Path) -> None:
    """Write .cursor/rules/halal-graphify.mdc with alwaysApply: true."""
    ...
    if rule_path.exists() and rule_path.read_text(encoding="utf-8") == _CURSOR_RULE:
        print(f"halal-graphify rule at {rule_path} already configured (no change)")
        return
    # File is wholly graphify-owned. Overwrite on upgrade so older
    # report-first wording does not silently linger (issue #580).
    action = "updated" if rule_path.exists() else "written"
    rule_path.write_text(_CURSOR_RULE, encoding="utf-8")
    print(f"halal-graphify rule {action} at {rule_path.resolve()}")
    print()
    print("Cursor will now always include the knowledge graph context.")
    print("Run /halal-graphify . first to build the graph if you haven't already.")
def _cursor_uninstall(project_dir: Path) -> None:
    """Remove .cursor/rules/halal-graphify.mdc."""
    rule_path = (project_dir or Path(".")) / _CURSOR_RULE_PATH
    if not rule_path.exists():
        print("No halal-graphify Cursor rule found - nothing to do")
        return
    rule_path.unlink()
    print(f"halal-graphify Cursor rule removed from {rule_path.resolve()}")
...
Rules:
- For codebase or architecture questions, when `graphify-out/graph.json` exists, first run `halal-graphify query "<question>"` (or `halal-graphify path "<A>" "<B>"` / `halal-graphify explain "<concept>"`). These return a scoped subgraph, usually much smaller than `GRAPH_REPORT.md` or raw grep output.
- If graphify-out/wiki/index.md exists, navigate it instead of reading raw files
  ...
  return {
    "tool.execute.before": async (input, output) => {
      ...
      if (input.tool === "bash") {
        ...
        output.args.command =
          'echo "[halal-graphify] Knowledge graph available. Read graphify-out/GRAPH_REPORT.md for hub nodes and architecture context before searching files." ; ' +
          output.args.command;
        reminded = true;
      }
    },
...
_OPENCODE_PLUGIN_JS = """\
// halal-graphify OpenCode plugin
// Injects a knowledge graph reminder before bash tool calls when the graph exists.
//
// IMPORTANT: keep the reminder string free of backticks and $(...) constructs.
// The hook prepends `echo "<reminder>" && <cmd>` to the user's bash command;
// backticks inside the double-quoted echo trigger bash command substitution,
// which both corrupts tool output and silently executes the very halal-graphify
  ...
  return {
    "tool.execute.before": async (input, output) => {
      ...
      if (input.tool === "bash") {
        ...
        output.args.command =
          'echo "[halal-graphify] knowledge graph at graphify-out/. For focused questions, run halal-graphify query with your question (scoped subgraph, usually much smaller than GRAPH_REPORT.md) instead of grepping raw files. Read GRAPH_REPORT.md only for broad architecture context." ; ' +
          output.args.command;
        reminded = true;
      }
    },
...
def _resolve_graphify_exe() -> str:
    """Return the absolute path to the halal-graphify executable, with forward slashes.

    Falls back to bare 'halal-graphify' if resolution fails. Using an absolute path
    ...
    character: a raw ``C:\\Users\\me\\halal-graphify.EXE`` collapses to
    ...
    """
    import shutil
    found = shutil.which("halal-graphify")
    if not found:
        # Derive from sys.executable: same Scripts/ (Windows) or bin/ (Unix) dir
        scripts_dir = Path(sys.executable).parent
        for name in ("halal-graphify.exe", "halal-graphify"):
            candidate = scripts_dir / name
            if candidate.exists():
                found = str(candidate)
                break
    return (found or "halal-graphify").replace("\\", "/")
def _install_codex_hook(project_dir: Path) -> None:
    """Add halal-graphify PreToolUse hook to .codex/hooks.json."""
    ...
    if not isinstance(pre_tool, list):
        _refuse_to_modify(hooks_path)
    hooks["PreToolUse"] = [h for h in pre_tool if "halal-graphify" not in str(h)]
...
def _uninstall_codex_hook(project_dir: Path) -> None:
    """Remove halal-graphify PreToolUse hook from .codex/hooks.json."""
    hooks_path = project_dir / ".codex" / "hooks.json"
    ...
    filtered = [h for h in pre_tool if "halal-graphify" not in str(h)]
    existing["hooks"]["PreToolUse"] = filtered
    hooks_path.write_text(json.dumps(existing, indent=2), encoding="utf-8")
    print(f"  .codex/hooks.json  ->  PreToolUse hook removed")
def _agents_install(project_dir: Path, platform: str) -> None:
    """Write the halal-graphify section to the local AGENTS.md for always-on platforms."""
...
def _amp_legacy_cleanup() -> None:
    """Best-effort removal of the pre-fix ~/.amp/skills/halal-graphify install dir.

    Older halal-graphify versions wrote the Amp skill to ~/.amp/skills, which Amp does
    ...
    """
    legacy = Path.home() / ".amp" / "skills" / "halal-graphify"
    if legacy.exists():
...
def _agents_platform_install(project_dir: Path | None = None) -> None:
    """`halal-graphify agents install`: skill into ~/.agents/skills + AGENTS.md.
    ...
    rendered hooks reference, which points at `halal-graphify agents install`. The bare
    `halal-graphify install --platform agents` path stays skill-only (via install()),
...
def _agents_platform_uninstall(project_dir: Path | None = None) -> None:
    """`halal-graphify agents uninstall`: remove the skill and the AGENTS.md section."""
...
def _project_install(platform_name: str, project_dir: Path | None = None, strict: bool = False) -> None:
    ...
    if platform_name in ("claude", "windows"):
        ...
        # root. `agents` -> ./.agents/skills/halal-halal_graphify/SKILL.md.
        skill_dst = _copy_skill_file(platform_name, project=True, project_dir=project_dir)
        _print_project_git_add_hint([_project_scope_root(skill_dst, project_dir)])
    else:
        install(platform=platform_name, project=True, project_dir=project_dir)
...
def _project_uninstall_all(project_dir: Path | None = None) -> None:
    ...
    project_dir = project_dir or Path(".")
    print("Uninstalling project-scoped halal-graphify files...\n")
...
def _agents_uninstall(project_dir: Path, platform: str = "") -> None:
    """Remove the halal-graphify section from the local AGENTS.md."""
    target = (project_dir or Path(".")) / "AGENTS.md"
    ...
    if cleaned is None:
        print("halal-graphify section not found in AGENTS.md - nothing to do")
        if platform == "opencode":
            _uninstall_opencode_plugin(project_dir or Path("."))
        elif platform == "kilo":
            _uninstall_kilo_plugin(project_dir or Path("."))
...
def claude_install(project_dir: Path | None = None, strict: bool = False) -> None:
    """Write the halal-graphify section to the local CLAUDE.md."""
    ...
    if strict:
        ...
        print("one `halal-graphify query` runs (toggle with GRAPHIFY_HOOK_STRICT=0).")
def _install_claude_hook(project_dir: Path, strict: bool = False) -> None:
    """Add halal-graphify PreToolUse hook to .claude/settings.json."""
    ...
    hooks["PreToolUse"] = [h for h in pre_tool if not (isinstance(h, dict) and h.get("matcher") in ("Glob|Grep", "Bash", "Bash|Grep", "Read|Glob") and "halal-graphify" in str(h))]
...
def _uninstall_claude_hook(project_dir: Path) -> None:
    """Remove the halal-graphify PreToolUse hook from .claude/settings.json and its
...
def _strip_graphify_hook(settings_path: Path) -> None:
    """Drop halal-graphify PreToolUse hooks from a single Claude settings file, if present."""
    if not settings_path.exists():
        return
    try:
        settings = json.loads(settings_path.read_text(encoding="utf-8"))
    ...
    filtered = [h for h in pre_tool if not (h.get("matcher") in ("Glob|Grep", "Bash", "Bash|Grep", "Read|Glob") and "halal-graphify" in str(h))]
...
def uninstall_all(project_dir: Path | None = None, purge: bool = False) -> None:
    """Remove halal-graphify from every platform detected in the current project."""
    pd = project_dir or Path(".")
    print("Uninstalling halal-graphify from all detected platforms...\n")

    # Skill-file / config-section uninstallers. remove_user_skill=True keeps the
    # historical `halal-graphify uninstall` behavior: global skill delete plus md/hook
    ...
    # Git hook
    try:
        from halal_graphify.hooks import uninstall as hook_uninstall
    ...
    print("\nDone. Run 'pip uninstall halal-graphify' to remove the package itself.")
def claude_uninstall(project_dir: Path | None = None, *, project: bool = False, remove_user_skill: bool | None = None) -> None:
    """Remove the halal-graphify skill tree (SKILL.md + references/) and the halal-graphify
    section from CLAUDE.md and its local-only variants, plus the PreToolUse hook.

    Mirrors gemini_uninstall: the bare `halal-graphify uninstall` and `halal-graphify claude
    ...
    if not existing:
        ...
        print("halal-graphify section not found in CLAUDE.md - nothing to do")

    _uninstall_claude_hook(project_dir)
def _strip_graphify_md_section(target: Path) -> bool:
    """Strip the ## halal-graphify section from one CLAUDE.md-style file.
    ...
    try:
        ...
        return False
    # Remove halal-graphify's ## halal-graphify section (heading matched exactly, never as a
    # substring of a user's ### halal-graphify) from the marker to the next H2 or EOF.
...
def codebuddy_install(project_dir: Path | None = None) -> None:
    """Install the halal-graphify skill and CODEBUDDY.md section for CodeBuddy."""
...
def _install_codebuddy_hook(project_dir: Path) -> None:
    """Add halal-graphify PreToolUse hook to .codebuddy/settings.json."""
...
def _uninstall_codebuddy_hook(project_dir: Path) -> None:
    """Remove halal-graphify PreToolUse hook from .codebuddy/settings.json."""
    settings_path = project_dir / ".codebuddy" / "settings.json"
    if not settings_path.exists():
        return
    try:
...
def codebuddy_uninstall(project_dir: Path | None = None, *, project: bool = False, remove_user_skill: bool | None = None) -> None:
    """Remove the halal-graphify skill tree (SKILL.md + references/) and the CODEBUDDY.md section.
    ...
    if cleaned is None:
        print("halal-graphify section not found in CODEBUDDY.md - nothing to do")
        return
...
def dispatch_install_cli(cmd: str) -> bool:
    """Handle `halal-graphify <install-command>` dispatch (install/uninstall + every
    ...
    if cmd == "install":
        ...
        if project_scope:
            ...
            if strict:
                print(
                    ...
                    "`halal-graphify install --project --strict` or `halal-graphify claude install --strict`.",
                    file=sys.stderr,
                )
        ...
        if subcmd == "install":
            ...
            print("Usage: halal-graphify claude [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "codebuddy":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: halal-graphify codebuddy [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "gemini":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: halal-graphify gemini [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "cursor":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: halal-graphify cursor [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "vscode":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: halal-graphify vscode [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "copilot":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: halal-graphify copilot [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "kilo":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: halal-graphify kilo [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "kiro":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: halal-graphify kiro [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "devin":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: halal-graphify devin [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "pi":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: halal-graphify pi [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    elif cmd == "amp":
        subcmd = sys.argv[2] if len(sys.argv) > 2 else ""
        if subcmd == "install":
            ...
            print("Usage: halal-graphify amp [install|uninstall]", file=sys.stderr)
            sys.exit(1)
        ...
        if subcmd == "install":
            ...
            print(f"Usage: halal-graphify {cmd} [install|uninstall]", file=sys.stderr)
            sys.exit(1)
        ...
        if subcmd == "install":
            ...
            print("Usage: halal-graphify antigravity [install|uninstall]", file=sys.stderr)
            sys.exit(1)
    return True
