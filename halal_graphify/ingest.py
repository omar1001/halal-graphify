...
from halal_graphify.security import safe_fetch, safe_fetch_text, validate_url
...
def ingest(url: str, target_dir: Path, author: str | None = None, contributor: str | None = None) -> Path:
    ...
    try:
        ...
        if url_type == "youtube":
            from halal_graphify.transcribe import download_audio
...
def save_query_result(
    ...
    that halal-graphify's extractor reads as node metadata. This closes the feedback loop:
    ...
    signals: they are written both to the frontmatter (so `halal-graphify reflect` can
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Fetch a URL into a halal-graphify /raw folder")
    ...
    print(f"Ready for halal-graphify: {out}")
