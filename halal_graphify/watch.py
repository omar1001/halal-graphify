...
# Single source of truth in halal_graphify.paths (#1423); re-exported as _GRAPHIFY_OUT.
from halal_graphify.paths import GRAPHIFY_OUT as _GRAPHIFY_OUT, is_absolute_any_platform
...
def _git_head(cwd: Path | str | None = None) -> str | None:
    ...
    inherits the caller's working directory, so `halal-graphify update <target>`
...
def _reconcile_existing_graph(
    ...
    *,
    ...
    ``halal-graphify update``) so the incremental/hook path keeps preserving a
    ...
    from halal_graphify.build import _load_existing_graph
    ...
    from halal_graphify.build import _is_ast_tier
    ...
    try:
        from halal_graphify.build import _norm_source_file as _nsf
        ...
        if newly_ignored_files:
            print(
                f"[halal-graphify watch] pruned {newly_ignored_nodes} node(s) from "
        ...
        if excluded_alive_files:
            print(
                f"[halal-graphify watch] fail-closed: kept {excluded_alive_nodes} node(s) "
        ...
        print(
            "[halal-graphify watch] reconcile of existing graph failed "
...
def _node_community_map(graph_data: dict) -> dict[str, int]:
    ...
    for node in graph_data.get("nodes", []):
        ...
        try:
            ...
            print(
                f"[halal-graphify watch] Skipping node with invalid community id: "
...
def _check_shrink(
    ...
    *,
    ...
    plain ``halal-graphify update`` after deleting a function refresh the graph without
    ...
    print(
        f"[halal-graphify] WARNING: new graph has {len(new_nodes)} nodes but existing "
...
def _stabilize_rebuild_cwd(watch_path: Path) -> bool:
    ...
    try:
        ...
        print(
            "[halal-graphify watch] Rebuild failed: current working directory "
...
def _reconcile_graph_html(out: Path, graph_data: dict) -> str | None:
    ...
    from halal_graphify.exporters.html import _HTML_STALE_MARKER, _viz_node_limit
    ...
    def clear_stale_marker() -> None:
        try:
            ...
            print(
                "[halal-graphify watch] graph.html stale marker could not be cleared; "
    ...
    try:
        from halal_graphify.export import to_html
        from halal_graphify.paths import load_node_link_graph
        ...
        if had_html:
            print(
                "[halal-graphify watch] Stale graph.html left unchanged; "
                f"regeneration will be retried: {exc}"
            )
        else:
            print(f"[halal-graphify watch] Missing graph.html could not be regenerated: {exc}")
        return None
    ...
    if not written or not html_target.exists():
        if had_html:
            ...
            print("[halal-graphify watch] Missing graph.html has no useful community view; skipped.")
        return None
    clear_stale_marker()
    return "rendered"
    ...
    *,
    ...
    if acquire_lock:
        ...
        with _rebuild_lock(out, blocking=block_on_lock) as got:
            if not got:
                print("[halal-graphify watch] Rebuild already in progress for "
    ...
    try:
        from halal_graphify.extract import extract, _get_extractor
        from halal_graphify.detect import detect
        from halal_graphify.build import build_from_json, _is_ast_tier, _norm_source_file as _nsf
        from halal_graphify.cluster import cluster, remap_communities_to_previous, score_all
        ...
        from halal_graphify.detect import ignored_predicate
        ...
        if not code_files and not existing_graph.exists():
            print("[halal-graphify watch] No code files found - nothing to rebuild.")
            return False
        ...
        if changed_paths is not None:
            ...
            if not wanted and not deleted_paths:
                print("[halal-graphify watch] No tracked code files in change set - skipping rebuild.")
                return True
        ...
        if no_cluster:
            ...
            candidate_graph_data = {
                **{k: v for k, v in result.items() if k not in ("edges", "nodes")},
                ...
                # `halal-graphify update --no-cluster` can't silently drop it -
            ...
            try:
                from halal_graphify.detect import save_manifest
            ...
            if same_graph:
                print("[halal-graphify watch] No code-graph changes detected (--no-cluster); outputs left untouched.")
            else:
                print(
                    "[halal-graphify watch] Rebuilt (no clustering): "
                    f"{len(candidate_graph_data.get('nodes', []))} nodes, "
                    f"{len(candidate_graph_data.get('links', []))} edges"
                )
                print(f"[halal-graphify watch] graph.json updated in {out}")
            return True
        ...
        # Inherit the existing graph's directed flag (#2342) so `halal-graphify
        ...
        if existing_graph_data:
            ...
            if same_topology:
                ...
                if html_action == "rendered":
                    print(
                        "[halal-graphify watch] No code-graph topology changes detected; "
                    ...
                    print("[halal-graphify watch] No code-graph topology changes detected; outputs left untouched.")
                return True
        ...
        # Without this, an incremental `halal-graphify update` launders stale names into
        # labels.json as though they were current (#label-stale).
        from halal_graphify.cluster import community_member_sigs
        ...
        if stale:
            print(
                f"[halal-graphify watch] community set changed since labeling "
        ...
        report = generate(G, communities, cohesion, labels, hubs, surprises, detection,
        ...
        if no_change:
            graph_tmp.unlink(missing_ok=True)
            print("[halal-graphify watch] No code-graph changes detected; graph.json/GRAPH_REPORT.md left untouched.")
        else:
            if not _check_shrink(
                force, existing_graph_data, candidate_graph_data,
                tmp=graph_tmp,
        ...
        if callflow_files and not no_change:
            try:
                from halal_graphify.callflow_html import write_callflow_html
                ...
                print(f"[halal-graphify watch] callflow HTML update skipped: {cf_err}")
        ...
        if not no_change:
            print(f"[halal-graphify watch] Rebuilt: {G.number_of_nodes()} nodes, "
            ...
            print(f"[halal-graphify watch] {products} updated in {out}")
        return True

    except Exception as exc:
        print(f"[halal-graphify watch] Rebuild failed: {exc}")
        return False
...
def check_update(watch_path: Path) -> bool:
    ...
    re-extraction via `/halal-graphify --update` — this function only signals
    that the update is needed.
    """
    flag = Path(watch_path) / _GRAPHIFY_OUT / "needs_update"
    if flag.exists():
        print(f"[halal-graphify check-update] Pending non-code changes in {watch_path}.")
        print("[halal-graphify check-update] Run `/halal-graphify --update` to apply semantic re-extraction.")
    return True
...
def _notify_only(watch_path: Path) -> None:
    ...
    print(f"\n[halal-graphify watch] New or changed files detected in {watch_path}")
    print("[halal-graphify watch] Non-code files changed - semantic re-extraction requires LLM.")
    print("[halal-graphify watch] Run `/halal-graphify --update` in Claude Code to update the graph.")
    print(f"[halal-graphify watch] Flag written to {flag}")
...
def _batch_triggers_rebuild(batch: list[Path]) -> bool:
    ...
    until the next code event or a manual `halal-graphify update` (#2580).
    """
...
def watch(watch_path: Path, debounce: float = 3.0) -> None:
    ...
    to run /halal-graphify --update (LLM extraction required).
    ...
    print(f"[halal-graphify watch] Watching {watch_path.resolve()} - press Ctrl+C to stop")
    print(f"[halal-graphify watch] Code changes rebuild graph automatically. "
          f"Doc/image changes require /halal-graphify --update.")
    print(f"[halal-graphify watch] Debounce: {debounce}s")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Watch a folder and auto-update the halal-graphify graph")
