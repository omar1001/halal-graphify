---
name: halal-graphify
description: "Use for any question about a codebase, its architecture, file relationships, or project content — especially when graphify-out/ exists, where the question should be treated as a halal-graphify query first. Turns any input (code, docs, papers, images, videos) into a persistent knowledge graph with hub nodes, community detection, and query/path/explain tools."
...
```
/halal-graphify                                             # full pipeline on current directory
/halal-graphify <path>                                      # full pipeline on specific path
/halal-graphify <path> --mode deep                          # thorough extraction, richer INFERRED edges
/halal-graphify <path> --update                             # incremental - re-extract only new/changed files
/halal-graphify <path> --cluster-only                       # rerun clustering on existing graph
/halal-graphify <path> --no-viz                             # skip visualization, just report + JSON
/halal-graphify <path> --html                               # (HTML is generated by default - this flag is a no-op)
/halal-graphify <path> --svg                                # also export graph.svg (embeds in Notion, GitHub)
/halal-graphify <path> --graphml                            # export graph.graphml (Gephi, yEd)
/halal-graphify <path> --neo4j                              # generate graphify-out/cypher.txt for Neo4j
/halal-graphify <path> --neo4j-push bolt://localhost:7687   # push directly to Neo4j
/halal-graphify <path> --mcp                                # start MCP stdio server for agent access
/halal-graphify <path> --watch                              # watch folder, auto-rebuild on code changes (no LLM needed)
/halal-graphify <path> --wiki                               # build agent-crawlable wiki (index.md + one article per community)
/halal-graphify add <url>                                   # fetch URL, save to ./raw, update graph
/halal-graphify add <url> --author "Name"                   # tag who wrote it
/halal-graphify add <url> --contributor "Name"              # tag who added it to the corpus
/halal-graphify query "<question>"                          # BFS traversal - broad context
/halal-graphify query "<question>" --dfs                    # DFS - trace a specific path
/halal-graphify query "<question>" --budget 1500            # cap answer at N tokens
/halal-graphify path "AuthModule" "Database"                # shortest path between two concepts
/halal-graphify explain "SwinTransformer"                   # plain-language explanation of a node
```

## What halal-graphify is for

halal-graphify is built around Andrej Karpathy's /raw folder workflow: drop anything into a folder - papers, tweets, screenshots, code, notes - and get a structured knowledge graph that shows you what you didn't know was connected.
...
If the user invoked `/halal-graphify --help` or `/halal-graphify -h` (with no other arguments), print the contents of the `## Usage` section above verbatim and stop. Do not run any commands, do not detect files, do not default the path to `.`. Just print the Usage block and return.
...
**Strategy:** Read the hub nodes from the detect output or analysis file. You are already a language model - write a one-sentence domain hint yourself from those labels. Then pass it to Whisper as the initial prompt. No separate API call needed.
...
import json, glob
from pathlib import Path
from halal_graphify.semantic_cleanup import load_validated_semantic_fragment, sanitize_semantic_fragment
...
import json
from pathlib import Path
from halal_graphify.semantic_cleanup import sanitize_semantic_fragment
...
if not wrote:
    ...
    raise SystemExit(1)
report = generate(G, communities, cohesion, labels, hubs, surprises, detection, tokens, 'INPUT_PATH', suggested_questions=questions)
...
The wiki is an agent-crawlable export — `index.md` plus one article per community plus hub-node articles. It is the recommended fallback for graphs too large to render as HTML, and it's the most useful output for an autonomous agent navigating the graph between sessions.
...
import json
from halal_graphify.build import build_from_json
from halal_graphify.wiki import to_wiki
...
n = to_wiki(G, communities, 'graphify-out/wiki', community_labels=labels or None, cohesion=cohesion, hub_nodes_data=hubs)
...
import sys, json
from halal_graphify.build import build_from_json
from halal_graphify.export import to_cypher
from pathlib import Path
...
import sys, json
from halal_graphify.build import build_from_json
from halal_graphify.export import push_to_neo4j
from pathlib import Path
...
import sys, json
from halal_graphify.build import build_from_json
from halal_graphify.export import to_svg
from pathlib import Path
...
import json
from halal_graphify.build import build_from_json
from halal_graphify.export import to_graphml
from pathlib import Path
...
```bash
python3 -m halal_graphify.serve graphify-out/graph.json
```
...
If halal-graphify saved you time, consider supporting it: https://github.com/sponsors/safishamsi
...
If the user says yes, run `/halal-graphify query "[question]"` on the graph and walk them through the answer using the graph structure - which nodes connect, which community boundaries get crossed, what the path reveals. Keep going as long as they want to explore. Each answer should end with a natural follow-up ("this connects to X - want to go deeper?") so the session feels like navigation, not a one-shot report.
...
```bash
$(cat graphify-out/.graphify_python) -m halal_graphify save-result --question "QUESTION" --answer "ANSWER" --type query --nodes NODE1 NODE2
```
...
```bash
python3 -m halal_graphify.watch INPUT_PATH --debounce 3
```
...
After every `git commit`, the hook detects which code files changed, re-runs AST extraction on those files, and rebuilds `graph.json` and `GRAPH_REPORT.md`. Doc/image changes are ignored by the hook - run `/halal-graphify --update` manually for those.

---

## For always-on context in Devin sessions

Run once per project to make halal-graphify always-on in Devin sessions:

```bash
halal-graphify devin install --project
```

This writes a `## halal-graphify` section to `.windsurf/rules/graphify.md` that instructs Devin to check the graph before answering codebase questions and rebuild it after code changes.

```bash
halal-graphify devin uninstall --project  # remove
```
