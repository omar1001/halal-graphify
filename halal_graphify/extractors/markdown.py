"""Markdown extractor. Moved verbatim from halal_graphify/extract.py."""
...
# not a declared dependency of halal-graphify (see manifest_ingest._parse_apm for the
...
def _active_scan_root() -> "Path | None":
    ...
    fallback stays off.
    """
    import halal_graphify.extract as _extract
...
def _build_link_index(root: Path) -> "dict[str, list[tuple[int, str, Path]]]":
    ...
    index dot-folders.
    """
    from halal_graphify.detect import _SKIP_DIRS
