...
`halal_graphify/extract.py` is being split into this package, one language per PR
...
3. **Facade re-export is mandatory.** `extract.py` must keep exporting every
   moved name (`from halal_graphify.extractors.<mod> import extract_<lang>  # noqa: F401`
...
4. **Never import from `halal_graphify.extract` inside this package.** Import
...
- run `grep -c '_name' halal_graphify/extract.py` AFTER your candidate move;
...
2. `grep -n 'def extract_<lang>' halal_graphify/extract.py`; the span ends at the
...
3. Save the span to a temp file. Create `halal_graphify/extractors/<lang>.py` with
   module docstring (`"""<Lang> extractor. Moved verbatim from halal_graphify/extract.py."""`),
