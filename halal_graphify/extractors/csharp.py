...
still lives in ``halal_graphify/extract.py``; per ``extractors/MIGRATION.md`` the
...
from halal_graphify.extractors.base import _make_id
...
class CsharpNameResolver:
    ...
    (``_resolve_csharp_member_calls`` in ``halal_graphify/extract.py``) can type a
