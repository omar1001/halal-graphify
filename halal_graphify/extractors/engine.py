"""engine — moved verbatim from halal_graphify/extract.py."""
from __future__ import annotations

import hashlib
import importlib
from halal_graphify.extractors.base import _LANGUAGE_BUILTIN_GLOBALS, _file_stem, _make_id, _read_text
from halal_graphify.ids import normalize_id
from halal_graphify.extractors.models import LanguageConfig
from halal_graphify.extractors.resolution import _resolve_js_import_target
...
def _js_import_binds_external(raw: str, str_path: str) -> bool:
    ...
    Reuses `_resolve_js_import_target`, so this is halal-graphify's own verdict rather
...
def _js_member_assignment_target(left, source: bytes):
    ...
    check at the caller avoids the bare-named / phantom-hub-node failure mode
...
def _js_extra_walk(node, source: bytes, file_nid: str, stem: str, str_path: str,
    ...
    if node.type in ("lexical_declaration", "variable_declaration"):
        ...
        # phantom hub-nodes. Bodies of arrow functions are walked separately
...
def _extract_generic(
    ...
    def _emit_indirect_by_name(ident_name: str, loc_node, scope_nid: str,
        ...
        cross-file resolver, which applies the single-definition hub-node guard and the
        ...
        # single-definition hub-node guard plus the GLOBAL callable-target check, so a
    ...
    def walk_calls(
        ...
        if node.type in config.call_types:
            ...
            if config.ts_module == "tree_sitter_swift":
                ...
                if recv is not None:
                    ...
                    if recv.type in ("identifier", "constant"):
                        ...
                        # bare class name (the hub-node guard bails if ambiguous).
            ...
            if callee_name and callee_name not in _LANGUAGE_BUILTIN_GLOBALS:
                ...
                # inflate it into a hub node (#2417). Qualified class/module calls are
