# halal-graphify reference: incremental update and cluster-only
...
import sys, json
from halal_graphify.detect import detect_incremental, save_manifest
...
If `code_only` is True: print `[halal-graphify update] Code-only changes detected - skipping semantic extraction (no LLM needed)`, run only Step 3A (AST) on the changed files, skip Step 3B entirely (no subagents), then go straight to merge and Steps 4–8.
...
if [ ! -f graphify-out/.graphify_extract.json ]; then
    echo '[halal-graphify update] Only deletions -- creating empty extraction for merge.'
    $(cat graphify-out/.graphify_python) -c "
import json
from pathlib import Path
...
import json
from pathlib import Path
from halal_graphify.build import build_merge
...
print(f'[halal-graphify update] Merged: {G.number_of_nodes()} nodes, {G.number_of_edges()} edges')
...
print(f'[halal-graphify update] Merged extraction written ({len(merged_out[\"nodes\"])} nodes, {len(merged_out[\"edges\"])} edges)')
...
print('[halal-graphify update] Manifest saved.')
"
```
...
$(cat graphify-out/.graphify_python) -c "
import json
from halal_graphify.analyze import graph_diff
