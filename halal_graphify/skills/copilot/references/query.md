# halal-graphify reference: query, path, explain

Load this when the user asks a question against an existing graph, or runs `/halal-graphify path` or `/halal-graphify explain`. The core's query stub points here for the full traversal flow. These flows use the `halal-graphify query` CLI when it is available and fall back to an inline NetworkX traversal otherwise.
...
```bash
halal-graphify query "QUESTION"
# or: halal-graphify query "QUESTION" --dfs --budget 3000
```
...
```bash
$(cat graphify-out/.graphify_python) -m halal_graphify save-result --question "ORIGINAL_QUESTION" --answer "ANSWER" --type query --nodes NODE1 NODE2
```
...
```bash
$(cat graphify-out/.graphify_python) -m halal_graphify save-result --question "Path from NODE_A to NODE_B" --answer "ANSWER" --type path_query --nodes NODE_A NODE_B
```

---

## For /halal-graphify explain

Give a plain-language explanation of a single node - everything connected to it. Prefer the CLI when installed:

```bash
halal-graphify explain "NODE_NAME"
```
...
```bash
$(cat graphify-out/.graphify_python) -m halal_graphify save-result --question "Explain NODE_NAME" --answer "ANSWER" --type explain --nodes NODE_NAME
```
