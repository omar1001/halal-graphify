...
from halal_graphify.security import sanitize_label, check_graph_file_size_cap
from halal_graphify.build import edge_data, edge_datas
from halal_graphify.paths import default_graph_json as _default_graph_json
...
def _load_graph(graph_path: str) -> nx.Graph:
    try:
        ...
        try:
            ...
            if _legacy(data.get("nodes", [])):
                print(
                    ...
                    "rebuild with `halal-graphify extract --force` for path-qualified IDs.",
                    file=sys.stderr,
                )
        except Exception:
            pass
        ...
        print(f"error: graph.json is corrupted ({exc}). Re-run /halal-graphify to rebuild.", file=sys.stderr)
        sys.exit(1)
...
def _build_server(graph_path: str):
    ...
    try:
        ...
        raise ImportError('mcp not installed. Run: pip install "halal-graphify[mcp]"') from e
    try:
    ...
    from halal_graphify import paths as _paths
    ...
    def _tool_hub_nodes(arguments: dict) -> str:
        from halal_graphify.analyze import hub_nodes as _hub_nodes
        nodes = _hub_nodes(G, top_n=int(arguments.get("top_n", 10)))
    ...
    def _tool_list_prs(arguments: dict) -> str:
        from halal_graphify.prs import fetch_prs, fetch_worktrees, format_prs_text, _detect_default_branch
    ...
    def _tool_get_pr_impact(arguments: dict) -> str:
        from halal_graphify.prs import fetch_pr_files, compute_pr_impact, _gh, _parse_ci
    ...
    def _tool_triage_prs(arguments: dict) -> str:
        ...
        from halal_graphify.prs import fetch_prs, fetch_worktrees, fetch_pr_files, compute_pr_impact, _STATUS_ORDER, _detect_default_branch
        repo = arguments.get("repo") or None
        base = arguments.get("base") or _detect_default_branch(repo)
        try:
            prs = fetch_prs(repo=repo, base=base)
    ...
    async def list_resources() -> list[types.Resource]:
        ...
        return [
            types.Resource(uri="halal-graphify://report", name="Graph Report", description="Full GRAPH_REPORT.md", mimeType="text/markdown"),
            types.Resource(uri="halal-graphify://stats", name="Graph Stats", description="Node/edge/community counts and confidence breakdown", mimeType="text/plain"),
            types.Resource(uri="halal-graphify://hub-nodes", name="Hub Nodes", description="Top 10 most-connected nodes", mimeType="text/plain"),
            types.Resource(uri="halal-graphify://surprises", name="Surprising Connections", description="Cross-community surprising connections", mimeType="text/plain"),
            types.Resource(uri="halal-graphify://audit", name="Confidence Audit", description="EXTRACTED/INFERRED/AMBIGUOUS edge breakdown", mimeType="text/plain"),
            types.Resource(uri="halal-graphify://questions", name="Suggested Questions", description="Suggested questions for this codebase", mimeType="text/plain"),
        ]

    async def read_resource(uri: AnyUrl) -> str:
        _select_graph(None)  # resources read the server's default graph
        uri_str = str(uri)
        if uri_str == "halal-graphify://report":
            report_path = Path(active_graph_path).parent / "GRAPH_REPORT.md"
            if report_path.exists():
                return report_path.read_text(encoding="utf-8")
            return "GRAPH_REPORT.md not found. Run halal-graphify extract first."
        if uri_str == "halal-graphify://stats":
            return _tool_graph_stats({})
        if uri_str == "halal-graphify://hub-nodes":
            return _tool_hub_nodes({"top_n": 10})
        if uri_str == "halal-graphify://surprises":
            try:
                from halal_graphify.analyze import surprising_connections
        ...
        if uri_str == "halal-graphify://audit":
            confs = [d.get("confidence", "EXTRACTED") for _, _, d in G.edges(data=True)]
            total = len(confs) or 1
            return (
                f"Total edges: {total}\n"
        ...
        if uri_str == "halal-graphify://questions":
            try:
                from halal_graphify.analyze import suggest_questions
    ...
    if hasattr(Server, "list_tools"):
        ...
        async def _on_read_resource(ctx, params) -> types.ReadResourceResult:
            ...
            mime = "text/markdown" if str(params.uri).startswith("halal-graphify://report") else "text/plain"
        ...
        try:
            ...
            _version = _pkg_version("halal-graphify")
        except Exception:
            _version = "0"
        server = Server(
            "halal-graphify",
    ...
    *,
    ...
    try:
        ...
        raise ImportError(
            ...
            'Run: pip install "halal-graphify[mcp]"'
        ) from e
...
def serve_http(
    ...
    *,
    ...
    print(
        f"halal-graphify MCP server (streamable-http) on http://{host}:{port}{path} - {auth_note}",
        file=sys.stderr,
    )
...
def _main(argv: list[str] | None = None) -> None:
    ...
    parser = argparse.ArgumentParser(
        prog="python -m halal_graphify.serve",
        description="Serve a halal-graphify knowledge graph over MCP (stdio or Streamable HTTP).",
    )
    parser.add_argument(
        "graph_path",
        nargs="?",
